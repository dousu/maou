...
from maou.domain.data.schema import (
    ...
    get_compressed_preprocessing_dtype,
    get_schema_info,
...
class TestCompressedPreprocessingSchema:
    ...
    def test_get_compressed_preprocessing_dtype(self) -> None:
        ...
        assert dtype["features_packed"].shape == (FEATURES_PACKED_SIZE,)
        assert str(dtype["features_packed"]).startswith("(")  # It's an array field
        assert dtype["moveLabel"] == np.dtype("uint16")
        assert dtype["resultValue"] == np.dtype("float16")
        assert dtype["legalMoveMask_packed"].shape == (LEGAL_MOVES_PACKED_SIZE,)
        assert str(dtype["legalMoveMask_packed"]).startswith("(")  # It's an array field
        assert "datetime64[D]" in str(dtype["partitioningKey"])

    def test_create_empty_compressed_preprocessing_array(self) -> None:
        """Test creating empty compressed preprocessing array."""
        size = 10
        array = create_empty_compressed_preprocessing_array(size)

        assert len(array) == size
        assert array.dtype == get_compressed_preprocessing_dtype()

        # Check that packed fields have correct shapes
        assert array["features_packed"].shape == (size, FEATURES_PACKED_SIZE)
        assert array["legalMoveMask_packed"].shape == (size, LEGAL_MOVES_PACKED_SIZE)
    ...
    def test_packed_field_sizes(self) -> None:
        ...
        expected_features_packed = (FEATURES_NUM * 9 * 9 + 7) // 8
        ...
        assert LEGAL_MOVES_PACKED_SIZE == expected_legal_moves_packed
...
class TestCompressedPreprocessingValidation:
    ...
    def test_validate_valid_compressed_array(self) -> None:
        ...
        array = create_empty_compressed_preprocessing_array(5)

        # Set valid values
        array["eval"] = [100, -100, 0, 1000, -1000]
        array["moveLabel"] = [0, 100, 500, 1000, MOVE_LABELS_NUM - 1]
        array["resultValue"] = [0.0, 0.25, 0.5, 0.75, 1.0]

        # Should not raise
        assert validate_compressed_preprocessing_array(array) is True

    def test_validate_invalid_dtype(self) -> None:
        """Test validating array with wrong dtype."""
        # Create array with wrong dtype
        wrong_array = np.zeros(5, dtype=[("id", "U128"), ("eval", "int16")])
    ...
    def test_validate_non_array(self) -> None:
        ...
        with pytest.raises(SchemaValidationError, match="Expected numpy ndarray"):
            validate_compressed_preprocessing_array("not an array")  # type: ignore

    def test_validate_eval_out_of_range(self) -> None:
        """Test validating array with eval values out of range."""
        array = create_empty_compressed_preprocessing_array(2)
        ...
        array["eval"][0] = -32768  # This should be caught by validation
    ...
    def test_validate_move_label_out_of_range(self) -> None:
        ...
        array = create_empty_compressed_preprocessing_array(2)
        array["moveLabel"] = [0, MOVE_LABELS_NUM]  # Second value is out of range
    ...
    def test_validate_empty_array(self) -> None:
        ...
        array = create_empty_compressed_preprocessing_array(0)
...
class TestSchemaInfo:
    ...
    def test_schema_info_includes_compressed_schema(self) -> None:
        """Test that schema info includes compressed preprocessing schema."""
        info = get_schema_info()

        # Check that compressed_preprocessing is included
        assert "compressed_preprocessing" in info

        # Check structure
        compressed_info = info["compressed_preprocessing"]
        assert "dtype" in compressed_info
        assert "description" in compressed_info
        assert "fields" in compressed_info

        # Check that it's described as compressed
        assert "compressed" in compressed_info["description"].lower()
        assert "8x" in compressed_info["description"]

        # Check field descriptions
        fields = compressed_info["fields"]
        assert "features_packed" in fields
        assert "legalMoveMask_packed" in fields

        # Check that packed field descriptions mention compression
        assert "bit-packed" in fields["features_packed"].lower()
        assert "bit-packed" in fields["legalMoveMask_packed"].lower()

    def test_schema_info_field_descriptions(self) -> None:
        """Test that compressed schema field descriptions are accurate."""
        info = get_schema_info()
        fields = info["compressed_preprocessing"]["fields"]
...
class TestMemoryUsage:
    ...
    def test_compressed_array_memory_usage(self) -> None:
        ...
        from maou.domain.data.schema import get_preprocessing_dtype

        size = 1000

        # Create standard and compressed arrays
        standard_array = np.zeros(size, dtype=get_preprocessing_dtype())
        compressed_array = create_empty_compressed_preprocessing_array(size)
    ...
    def test_packed_field_memory_savings(self) -> None:
        ...
        standard_features_bytes = size * FEATURES_NUM * 9 * 9  # uint8 per element
        packed_features_bytes = size * FEATURES_PACKED_SIZE

        features_ratio = standard_features_bytes / packed_features_bytes

        # Should be close to 8x compression
        assert 7.5 <= features_ratio <= 8.5

        # Calculate memory for legal moves field
        standard_legal_moves_bytes = size * MOVE_LABELS_NUM  # uint8 per element
        packed_legal_moves_bytes = size * LEGAL_MOVES_PACKED_SIZE

        legal_moves_ratio = standard_legal_moves_bytes / packed_legal_moves_bytes
...
class TestBackwardCompatibility:
    ...
    def test_standard_and_compressed_schemas_coexist(self) -> None:
