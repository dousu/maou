...
def _create_test_df(
    ...
    for idx, lv, wv in zip(label_indices, label_values, win_values):
...
class TestIntermediateStoreDualMerge:
    ...
    def test_bulk_upsert_with_win_values(self, tmp_path: Path) -> None:
        """New rows inserted with correct win values."""
        db_path = tmp_path / "test.duckdb"
        store = IntermediateDataStore(db_path=db_path, win_rate_threshold=2)
        try:
            df = _create_test_df(
                hash_id=100,
                count=3,
            ...
            assert win_rates[10] == pytest.approx(0.75, rel=1e-5)
            assert win_rates[20] == pytest.approx(0.5, rel=1e-5)
        finally:
            store.close()

    def test_bulk_upsert_conflict_merge(self, tmp_path: Path) -> None:
    ...
    def test_finalize_produces_move_win_rate(self, tmp_path: Path) -> None:
        ...
        try:
            ...
            assert result_df["moveWinRate"].dtype == pl.List(pl.Float32)
            assert len(result_df["moveWinRate"].to_list()[0]) == 1496
        finally:
            store.close()

    def test_finalize_produces_best_move_win_rate(self, tmp_path: Path) -> None:
        ...
        try:
            ...
            assert result_df["bestMoveWinRate"].dtype == pl.Float32

            # best = max(3.0/4=0.75, 1.0/2=0.5) = 0.75
            assert result_df["bestMoveWinRate"][0] == pytest.approx(0.75, rel=1e-5)
        finally:
            store.close()
