...
class TestFeaturesPacking:
    ...
    def test_pack_valid_features(self) -> None:
        ...
        features = np.random.choice([0, 1], size=FEATURES_SHAPE).astype(np.uint8)
    ...
    def test_pack_invalid_features_shape(self) -> None:
        ...
        with pytest.raises(CompressionError, match="Invalid features shape"):
    ...
    def test_pack_invalid_features_dtype(self) -> None:
        ...
        invalid_features = np.zeros(FEATURES_SHAPE, dtype=np.float32)

        with pytest.raises(CompressionError, match="Invalid features dtype"):
            pack_features_array(invalid_features)

    def test_pack_features_invalid_values(self) -> None:
        """Test packing features with invalid values."""
        invalid_features = np.full(FEATURES_SHAPE, 2, dtype=np.uint8)

        with pytest.raises(CompressionError, match="contains values other than 0 or 1"):
            pack_features_array(invalid_features)
    ...
    def test_unpack_invalid_packed_features_shape(self) -> None:
        ...
        with pytest.raises(CompressionError, match="Invalid packed features shape"):
    ...
    def test_unpack_invalid_packed_features_dtype(self) -> None:
        ...
        invalid_packed = np.zeros(FEATURES_PACKED_SIZE, dtype=np.float32)

        with pytest.raises(CompressionError, match="Invalid packed features dtype"):
            unpack_features_array(invalid_packed)
...
class TestLegalMovesPacking:
    ...
    def test_pack_valid_legal_moves(self) -> None:
        ...
        legal_moves = np.random.choice([0, 1], size=LEGAL_MOVES_SHAPE).astype(np.uint8)
    ...
    def test_unpack_legal_moves_roundtrip(self) -> None:
        ...
        # Create deterministic test data
        legal_moves = np.zeros(LEGAL_MOVES_SHAPE, dtype=np.uint8)
        legal_moves[0] = 1
        legal_moves[100] = 1
        legal_moves[-1] = 1
    ...
    def test_pack_invalid_legal_moves_shape(self) -> None:
        ...
        with pytest.raises(CompressionError, match="Invalid legal moves shape"):
    ...
    def test_pack_invalid_legal_moves_dtype(self) -> None:
        ...
        invalid_legal_moves = np.zeros(LEGAL_MOVES_SHAPE, dtype=np.float32)

        with pytest.raises(CompressionError, match="Invalid legal moves dtype"):
            pack_legal_moves_mask(invalid_legal_moves)

    def test_pack_legal_moves_invalid_values(self) -> None:
        """Test packing legal moves with invalid values."""
        invalid_legal_moves = np.full(LEGAL_MOVES_SHAPE, 2, dtype=np.uint8)

        with pytest.raises(CompressionError, match="contains values other than 0 or 1"):
            pack_legal_moves_mask(invalid_legal_moves)

    def test_unpack_invalid_packed_legal_moves_shape(self) -> None:
        """Test unpacking packed legal moves with invalid shape."""
        invalid_packed = np.zeros(100, dtype=np.uint8)

        with pytest.raises(CompressionError, match="Invalid packed legal moves shape"):
            unpack_legal_moves_mask(invalid_packed)

    def test_unpack_invalid_packed_legal_moves_dtype(self) -> None:
        """Test unpacking packed legal moves with invalid dtype."""
        invalid_packed = np.zeros(LEGAL_MOVES_PACKED_SIZE, dtype=np.float32)

        with pytest.raises(CompressionError, match="Invalid packed legal moves dtype"):
            unpack_legal_moves_mask(invalid_packed)
...
class TestPreprocessingRecordPacking:
    ...
    def test_pack_preprocessing_record(self) -> None:
        ...
        packed_features, packed_legal_moves = pack_preprocessing_record(record)

        assert packed_features.shape == (FEATURES_PACKED_SIZE,)
        assert packed_legal_moves.shape == (LEGAL_MOVES_PACKED_SIZE,)
        assert packed_features.dtype == np.uint8
        assert packed_legal_moves.dtype == np.uint8

    def test_unpack_preprocessing_fields_roundtrip(self) -> None:
        ...
        packed_features, packed_legal_moves = pack_preprocessing_record(record)
        unpacked_features, unpacked_legal_moves = unpack_preprocessing_fields(
            packed_features, packed_legal_moves
        )

        assert np.array_equal(record["features"], unpacked_features)
        assert np.array_equal(record["legalMoveMask"], unpacked_legal_moves)
...
class TestCompressionStats:
    ...
    def test_get_compression_stats(self) -> None:
        ...
        assert features_stats["original_size"] == FEATURES_NUM * 9 * 9
        assert features_stats["packed_size"] == FEATURES_PACKED_SIZE
        assert (
        ...
        assert legal_moves_stats["original_size"] == MOVE_LABELS_NUM
        assert legal_moves_stats["packed_size"] == LEGAL_MOVES_PACKED_SIZE
        assert (
        ...
        expected_original = FEATURES_NUM * 9 * 9 + MOVE_LABELS_NUM
        expected_packed = FEATURES_PACKED_SIZE + LEGAL_MOVES_PACKED_SIZE
        assert total_stats["original_size"] == expected_original
        assert total_stats["packed_size"] == expected_packed
        assert total_stats["compression_ratio"] == expected_original / expected_packed

    def test_compression_ratio_is_approximately_8x(self) -> None:
...
class TestEdgeCases:
    ...
    def test_sparse_data(self) -> None:
        ...
        unpacked_features = unpack_features_array(packed_features)
        assert np.array_equal(features, unpacked_features)

        # Test legal moves
        packed_legal_moves = pack_legal_moves_mask(legal_moves)
        unpacked_legal_moves = unpack_legal_moves_mask(packed_legal_moves)
