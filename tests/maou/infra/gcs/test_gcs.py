...
if skip_test:
    logger.debug(
        f"Skip gcs.py TEST_GCP: {os.getenv('TEST_GCP', '')}"
    )
...
class TestGCS:
    ...
    def default_fixture(self) -> None:
        ...
        self.bucket_name = (
            "maou-test-" + self.__calculate_file_crc32c(path)
        )
        logger.debug(f"Test Bucket: {self.bucket_name}")
        self.test_class = GCS(
            bucket_name=self.bucket_name, base_path="gcs"
        )
    ...
    def test_save_object(self, default_fixture: None) -> None:
        ...
        assert blob.exists()
        blob_content_str = blob.download_as_bytes().decode(
            "utf-8"
        )
        logger.debug(
            f"contents of object gcs/{cloud_path}: {blob_content_str}"
        )
        assert (
    ...
    def test_upload_folder(self, default_fixture: None) -> None:
        ...
        # アップロードされたファイルを確認
        blobs = list(
            self.bucket.list_blobs(prefix=f"gcs/{cloud_folder}")
        )
        ...
        test_blob = self.bucket.blob(
            f"gcs/{cloud_folder}/file1.txt"
        )
        assert test_blob.exists()
        blob_content_str = test_blob.download_as_bytes().decode(
            "utf-8"
        )
        assert (
