...
class TestStage2RecordRendererBoardReconstruction:
    """Stage2RecordRenderer の盤面再構築と表示フィールドのテスト．"""
    ...
    def test_create_board_from_record_initial_position(
        self, renderer: Stage2RecordRenderer
    ) -> None:
        """初期局面のレコードから正しくBoardが再構築されることを検証する．"""
        from maou.domain.board.shogi import Board

        board = Board()
        record = {
            "boardIdPositions": board.get_normalized_board_id_positions().tolist(),
            "piecesInHand": board.get_normalized_pieces_in_hand().tolist(),
        }

        reconstructed = renderer._create_board_from_record(
            record
        )

        assert (
            reconstructed.get_sfen().split(" ")[0]
            == board.get_sfen().split(" ")[0]
        )

    def test_create_board_from_record_falls_back_to_hirate(
        self, renderer: Stage2RecordRenderer
    ) -> None:
        """不正なレコードでは平手初期局面にフォールバックすることを検証する．"""
        from maou.domain.board.shogi import Board

        record: dict[str, Any] = {
            "boardIdPositions": [[99] * 9] * 9,
            "piecesInHand": [0] * 14,
        }
        ...
        assert reconstructed.get_sfen() == Board().get_sfen()
    ...
    def test_extract_display_fields_valid_usi(
        ...
        board_pos = board.get_normalized_board_id_positions()
        pieces_in_hand = board.get_normalized_pieces_in_hand()
    ...
    def test_white_turn_normalized_record_reconstructs(
        self, renderer: Stage2RecordRenderer
    ) -> None:
        """後手番の正規化済みレコードからBoardが再構築できることを検証する．"""
        from maou.domain.board.shogi import Board
        ...
        # get_normalized_board_id_positions は後手番なら180度回転
        record = {
            "boardIdPositions": board.get_normalized_board_id_positions().tolist(),
            "piecesInHand": board.get_normalized_pieces_in_hand().tolist(),
        }
        ...
        board_part = reconstructed.get_sfen().split(" ")[0]
        assert any(
