...
class TestIntegrationConverterPreprocess:
    ...
    def default_fixture(self) -> Generator[None, None, None]:
        ...
        self.table_name_hcpe = (
            "test_" + self.__calculate_file_crc32c(hcpe_path)
        )
        self.table_name_preprocess = (
            "test_"
            + self.__calculate_file_crc32c(preprocess_path)
        )
        logger.debug(
            f"Test table (HCPE): {self.dataset_id}.{self.table_name_hcpe}"
        )
        logger.debug(
    ...
    def test_file_datasource_local_bq(
        ...
        output_paths_hcpe = [
            hcpe_option.output_dir
            / input_path.with_suffix(".npy").name
            for input_path in input_paths
        ]

        preprocess_option: PreProcess.PreProcessOption = (
        ...
        transformer = PreProcess(
            ...
            feature_store=feature_store_preprocess,
        )
    ...
    def test_bq_datasource(
        ...
        HCPEConverter(feature_store=feature_store_hcpe).convert(
            hcpe_option
        )
