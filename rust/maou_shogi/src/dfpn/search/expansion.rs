...
pub(crate) struct LocalExpansion {
    ...
    /// AND node の透過中合いマス集合 (build 時設定; 未設定/OR node は空)．AND-proven の
...
impl LocalExpansion {
    ...
    /// idx 全体を comparer で sort する (stable sort; 完全同点 tie は movegen 順を保つ)．
    ...
    /// 透過中合いマス集合を設定する (build 後に呼ぶ; AND-proven mate_len 集計で使用)．
    #[inline]
    ...
    ///
    /// proof/disproof hand は hand-set 極小化 (`before_hand` / `ProofHandSet` /
    /// `DisproofHandSet`) で計算し，cross-hand TT の再利用を効かせる．
        ...
        if self.get_phi() == 0 {
            ...
            for &ir in &self.idx {
                ...
                if self.or_node {
                    ...
                    // AND-proven max-resistance．透過中合い (chain マス) への合駒 drop 子は
    ...
    /// 子に渡す (pn, dn) 閾値．
    ///
    /// 子 φ 予算は `2nd_phi + 2nd_phi/8 + 1` (ノード内 1+ε, ε=1/8)．標準式 `2nd_phi + 1`
    /// より best child の切替 (seesaw) と閾値反復の再展開が減る．
