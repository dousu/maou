...
/// Df-Pn ソルバー．
/// 探索チューニングパラメータ群 (旧 DfPnSolver の param_* フィールドを集約)．
...
pub(super) struct Params {
    ...
    pub(super) use_visit_history_dominance: bool,
    ...
    ///
    /// デフォルトは false．
    pub(super) use_handset_combination: bool,
    ...
    pub(super) use_delayed_move_list: bool,
    ...
    /// default OFF．
    pub(super) comparer_and_only: bool,
    ...
    pub(super) is_sum_delta_node: bool,
    ...
    pub(super) minimal_proof_hand: bool,
    ...
    pub(super) minimal_disproof_hand: bool,
    ...
    pub(super) deferred_penalty_denom: u32,
    /// deferred penalty `.max(1)` floor．
    pub(super) deferred_penalty_floor: bool,
    /// TCA gate を is_shallow ベースにするか (true=is_shallow, false=!is_first_visit)．
    pub(super) tca_use_shallow_gate: bool,
    ...
    ///   ...
    /// ```
    pub(super) root_trace: bool,

    /// 診断: trace 対象の ply (default 0 = root)．
    pub(super) trace_ply: u32,

    /// 診断: 全 path 上の child の pn/dn を 1 dump 当たり最大何件出すか．
    pub(super) trace_full_children: bool,
    ...
    pub(super) use_exhaustive_and: bool,
    ...
    pub(super) use_and_proven_bitmap: bool,
    ...
    pub(super) root_child_pn_floor: u32,
    ...
    pub(super) disable_periodic_gc: bool,
    ...
    pub(super) skip_ids_shallow: bool,
    ...
    pub(super) use_per_move_support: bool,
    ...
    pub(super) refut_or_success_cache: bool,
    ...
    pub(super) refut_extended_lookup: bool,
    /// IDS-17 無効化フラグ．
    /// true にすると saved_depth 20-26 での depth=16→17 挿入をスキップする．
    /// デフォルト false = IDS-17 有効 (depth=17 を明示的に経由)．
    pub(super) no_ids17: bool,
    /// refutable check の再帰深さ (デフォルト 5)．
    pub(super) refutable_depth: u32,
    /// refutable check の呼び出し回数上限 (デフォルト 10,000)．
    pub(super) refutable_call_limit: u32,
    /// PNS 初期フェーズ専用の refutable call limit (デフォルト 500)．
    /// MID では param_refutable_call_limit (=10,000) を使用する．
    pub(super) pns_refutable_call_limit: u32,
}

/// 探索診断・計装カウンタ群 (旧 DfPnSolver の diag_* フィールドを集約)．
#[allow(clippy::struct_excessive_bools)]
pub(super) struct Diagnostics {
    /// サマリキャッシュの disproof ヒット回数 (tt_diag 診断用)．現在は未使用．
    #[cfg(feature = "tt_diag")]
    #[allow(dead_code)]
    pub(super) pc_summary_disproof_hits: u64,
    /// 診断: deferred penalty 発火フレーム数．
    pub(super) deferred_frames: u64,
    /// 診断: deferred penalty 合計値．
    pub(super) deferred_penalty_sum: u64,
    /// 診断: has_old_child=true (is_shallow gate)．
    pub(super) tca_shallow_fire: u64,
    /// 診断: has_old_child=false だが旧ロジックでは true になるケース．
    pub(super) tca_shallow_would_fire: u64,
    // Cell で `&self` の look_up_pn_dn からも更新可能にする
    pub(super) tt_lookups: std::cell::Cell<u64>,
    pub(super) tt_misses: std::cell::Cell<u64>,
    pub(super) tt_proven: std::cell::Cell<u64>,
    pub(super) tt_disproven: std::cell::Cell<u64>,
    pub(super) tt_working: std::cell::Cell<u64>,

    /// 診断: per-depth proven 蓄積カウンタ．
    /// `store_proof_with_tag` 呼び出し時に `path_len` (= 現在の ply) を
    /// インデックスとして increment．solve() 完了後に proof がどの深さで
    /// 累積したかのヒストグラムが取れる．
    pub(super) proven_per_ply: [u64; 64],
    ...
    pub(super) and_visit_count: u64,
    pub(super) and_proven_sum: u64,  // proven_count の合計
    pub(super) and_total_sum: u64,   // total_children の合計
    pub(super) and_zero_proven: u64, // proven_count == 0 で exit した回数
    pub(super) and_full_proven: u64, // proven_count == total で exit (本来は ProveAND して exit)

    /// 診断: OR scan coverage 統計 (AND と対称)．
    pub(super) or_visit_count: u64,
    pub(super) or_proven_count_visits: u64, // proven child があった visits の数
    pub(super) or_total_sum: u64,

    /// 診断: per-position revisit count．`(pos_key, or_node) → visit_count`．
    /// max 1M entries に制限してメモリ暴走防止．
    pub(super) pos_visits: rustc_hash::FxHashMap<u64, u32>,
    pub(super) pos_visits_capped: bool, // 1M 超えたら collection 停止
    ...
    #[cfg(feature = "tt_diag")]
    #[allow(dead_code)]
    pub(super) alpha_x_filter_applied: u64,
}
...
impl DfPnSolver {
    ...
    pub fn set_pns_arena_max(&mut self, max_nodes: usize) {
        self.params.pns_arena_max = max_nodes.max(1024);
    }
    ...
    pub fn set_use_visit_history_dominance(&mut self, on: bool) {
        self.params.use_visit_history_dominance = on;
    }
    ...
    pub fn set_use_handset_combination(&mut self, on: bool) {
        self.params.use_handset_combination = on;
    }
    ...
    pub fn set_use_delayed_move_list(&mut self, on: bool) {
        self.params.use_delayed_move_list = on;
    }
    ...
    pub fn set_disable_periodic_gc(&mut self, on: bool) -> &mut Self {
        self.params.disable_periodic_gc = on;
        self
    }
    ...
    pub fn set_skip_ids_shallow(&mut self, on: bool) -> &mut Self {
        self.params.skip_ids_shallow = on;
        self
    }
    ...
    pub fn get_tt_lookup_stats(&self) -> (u64, u64, u64, u64, u64) {
        (
            self.diag.tt_lookups.get(),
            self.diag.tt_misses.get(),
            self.diag.tt_proven.get(),
            self.diag.tt_disproven.get(),
            self.diag.tt_working.get(),
        )
    }
    ...
    pub fn set_obvious_final_max_depth(&mut self, d: u32) -> &mut Self {
        self.params.obvious_final_max_depth = d;
        self
    }
    ...
    pub fn set_is_sum_delta_node(&mut self, on: bool) -> &mut Self {
        self.params.is_sum_delta_node = on;
        self
    }
    ...
    pub fn set_minimal_proof_hand(&mut self, on: bool) -> &mut Self {
        self.params.minimal_proof_hand = on;
        self
    }
    ...
    pub fn set_minimal_disproof_hand(&mut self, on: bool) -> &mut Self {
        self.params.minimal_disproof_hand = on;
        self
    }
    ...
    pub fn get_phase21_diag(&self) -> (u64, u64, u64, u64) {
        (
            self.diag.deferred_frames,
            self.diag.deferred_penalty_sum,
            self.diag.tca_shallow_fire,
            self.diag.tca_shallow_would_fire,
        )
    }
    ...
    pub fn set_root_child_pn_floor(&mut self, floor: u32) -> &mut Self {
        self.params.root_child_pn_floor = floor;
        self
    }
    ...
    pub fn get_and_coverage_stats(&self) -> (u64, u64, u64, u64, u64) {
        (
            self.diag.and_visit_count,
            self.diag.and_proven_sum,
            self.diag.and_total_sum,
            self.diag.and_zero_proven,
            self.diag.and_full_proven,
        )
    }
    ...
    pub fn get_or_coverage_stats(&self) -> (u64, u64, u64) {
        (
            self.diag.or_visit_count,
            self.diag.or_proven_count_visits,
            self.diag.or_total_sum,
        )
    }
    ...
    pub fn get_pos_visit_stats(&self) -> (usize, u64, bool, Vec<u32>) {
        let unique = self.diag.pos_visits.len();
        let total: u64 = self.diag.pos_visits.values().map(|&v| v as u64).sum();
        let capped = self.diag.pos_visits_capped;
        let mut counts: Vec<u32> = self.diag.pos_visits.values().copied().collect();
    ...
    pub fn get_pos_visit_histogram(&self) -> [u64; 24] {
        let mut h = [0u64; 24];
        for &count in self.diag.pos_visits.values() {
    ...
    pub fn set_use_exhaustive_and(&mut self, on: bool) -> &mut Self {
        self.params.use_exhaustive_and = on;
        self
    }
    ...
    pub fn set_use_and_proven_bitmap(&mut self, on: bool) -> &mut Self {
        self.params.use_and_proven_bitmap = on;
        self
    }

    pub fn set_use_dag_correction(&mut self, on: bool) {
        self.params.use_dag_correction = on;
    }
    ...
    pub fn set_use_per_move_support(&mut self, on: bool) {
        self.params.use_per_move_support = on;
    }
    ...
    pub fn set_disproof_remaining_threshold(&mut self, threshold: u16) {
        self.params.disproof_remaining_threshold = threshold;
    }
    ...
    pub fn enable_adaptive_disproof_remaining_threshold(&mut self) {
        self.params.disproof_remaining_threshold = DISPROOF_THRESHOLD_ADAPTIVE;
    ...
    pub fn set_refine_iter_cap(&mut self, cap: u64) -> &mut Self {
        self.params.refine_iter_cap = cap;
        self
    }
    ...
    pub fn set_refutable_params(&mut self, depth: u32, call_limit: u32) -> &mut Self {
        self.params.refutable_depth = depth;
        self.params.refutable_call_limit = call_limit;
        self
    }
    ...
    pub fn set_pns_refutable_call_limit(&mut self, limit: u32) -> &mut Self {
        self.params.pns_refutable_call_limit = limit;
        self
    }
    ...
    pub fn set_refut_full_eval(&mut self, enable: bool) -> &mut Self {
        self.params.refut_full_eval = enable;
        self
    }
    ...
    pub fn set_refut_partial_recursion(&mut self, enable: bool) -> &mut Self {
        self.params.refut_partial_recursion = enable;
        self
    }
    ...
    pub fn set_refut_or_success_cache(&mut self, enable: bool) -> &mut Self {
        self.params.refut_or_success_cache = enable;
        self
    }
    ...
    pub fn set_refut_extended_lookup(&mut self, enable: bool) -> &mut Self {
        self.params.refut_extended_lookup = enable;
        self
    }
    ...
    pub fn set_tt_diag(&mut self, ply: u32, move_usi: &str, max_iterations: u32) -> &mut Self {
        self.diag.ply = ply;
        self.diag.move_usi = move_usi.to_string();
        self.diag.max_iterations = max_iterations;
        self
    }
