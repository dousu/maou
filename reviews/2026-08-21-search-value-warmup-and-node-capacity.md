...
- **L4 の A/B は `identical: False` だった**．DevContainer で確認した
  ...
  **【2026-08-21 訂正 — 上の非一致の機序として当初書いた診断は誤りだった】**
  当初ここには「playouts 800 では dfpn が MCTS と並走するので，確保が速く
  なったぶん同じ壁時計の中でより深く詰みを証明し，木が変わる (意図した
  挙動)」と書いていた．**実測がこれを支持しない**．L4 で同一設定を 2 回
  走らせた対照 (`nc_new` / `nc_new2`) を取ったところ:

  - 50 行中 **49 行が完全一致**，差の出た 1 行も `|Δ| = 0.0235`
  - **`root_proven` の 4 行は 3 本すべてで完全一致** (0.0 / 0.0 / 1.0 / 1.0)．
    `stop` の不一致は 0 件 — **dfpn が決めた行はまったく揺れていない**
  - 揺れたのは `playout_limit` 行の playout 数 (812 / 695 / 849 / 811)．
    予算判定が `playouts + terminal_backprops` の**合計**なので，
    空回りの本数がスレッドタイミングで変わる

  したがって非一致の正体は**run 間のタイミング依存**であって，dfpn が
  深くなったからではない．**同じノイズが `identical: False` を説明する**．

- ~~既存の 1.25M 行は**プール導入前 (= OFF 相当) の出力**なので，今後 ON で
  積むと教師データが異種混合になる~~ **【2026-08-21 撤回】** 上の対照により，
  OFF/ON の差は run 間ノイズの内側にあることが分かった．混ぜてよい
  (`±0.02` が 2% の行に乗るだけで，0/1 → 探索値という本来の変更に対して
  無視できる)
