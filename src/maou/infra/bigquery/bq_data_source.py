...
class BigQueryDataSource(learn.DataSource):
    ...
    def __init__(
        ...
        *,
        ...
        Args:
            dataset_id (str): BigQuery データセット名
            table_name (str): BigQuery テーブル名
    ...
    def __get_total_rows(self) -> int:
        ...
        if meta_num_rows is None:
            query = (
                f"SELECT COUNT(*) AS total FROM `{self.dataset_fqn}.{self.table_name}`"
            )
            result = self.client.query(query).result()
    ...
    def __get_page(self, page_num: int) -> pa.Table:
        """
    ...
    def __getitem__(self, idx: int) -> dict[str, Any]:
        ...
        if self.clustering_key:
            ...
            if group_idx is None:
                raise IndexError(f"Index {idx} cannot be mapped to a cluster group.")
            page_table = self.__get_page(group_idx)
            ...
            page_table = self.__get_page(page_num)
            row_table = page_table.slice(row_offset, 1)

        row_dict = row_table.to_pydict()
        return {col: vals[0] for col, vals in row_dict.items()}
    ...
    def iter_batches(self) -> Generator[pa.Table, None, None]:
        ...
        """
        if self.clustering_key:
            total_pages = len(self.__cluster_info)
        else:
            total_pages = (self.total_rows + self.batch_size - 1) // self.batch_size
        for page_num in range(total_pages):
            yield self.__get_page(page_num)
