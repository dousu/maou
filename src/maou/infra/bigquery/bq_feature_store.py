...
class BigQueryFeatureStore(
    ...
    def __init__(
        ...
        *,
        ...
        try:
            ...
            self.logger.debug(
                f"Dataset '{dataset_fqn}' has been created."
            )
        # 特徴量書き込み周りの設定
        # バッファを初期化
    ...
    def __create_table_if_not_exists(
        ...
        *,
        ...
        try:
            ...
            self.logger.debug(
                f"Table '{table.full_table_id}' already exists."
            )
        except Exception:
            self.logger.debug(
                f"Table '{table_id}' not found. Creating a new table..."
            )

            table = bigquery.Table(
                table_ref=table_id, schema=schema
            )
            ...
            if partitioning_key_date is not None:
                table.time_partitioning = (
                    bigquery.TimePartitioning(
                        ...
                        field=partitioning_key_date,
                    )
                )
                table.require_partition_filter = True
            table = self.client.create_table(table=table)
            self.logger.debug(
                f"Table '{table.full_table_id}' has been created."
            )
    ...
    def flush_features(
        ...
        *,
        ...
        self.logger.debug(
            f"Target table: {table.full_table_id}"
        )
        # 既存のテーブルのスキーマが
        # 追加しようとしているデータのスキーマと一致しているかチェックする
        if set(table.schema) != set(schema):
            self.logger.error(
                f"スキーマの不一致: {table.schema}, {schema}"
            )
    ...
    def __cleanup(self) -> None:
        ...
        # bufferが空のときはスキップする
        if (
            self.last_key_columns is not None
            and self.__buffer_size != 0
        ):
            self.flush_features(
                key_columns=self.last_key_columns,
