...
class S3(learn.CloudStorage):
    ...
    def __init__(
        self, *, bucket_name: str, base_path: str, region: str = "ap-northeast-1"
    ):
    ...
    def upload_from_local(self, *, local_path: Path, cloud_path: str) -> None:
        ...
        self.logger.debug(f"Uploaded {local_path} to s3://{self.bucket_name}/{s3_path}")
    ...
    def upload_folder_from_local(
        ...
        *,
        ...
        if not local_folder.is_dir():
            raise ValueError(f"Path `{local_folder}` is not a directory.")
        ...
        for file_path in files:
            ...
            # ファイルをアップロード
            self.upload_from_local(local_path=file_path, cloud_path=cloud_path)
