...
2. **`moveWinRate` は勝率の軸に載る必要がある**．
   当時の `learn-model --value-target-mode best-move-win-rate`
   (2026-08-04 に削除，§5 参照) は
...
- `--win-rate-fallback raw-outcome`: 平滑化なしの実勝敗で記録する
...
(オプションは 2026-08-04 に `--win-rate-fallback {neutral,raw-outcome}` へ改名)．
...
| `max(moveWinRate)` | **却下** (2026-08-04) | 下記の通り上方バイアスを持つ |
| 加重平均 | 見送り | resultValueと近い値になり変更の意味が薄い |

#### 追記 (2026-08-04): `max(moveWinRate)` を選択肢から削除した

「将来検討」としていた `max(moveWinRate)` は `--value-target-mode
best-move-win-rate` として一時的に既定値になっていたが，**削除した**．
value 教師は `resultValue` 固定である．

理由は **`max` が複数のノイズ推定の最大値であり，構造的に上方バイアスを
持つ (winner's curse)** こと．真の勝率が全手 0.500 の局面でも，
出現回数と異手数によっては教師が 0.59〜0.62 になる (シミュレーション):
...
実際，学習済みモデルの平手初期局面の勝率は 0.61〜0.71 と，真値 (~0.5) から
この帯の分だけ上振れしていた．

`max` は粒度の面でも `resultValue` に劣る．出現 1 回の局面では両者とも
0.0/1.0 で同一だが，出現 2 回以上では `resultValue = win_count / count` が
実測勝率をそのまま返すのに対し，`max(moveWinRate)` は Beta 平滑化で
0.5 方向に圧縮される．とくに **出現 2 回で 1 勝 1 敗の局面 (真値 0.5) が
`max` では 1.000 になる**という病理があった．
