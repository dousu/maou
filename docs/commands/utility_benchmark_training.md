...
| `--input-cache-mode {file,memory,mmap}` | default `file` | In-memory layout for local inputs. **Both modes load every file into RAM**; the difference is that `file` keeps one array per input file while `memory` concatenates them into a single array (removing one `searchsorted` per sample, at the cost of a transient 2x memory spike during concatenation). This is not a residency knob — to avoid holding the dataset, use the streaming path instead. `mmap` is **deprecated** and internally converted to `file`.【F:src/maou/infra/file_system/file_data_source.py†L321-L436】【F:src/maou/infra/console/utility.py†L469-L475】 |
...
| `--gradient-accumulation-steps INT` | Gradient accumulation | Number of gradient accumulation steps (default `1`). Effective batch size = batch_size × steps. Ignored when `--adaptive-batch` is enabled. Mirrors the `learn-model` option so benchmark results reflect the target training configuration. |
| `--adaptive-batch`, `--adaptive-batch-min-steps INT`, `--adaptive-batch-max-steps INT`, `--adaptive-batch-interval INT`, `--adaptive-batch-smoothing FLOAT`, `--adaptive-batch-measurement-interval INT` | Adaptive batch (GNS) | Enables Gradient Noise Scale based dynamic adjustment of accumulation steps during the benchmarked epoch, using the same `AdaptiveBatchConfig` construction rules as `learn-model`. Only applies to Stage 3 (policy+value); Stage 1/2 use the fixed `--gradient-accumulation-steps`. |
...
| Loss & optimizer knobs (`--policy-loss-ratio`, `--value-loss-ratio`, `--policy-target-mode`, `--learning-ratio`, `--momentum`, `--optimizer`, `--optimizer-beta1`, `--optimizer-beta2`, `--optimizer-eps`) | Fine-tune optimization math | Range-checked before populating the benchmark config so unstable hyperparameters never reach the loop. `--policy-target-mode` accepts `move-label`, `win-rate` (default), or `weighted`.【F:src/maou/infra/console/utility.py†L617-L683】【F:src/maou/interface/utility_interface.py†L255-L329】 |
...
| `--vit-embed-dim INT`, `--vit-num-layers INT`, `--vit-num-heads INT`, `--vit-mlp-ratio FLOAT`, `--vit-dropout FLOAT` | ViT architecture overrides | Override Vision Transformer parameters (embedding dimension, encoder layers, attention heads, MLP ratio, dropout). Only effective when `--model-architecture=vit`. Defaults match `VisionTransformerConfig`: embed_dim=512, num_layers=6, num_heads=8, mlp_ratio=4.0, dropout=0.1. |
| `--gradient-checkpointing` | Memory optimization | Enable gradient checkpointing to reduce activation memory. Only effective with ViT architecture. Recommended for large batch sizes. |
| `--freeze-backbone` | Layer freezing | Freeze backbone parameters (embedding, backbone, pool, hand projection). Policy and value heads remain trainable. Optimizer is recreated after freezing to exclude frozen parameters. |
| `--trainable-layers INT` | Partial freezing | Number of trailing backbone layer groups to keep trainable. `0` = freeze all backbone layers. Unset = no freezing. Takes precedence over `--freeze-backbone` when both are specified. |
| `--stage1-batch-size INT` | Stage 1 batch size | Batch size override for Stage 1 benchmark. Defaults to `--batch-size` when unset. Applies to both map-style DataLoader. |
| `--stage2-batch-size INT` | Stage 2 batch size | Batch size override for Stage 2 benchmark. Defaults to `--batch-size` when unset. Applies to both map-style DataLoader and streaming `StreamingStage2Dataset`. |
