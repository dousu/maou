...
| Loss & optimizer knobs (`--gce-parameter`, `--policy-loss-ratio`, `--value-loss-ratio`, `--policy-target-mode`, `--value-target-mode`, `--learning-ratio`, `--momentum`, `--optimizer`, `--optimizer-beta1`, `--optimizer-beta2`, `--optimizer-eps`) | Fine-tune optimization math | Range-checked before populating the benchmark config so unstable hyperparameters never reach the loop. `--policy-target-mode` accepts `move-label`, `win-rate` (default), or `weighted`. `--value-target-mode` accepts `result-value` or `best-move-win-rate` (default).【F:src/maou/infra/console/utility.py†L617-L683】【F:src/maou/interface/utility_interface.py†L255-L329】 |
