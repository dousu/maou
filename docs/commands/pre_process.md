...
| --- | --- | --- |
| `--process-max-workers INT` | `8` | Caps CPU workers dedicated to preprocessing. Negative values raise `ValueError`; omitting the flag defaults to `min(8, cpu_count)`. A100 High Memory (12 cores) 向けに最適化．【F:src/maou/infra/console/pre_process.py†L540-L575】【F:src/maou/interface/preprocess.py†L47-L71】 |
