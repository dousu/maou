...
| `--search-value-path PATH` | optional | [`maou utility search-values`](utility_search_values.md) の出力 feather．該当する局面の `resultValue` を対局結果由来の値から**探索値**へ差し替える．対局結果は 1 対局の約 110 局面すべてで同じ値になるため「どの対局か」を思い出す近道が成立するが，探索値は局面ごとに異なるのでその近道が効かなくなる．出力に無い局面は対局結果由来のまま残るので**部分適用できる**．差し替えた行数は結果 JSON の `search_values_applied` に出る．背景は [docs/design/training-quality/](../design/training-quality/index.md) §3.3． |
