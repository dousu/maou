...
| `--position-count-threshold INT` | `2` | 指し手別勝率計算の最小出現回数閾値．局面の出現回数(`count`)がこの値未満の場合，`moveWinRate` と `bestMoveWinRate` を `--best-move-win-rate-fallback` で選んだフォールバック値に置換する．推奨範囲: 2〜4．【F:src/maou/infra/console/pre_process.py†L251-L258】 |
...
| `--best-move-win-rate-fallback {uniform,raw-outcome}` | `uniform` | 閾値未満局面での勝率フォールバック値．`uniform`(デフォルト)は中立値0.5，`raw-outcome` は平滑化なしの実勝敗(`win_values/label_values`)で，出現1回の局面では0.0/1.0(引き分けは0.5)になる．`moveWinRate` 配列と `bestMoveWinRate` の双方に適用され，`bestMoveWinRate` は常に配列の最大値である．`--drop-below-threshold` 指定時は無視される．【F:src/maou/infra/console/pre_process.py†L269-L279】 |
