...
| `--value-target-mode {result-value,best-move-win-rate}` | `result-value` | Value教師信号モード．`result-value`=局面の勝率(resultValue = win_count / count)，`best-move-win-rate`=最善手の勝率(moveWinRateの最大値)．`best-move-win-rate`モードにはpreprocessパイプラインで`moveWinRate`カラムが生成されたデータが必要．|
