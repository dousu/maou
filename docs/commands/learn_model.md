...
| `--stage3-batch-size INT` | `None` (inherits `--batch-size`) | Batch size for Stage 3 (policy+value). When unset, inherits the global `--batch-size` (which itself defaults to `4096` in multi-stage mode). Lower this to reduce Stage 3 streaming DataLoader worker memory when workers are OOM-killed (`DataLoader worker ... killed by signal: Killed`).【F:src/maou/infra/console/learn_model.py†L460-L468】 |
