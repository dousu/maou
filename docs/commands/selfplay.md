...
| `--time-curve-peak-ply INT` | default `55` | Ply where the midgame time curve peaks (`--ab-mode timecurve`). |
| `--time-curve-half-width-ply INT` | default `35` | Plies from the peak down to the floor weight. |
| `--time-curve-peak-permille INT` | default `1800` | Multiplier at the peak, per mille. Scales **only the discretionary share** (`remaining / horizon`), never byoyomi or the Fischer increment. |
