...
探索アーキテクチャ・最適化手法の詳細は
[docs/design/tsume-solver/](design/tsume-solver/index.md) を参照．
...
result.best_mate       # unknown (minimality_unconfirmed) でも残った検証済み詰み手順
                       #   (USI 形式; checkmate 時は空で手順は moves 側)．最短保証なし
...
- `stop_reason == "minimality_unconfirmed"`: **詰み自体は検証済** (`mate_len_found` に
  ...
  になる最有力ケース．**このとき `result.best_mate` に，そこまでに見つけた検証済みの詰み
  手順 (USI 形式) が入る**ため，最短を確定しきれず打ち切っても大きなリソースを費した探索の
  成果 (詰み手順そのもの) を回収できる (最短である保証はない; `mate_len_found` と同じ手数)．
...
if r.status == "unknown":
    ...
    if r.best_mate:               # 最短未確定でも見つかった詰み手順を回収 (最短保証なし)
        print("best mate:", " ".join(r.best_mate))
...
let result = solve_tsume_with_timeout(
    ...
    None,            // pv_nodes_per_child (default: 1024)
    None,            // tt_gc_threshold (default: 0 = 無効)
)?;

// ソルバーを直接構築
let mut solver = DfPnSolver::with_timeout(31, 1_048_576, 300);
