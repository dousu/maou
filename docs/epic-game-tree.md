...
BFS中の盤面状態はcshogiの`Board`オブジェクトをキュー内で管理する．
各ノード処理時に「ルートからの指し手列(move path)」を保持し，
初期局面から指し手を順に`board.push(move)`して復元する．

```python
# BFSキューの要素: (zobrist_hash, move_path: list[int])
queue: deque[tuple[int, list[int]]] = deque([(initial_hash, [])])
...
preprocessデータの`boardIdPositions`からの盤面復元(SFEN経由)は不要．
BFSは初期局面から指し手を積み上げるため，盤面状態は常に正確に維持される．
...
5. **Python連携(Gradio 6.0+ `gr.HTML`のイベント機構):**
   ...
   ```javascript
   // game_tree.js: Cytoscape.jsノードクリック時
   cy.on('tap', 'node', function(evt) {
       const nodeId = evt.target.id();
       // Gradioのhidden Textboxを取得してvalueを更新
   ...
   gr.HTML (Cytoscape.js)
       │ tap event
       ▼
   JS: document.querySelector('#selected-node-id textarea')
       │ .value = nodeId + dispatchEvent('input')
       ▼
   gr.Textbox (hidden, elem_id="selected-node-id")
       │ .change() event
       ▼
   Python callback: on_node_selected(node_id, tree_data)
       │
       ├──→ gr.HTML (盤面SVG)
       ├──→ gr.JSON (局面統計)
       ├──→ gr.Dataframe (指し手一覧)
