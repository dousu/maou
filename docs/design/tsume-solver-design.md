...
```
// 自然精度パターン: PN_UNIT > 1 で epsilon が増大する
...
S = PN\_UNIT（現在 1，拡大候補: 16）．

*初期値(子ノード展開時に TT に格納される値):*

| 値 | S 倍率 | 適用対象 | 条件 | 節 |
|---|-------|---------|------|---|
| S | 1 | OR 子 pn | 標準局面，逃げ場 1〜3 | §5.1 |
| S〜1.5S | 1〜1.5 | OR 子 pn | 逃げ場 4+(PN\_UNIT > 1 で中間値可能) | §5.1 |
| 2S〜3S | 2〜3 | OR 子 pn | 王手少＋逃げ場多，開放空間 | §5.1 |
| n×S | n | AND 子 pn | 応手数 n ベース(逃げ場で 0.67n〜1.5n に調整) | §5.1 |
...
**方針 A: PN\_UNIT 拡大 + heuristic 高解像度化 + 自然精度 epsilon**
...
**方針 B: IDS フルデプスの動的予算配分**

現在は MID 1/2 → Frontier 1/2 の固定分割．
MID が新しい TT エントリを生成しなくなった時点で停滞と判定し，
残り予算を動的に Frontier に回す方式．
方針 A で MID の到達範囲が拡大すれば，Frontier の予算が減っても
十分な可能性がある．方針 A と組み合わせて効果を検証する．
