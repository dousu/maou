...
| 46 | ProvenTT hand\_hash 混合インデクシング | — | §6.6.4 | v0.24.0 |
| 47 | 2段階検索 (look\_up\_proven / look\_up\_working) | — | §6.6.4 | v0.24.0 |
| 48 | 段階的 retain\_proofs (confirmed disproof 保持) | — | §6.6.4 | v0.24.0 |
...
#### 6.6.3 Dual TT (v0.24.0)

v0.24.0 でクラスタ飽和問題(§6.6.1)の構造的解決を目指し，
単一の TranspositionTable を **ProvenTT + WorkingTT** に分離した．

**構成:**

| テーブル | 内容 | クラスタ | エントリ/クラスタ | メモリ |
|---------|------|--------|-----------------|--------|
| ProvenTT | proof (pn=0) + confirmed disproof | 2M | 4 | 256 MB |
| WorkingTT | intermediate + depth-limited disproof | 2M | 6 | 384 MB |
| 合計 | — | — | — | 640 MB |

**エントリ圧縮 (v0.24.0):**

TTFlatEntry を 40→32 bytes に圧縮:

| フィールド | 変更 | 節約 |
|-----------|------|------|
| source | u64→u32 | -4B |
| amount | u16→u8 | -1B |
| path\_dependent + remaining | remaining\_flags: u16 に pack | -1B |

DfPnEntry: 32→24 bytes，TTFlatEntry (pos\_key u64 + DfPnEntry): 40→32 bytes．

**lookup フロー:**

```
Pass 1: ProvenTT — proof(pn=0) → early return (hand_gte)
Pass 2: ProvenTT — confirmed disproof(dn=0) → hand_gte + remaining check
Pass 3: WorkingTT — depth-limited disproof + exact_match
```

**GC 戦略:**

| メソッド | ProvenTT | WorkingTT |
|---------|---------|-----------|
| retain\_proofs() | そのまま | 全クリア |
| retain\_proofs\_only() | confirmed disproof 除去 | 全クリア |
| clear\_proven\_disproofs() | confirmed disproof 除去 | そのまま |
| gc\_by\_amount() | そのまま | amount ベース除去 |

`retain_proofs()` は Frontier サイクルで呼ばれ，WorkingTT を全クリアする．
ProvenTT の proof は全フェーズを通じて永続する．

**IDS depth 切り替え時の NoMate バグ対策 (v0.24.0):**

IDS の浅い depth で REMAINING\_INFINITE として格納された confirmed disproof が，
深い depth の探索で偽の不詰判定(NoMate)を引き起こす問題を発見・修正した．

再現: 39手詰め ply 22 で depth=19→Mate(23)，depth=21→NoMate(偽陽性)．

修正: `clear_proven_disproofs()` を IDS depth 切り替え時に呼び出し，
ProvenTT の confirmed disproof を除去する．Proof (pn=0) は影響を受けない．

**ProvenTT 置換ポリシー:**

ply ベースの amount: `proof.amount = 255 - ply`，`disproof.amount = 128 - ply`．
ルートに近い proof ほど高い amount を持ち，eviction 耐性が高い．
replace\_weakest\_proven は lowest amount を evict するが，
新エントリの amount が既存最弱以上の場合のみ置換する（ルート近傍保護）．

**祖先チェックによる ProvenTT 挿入スキップ (v0.24.0):**

proof (pn=0) を store する際，`path` 配列（ルートからの祖先チェーン）を遡り，
祖先の pos\_key + hand が既に ProvenTT に proof を持つ場合は **挿入をスキップ** する．
祖先の証明が子の証明を包含するため，子の proof は探索目的では不要．

```
mid() の path に pos_key + hand を並行記録:
  path[i]          = full_hash  (ループ検出用)
  path_pos_key[i]  = pos_key    (祖先チェック用)
  path_hand[i]     = hand       (祖先チェック用)

store(pn=0) 時:
  for ancestor in path[0..path_len-1].rev():
    if ProvenTT.has_proof(ancestor.pos_key, ancestor.hand):
      return  // 挿入スキップ
```

この最適化により ProvenTT が恒常的にスリムに保たれ，
WorkingTT への圧迫が解消される:

| 指標 | ベースライン (v0.24.0 初期) | +祖先チェック | 変化 |
|------|--------------------------|-------------|------|
| ProvenTT overflow (100M) | 47.0M | 37.3M | -21% |
| WorkingTT overflow (100M) | 8.3M | 8.9M | +7% |
| NPS (100M) | 244K | **285K** | **+17%** |

**クラスタ飽和の残存問題 (v0.24.0):**

祖先チェックにより ProvenTT の same-pos\_key 冗長挿入は抑制されるが，
**foreign 衝突**（異なる pos\_key が同一クラスタにマップ）は解消されない．
100M ノードで ProvenTT overflow = 37.3M (37%) が残存しており，
TT 実効容量は全スロットの 7.3% に留まる．

#### 6.6.4 hand\_hash 混合インデクシング + 2段階検索 (v0.24.0)

**ProvenTT の hand\_hash 混合:**

ProvenTT のクラスタインデックスを `pos_key XOR hand_hash(hand)` に変更し，
hand バリアントを異なるクラスタに分散する:

```rust
fn hand_hash(hand: &[u8; 7]) -> u64 {
    let packed = u64::from_le_bytes([hand[0], ..., hand[6], 0]);
    packed.wrapping_mul(0x9E3779B97F4A7C15) // golden ratio hash
}
...
同一盤面の異なる hand バリアントが異なるクラスタにマップされるため，
hand バリアント偏在によるクラスタ飽和が構造的に解消される．

WorkingTT は pos\_key ベースインデクシングを維持（hand\_gte disproof 再利用のため）．

**2段階検索 (look\_up\_proven / look\_up\_working):**

ProvenTT と WorkingTT を独立した検索メソッドに分離:
- `look_up_proven(pos_key, hand, remaining)`: ProvenTT のみ(proof + confirmed disproof)
- `look_up_working(pos_key, hand, remaining)`: WorkingTT のみ(intermediate + depth-limited disproof)
- `look_up(...)`: 統合ラッパー(proven → working fallback)

2段階検索により，将来的にそれぞれのインデクシング方式や GC 戦略を独立して変更可能．

**段階的 retain\_proofs:**

Frontier サイクル間の `retain_proofs()` を段階的クリアに変更:
- WorkingTT の confirmed disproof (!path\_dep, remaining=INFINITE) は保持
- 中間エントリと path-dep disproof は除去

IDS depth 切り替え時は `clear_working()` で全クリア(構造的不詰の汚染防止)．

**ベンチマーク結果:**

| 構成 | 予算 | proven\_overflow | working\_overflow | NPS |
|------|------|----------------|-------------------|-----|
| v0.24.0 初期 | 50M | 12.0M | 5.1M | 183K |
| +祖先チェック | 50M | 11.8M | 6.3M | 201K |
| **+hand\_hash + 段階retain** | **50M** | **44.5K (-99.6%)** | **4.3M (-16%)** | **244K (+33%)** |
| v0.24.0 初期 | 100M | 47.0M | 8.3M | 244K |
| **+hand\_hash + 段階retain** | **100M** | **3.0M (-94%)** | **8.9M (+7%)** | **260K (+7%)** |

ProvenTT overflow が **99.6% 削減**され，NPS が **33% 改善**．

**不採用とした WorkingTT 改善案:**

| 案 | working\_overflow 改善 | NPS 影響 | 不採用理由 |
|----|----------------------|---------|-----------|
| WORKING\_CLUSTER\_SIZE=8 | +19%(悪化) | -2% | overflow 改善せず，メモリ増加 |
| WorkingTT hand\_hash 混合 | +33%(悪化) | -17% | hand\_gte disproof 再利用の喪失 |
| WorkingTT 2-way set associative | -5〜15% | **-18〜21%** | secondary スキャンの NPS コスト大 |

WorkingTT は pos\_key インデクシング（hand\_gte 再利用あり）のままが最適．
hand\_gte による disproof 再利用の探索効率向上がクラスタ飽和のコストを上回る．

詳細は §10.2 の構造的課題分析を参照．
...
v0.24.0 では Dual TT の `retain_proofs()` が WorkingTT 全クリアに変更されたため，
...
**クラスタ飽和の定量分析 (v0.24.0, ProvenTT + 祖先チェック後):**

| 予算 | ProvenTT overflow | WorkingTT overflow | overflow/nodes |
|------|-------------------|-------------------|---------------|
| 50M | 11.8M | 6.3M | 24% + 13% |
| 100M | 37.3M | 8.9M | 37% + 9% |

ノードの 37%(100M 時)で ProvenTT の store が overflow(既存エントリの強制置換)．
TT 全体では 20M スロット中 1.45M(7.3%)しか使用されていない．
これは **foreign 衝突**（異なる pos\_key が同一クラスタにマップ）が主因であり，
クラスタサイズ拡大やクラスタ数増加では根本解決しない:

| 対策 | ProvenTT overflow 改善 | 副作用 | NPS 変化 |
|------|----------------------|--------|---------|
| CLUSTER\_SIZE=6 | -29〜54% | WorkingTT overflow +63〜133% | -14% |
| 4M clusters | -32〜40% | WorkingTT overflow +61〜69% | -9〜16% |
| ply-based amount | -10〜29% | WorkingTT overflow +29〜72% | -7〜13% |
| 祖先チェック | -21% | WorkingTT +7%(ほぼ同等) | +17% |
| **hand\_hash 混合 + 段階retain** | **-99.6%** | **WorkingTT -16%** | **+33%** |

hand\_hash 混合インデクシング(§6.6.4)により ProvenTT のクラスタ飽和が
構造的に解消され，段階的 retain\_proofs との組み合わせで NPS が 33% 改善．

**WorkingTT overflow の残存と不採用案:**

WorkingTT overflow (4.3M at 50M) は hand\_gte disproof 再利用のために
pos\_key インデクシングを維持する必要があり，構造的に残存する:

| WorkingTT 改善案 | overflow 改善 | NPS 影響 | 判定 |
|-----------------|-------------|---------|------|
| CLUSTER\_SIZE=8 | 悪化(+19%) | -2% | 不採用 |
| hand\_hash 混合 | 悪化(+33%) | -17% | 不採用(hand\_gte 喪失) |
| 2-way set associative | -5〜15% | **-18〜21%** | 不採用(スキャンコスト大) |

hand\_gte による disproof 再利用の探索効率向上がクラスタ飽和コストを上回るため，
WorkingTT は pos\_key インデクシングのままが最適．

#### 構造的課題の分析 (v0.24.0 最終)

39手詰めが 500M ノードでも解けない原因の分析と，
v0.24.0 での改善結果を以下にまとめる．

**課題 A: クラスタ飽和による TT 実効容量の損失 — ProvenTT は解決，WorkingTT は残存**

v0.24.0 初期: ProvenTT overflow = 47.0M (100M ノード)，
TT 実効容量は全スロットの 7.3%．

hand\_hash 混合インデクシング(§6.6.4)により ProvenTT のクラスタ飽和を
構造的に解消(overflow 99.6% 削減)．

WorkingTT overflow (4.3M at 50M) は hand\_gte disproof 再利用を維持するため
pos\_key インデクシングが必要であり残存する．
ただし以下の WorkingTT 改善案は全て NPS 低下を招くため不採用:
- CLUSTER\_SIZE 拡大: overflow 改善せず NPS 微減
- hand\_hash 混合: hand\_gte 喪失で NPS -17%
- 2-way set associative: secondary スキャンで NPS -18〜21%

**課題 B: IDS の depth 切り替えによる情報損失 — 部分解決**

**Frontier サイクル間:** `retain_proofs()` を段階的クリアに変更(§6.6.4)．
confirmed disproof (!path\_dep, REMAINING\_INFINITE) を Frontier サイクル間で保持する．
中間エントリと path-dep disproof は除去．

**IDS depth 切り替え時:** `clear_working()` で WorkingTT を全クリア．
構造的不詰エントリの汚染防止のため全クリアが必要(NoMate バグ §6.6.3)．
ProvenTT は proof only(confirmed disproof は WorkingTT)なので
`clear_proven_disproofs()` は不要．

**段階的 retain の効果:**
ProvenTT のクラスタ飽和が解消された状態(§6.6.4)で段階的 retain を導入することで，
...
| 課題 | 状態 | 改善内容 | 残存影響 |
|------|------|---------|---------|
| A: ProvenTT クラスタ飽和 | **解決** | hand\_hash 混合(§6.6.4) | overflow -99.6% |
| A: WorkingTT クラスタ飽和 | **残存** | hand\_gte 維持のため pos\_key 必須 | overflow 4.3M (50M) |
| B: IDS 情報損失 | **部分解決** | 段階 retain(§6.6.4) | IDS 切替時の全クリアは残存 |
...
| 日付 | 版 | 内容 |
|------|-----|------|
| 2026-04-04 | v0.24.0 | §6.6.3 Dual TT + エントリ圧縮(40→32B) + NoMate バグ修正 + ply ベース amount + 祖先チェック．§6.6.4 ProvenTT hand\_hash 混合(overflow -99.6%) + 2段階検索 + 段階的 retain\_proofs．NPS +33%(50M: 183K→244K)．WorkingTT 改善3案(CLUSTER拡大/hand\_hash/2-way)は全て NPS 低下で不採用．§9-b PV 復元ドキュメント．§10.2 構造的課題分析(A: ProvenTT 解決，WorkingTT 残存，B: 部分解決，C: 残存) |
