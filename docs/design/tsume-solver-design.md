...
**現状 (v0.20.35):** テストは 10M ノード / 60秒の予算で `#[ignore]` として設定．
10M ノードでは UNKNOWN(未解決)となり，MID 閾値飢餓の影響で
フルデプスの証明に到達できない．
...
にもかかわらず ply 24 全体は 1M ノードでも UNKNOWN(全体ベンチマークでは 100M でも UNKNOWN)である．

この乖離の原因は **MID の閾値配分の非効率性**にある:
MID がルートから ply 24 に到達するまでに閾値が枯渇し，
ply 24 の各応手に十分なノード予算を配分できない．
個別には各 200K ノードで解ける問題群が，
統合探索では閾値制御のオーバーヘッドで 100M+ ノードでも解けない．

#### MID 閾値飢餓 (Threshold Starvation)
...
ただし PNS オーバーヘッドにより NPS が ~27% 低下し，100M ノードでも未解決．
...
4. **未解決の根本原因**: 100M ノードでも UNKNOWN．
   ply 25 の個別応手分析では 23応手全てが 200K ノード以内で解けるが，
   MID がルートから ply 24 に到達するまでに閾値が枯渇し，
