...
```
// NG: PN_UNIT > 1 で除算精度が変わり挙動が一致しない
let epsilon = second_best / 4 + PN_UNIT;

// OK: PN_UNIT=1 相当で除算してからスケール
let epsilon = second_best / PN_UNIT / 4 * PN_UNIT + PN_UNIT;
```

例: `second_best = 3 * PN_UNIT` のとき
- NG (PN\_UNIT=64): `192 / 4 + 64 = 112`（期待値 64 との乖離 75%）
- OK (PN\_UNIT=64): `192 / 64 / 4 * 64 + 64 = 0 + 64 = 64` ✓
...
PN\_UNIT=1 と PN\_UNIT=64 で backward 解析が ply 24 まで完全一致した．

**v0.20.36 の改善(PN\_UNIT 導入後):**

PN\_UNIT(§3.5)の導入と divide-at-unit-scale パターンの適用により，
PN\_UNIT=1 と PN\_UNIT=64 で backward 解析が ply 24 まで完全一致することを確認．
これにより PN\_UNIT の拡大が**安全に**閾値飢餓を緩和できる基盤が整った．

**採用済み手法:**

| 手法 | 効果 | 備考 |
|------|------|------|
| チェーン AND pn\_floor boost (§3.3) | ply 24 サブ: 1M Unknown → 397K Mate | チェーン AND 内部のみ改善 |
| Frontier Variant (§11.7) | MID 停滞後も TT 成長継続 | NPS ~20% 低下 |
| PN\_UNIT 統一スケーリング (§3.5) | PN\_UNIT 拡大の基盤整備 | PN\_UNIT=64 で全テスト通過 |

#### 今後の改善方針

1. **heuristic\_or\_pn の高解像度化**: PN\_UNIT の拡大により pn 初期値の
   解像度が向上し，局面評価をより精密に行える．
   現在の heuristic\_or\_pn は 1〜3 PN\_UNIT の 3 段階だが，
   PN\_UNIT=16 なら整数 16〜48 の範囲で細かい初期 pn を設定でき，
   例えば safe\_escapes の数や王手数に応じた連続的なスケーリングが
   整数精度の範囲で実現可能になる．
   divide-at-unit-scale パターン(§3.5)により PN\_UNIT=1 との
   等価性を維持したまま，3S を上限として多段評価を導入できる．
   heuristic\_and\_pn についても同様に解像度を上げて
   応手数・逃げ場の組み合わせをより正確に反映できる．
2. **Frontier Variant の軽量化**: PNS アリーナ確保のオーバーヘッドが NPS を
   ~27% 低下させる(§11.7)．アリーナの再利用や増分更新で NPS 低下を抑えれば，
   MID 停滞後の Frontier フェーズで解決に到達できる可能性がある．
3. **IDS フルデプスの予算配分最適化**: 現在は MID 1/2 → Frontier 1/2 の固定分割．
   MID が新しい TT エントリを生成しなくなった時点で停滞と判定し，
   残り予算を動的に Frontier に回す方式が有効．
4. **並列 df-pn (§11.6)**: マルチスレッドでの探索は構造的にスケールし，
