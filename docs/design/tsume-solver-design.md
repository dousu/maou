...
クラスタサイズ・クラスタ数の拡大は ProvenTT overflow を減らすが，
WorkingTT overflow が連鎖的に増加し NPS が低下する．
唯一 NPS が改善したのは祖先チェックのみ．

#### 構造的課題の分析 (v0.24.0)

39手詰めが解けない根本原因は以下の 3 つの構造的課題の複合:

**課題 A: クラスタ飽和による TT 実効容量の損失**

TT 全スロットの 7.3% しか使用されていないにもかかわらず，
特定クラスタが満杯(foreign 衝突 + hand バリアント偏在)で store が overflow する．
この問題は§6.6.1で特定済みだが，Dual TT でも根本解決されていない．

解決の方向性:
- ProvenTT を **full\_hash ベースインデクシング** に移行
  (hand バリアントが異なるクラスタに分散)
- ただし hand\_gte proof 再利用が失われるため，
  dedicated proof cache や WorkingTT での fallback lookup が必要
- あるいは **クラスタインデックスに hand の一部ビットを混合**
  (pos\_key XOR hand\_hash)して分散しつつ，
  同一 pos\_key の look\_up は複数クラスタをスキャン

**課題 B: IDS の depth 切り替えによる情報損失**

IDS の各 depth で `retain_proofs()` が WorkingTT を全クリアし，
`clear_proven_disproofs()` が ProvenTT の confirmed disproof を全クリアする．
浅い depth で蓄積した中間エントリ・confirmed disproof が全て失われる．

NoMate バグ(§6.6.3)の修正として `clear_proven_disproofs()` を導入したが，
これは同時に有効な confirmed disproof も除去してしまう．

解決の方向性:
- **段階的クリア**: depth 切り替え時に「新 depth - 2」以下の remaining を持つ
  disproof のみクリア(全クリアではなく段階的に)
- **depth タグ**: confirmed disproof に「格納時の IDS depth」を記録し，
  現在の IDS depth と一致する場合のみ使用(クリアではなく条件付き使用)
...
| 課題 | 影響度 | 実現難易度 | 推奨アクション |
|------|--------|-----------|--------------|
| A: クラスタ飽和 | 高(TT 93% 未使用) | 中 | full\_hash ProvenTT or ハッシュ混合 |
| B: IDS 情報損失 | 中(depth 切り替え回数依存) | 低〜中 | 段階的クリア or depth タグ |
| C: 合駒分岐 | 高(指数的増加の主因) | 高 | 合駒フィルタ強化 |

**KomoringHeights との比較:**
KomoringHeights はこの 39手詰め問題を ~10分で解く．
リニアプロービング + amount ベース置換でクラスタ飽和問題がなく，
TT の実効容量が高い．maou\_shogi が同等の効率に達するには，
少なくとも課題 A+B の解決が必要と推定される．
...
| 日付 | 版 | 内容 |
|------|-----|------|
| 2026-04-04 | v0.24.0 | §6.6.3 Dual TT(ProvenTT + WorkingTT)+ エントリ圧縮(40→32B)．§6.6.3 NoMate バグ修正(IDS depth 切替時 confirmed disproof クリア)．§6.6.3 ply ベース amount + 祖先チェックによる ProvenTT 挿入スキップ(NPS +17%)．§9-b PV 復元ドキュメント追加．§10.2 39手詰め構造的課題分析(クラスタ飽和・IDS 情報損失・合駒分岐の 3 課題を特定) |
