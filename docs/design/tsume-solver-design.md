...
- **PN フロア(通常)**: `pn_floor = eff_pn_th * 2 / 3` (v0.21.1 で 1/2→2/3 に引き上げ)
- **PN フロア(チェーン AND)**: `pn_floor = max(DN_FLOOR, eff_pn_th * 2 / 3)`
...
しかし 39 手詰めの統合探索は依然 UNKNOWN．29 手詰め診断で残存課題の
主因が**閾値飢餓ではなく TT クラスタ飽和**(§6.6.1)であることが判明:

- ply 26 で pn\_th=373 (23S) が十分渡されているのに探索が停滞
- 原因: NM の store 失敗(foreign protected がクラスタ占有)→ pn/dn 不変
  → 1+ε の予算増加メカニズムが回らない(§6.6.1 参照)
