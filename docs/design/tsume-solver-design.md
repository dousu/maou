...
**現状 (v0.20.36):** 100M ノードでも UNKNOWN(未解決)．
...
1. **heuristic\_or\_pn の高解像度化**: PN\_UNIT を拡大し，
   pn 初期値の解像度を上げることで局面評価をより精密に行う．
   現在の heuristic\_or\_pn は 1〜3 PN\_UNIT の 3 段階だが，
   PN\_UNIT=16 なら 1S〜3S = 16〜48 の範囲内で中間値を設定でき，
   例えば safe\_escapes や王手数に応じた連続的なスケーリングが
   整数精度で実現できる．PN\_UNIT=1 では整数切り捨てにより
   区別できなかった局面差を，PN\_UNIT の拡大で表現可能になる．
   heuristic\_and\_pn についても同様に高解像度化が可能．
