...
```
solve(board)
├── Phase 1: PNS (Best-First)
│   ├── 予算: min(max_nodes/4, 150K)
│   ├── アリーナ上で most-proving node を選択・展開
│   ├── 証明済み → PV 抽出して返却
│   └── 未解決 → pns_store_to_tt() で証明/反証を TT に転写
│
├── Phase 2: IDS-MID (Iterative Deepening)
│   ├── 残り予算で反復深化
│   ├── 各反復: mid(root, INF-1, INF-1, 0, true)
│   ├── 反復間: TT 清掃 + NM 昇格検証
│   └── 証明済み or 反証済み → 終了
│
└── PV 抽出
    ├── PNS アリーナの PV(Phase 1 で証明済みの場合)
    ├── TT ベースの extract_pv(Phase 2 の場合)
    └── find_shortest: 最短手順の深さ制限付き再探索
```
...
- **期待効果:** 中．OR ノードで有望な王手を先に試し，閾値再配分回数を削減
- **実装コスト:** 低．2D 配列 + カットオフ時のインクリメント + ソートへの組み込み

### 11.2 PNS 予算適応配分

PNS の予算上限(`PNS_BUDGET_CAP = 150K`)を問題の深さに応じて適応的に設定する．
深い問題ほど PNS のグローバル最適選択の価値が高い．

- **期待効果:** 中．深い問題での PNS 効率向上
- **実装コスト:** 低．定数変更 + 条件分岐

### 11.3 AND ノード応手順序改善

応手のヒューリスティック dn を改善する:

- 玉の逃げ: 逃げ先の安全度(利きの数)に基づく dn 初期値
- 合駒: 駒種に基づく dn(歩合は安い = dn 低，飛合は高い = dn 高)

- **期待効果:** 中．不詰証明の高速化
- **実装コスト:** 中

### 11.4 TT フラットハッシュテーブル化

`FxHashMap<u64, Vec<DfPnEntry>>` を固定サイズのフラットハッシュテーブルに置換．

- **期待効果:** 中．Vec ヒープアロケーション削減，キャッシュ効率向上
- **実装コスト:** 高．TT 全体の再設計

### 11.5 Lazy Move Generation

OR ノードで手を段階的に生成する:
TT best move → killer → 捕獲王手 → 成王手 → その他

- **期待効果:** 中．浅い IDS 反復で残りの手生成コストを省ける
- **実装コスト:** 高．MID ループの構造変更

### 11.6 並列 df-pn (Kaneko 2010)
...
### 11.7 Frontier Variant (PNS→局所MID)
