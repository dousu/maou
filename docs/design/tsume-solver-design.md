...
| 33 | TT フラットハッシュテーブル | — | §11.4 | v0.20.33 |
...
| 構成 | ノード数 | TT エントリ数 | 時間 | NPS | 結果 |
|------|---------|-------------|------|-----|------|
| ベースライン (v0.20.32, HashMap) | 10,000,000 | 14,290,056 | 44.1s | ~227K | UNKNOWN |
| + Frontier Variant のみ | 9,995,631 | 14,266,367 | 46.0s | ~217K | UNKNOWN |
| + **フラットハッシュテーブル** | **9,995,434** | **1,304,639** | **11.5s** | **~868K** | UNKNOWN |

**分析:**

フラットハッシュテーブル(§11.4)による NPS 3.83× 高速化が最も効果的．
HashMap の Vec ヒープアロケーションとポインタチェーシングが
探索の主要ボトルネックだった．

10M ノードでは依然として 39手詰めを証明できないが，
同一時間で 3.83 倍のノードを探索できるため，
実質的な探索力が大幅に向上している．
...
| 構成 | ノード数 | TT エントリ数 | 時間 |
|------|---------|-------------|------|
| ベースライン (HashMap) | 50 | 197 | 0.002s |
| フラットハッシュテーブル | 50 | 196 | 0.094s |

フラットテーブルの初期化コスト(1M クラスタ確保)により
浅い問題では 0.09s のオーバーヘッドが発生する．
大規模探索では初期化コストは無視できる．
...
**実装 (v0.20.33):** 1M クラスタ × 6 エントリ/クラスタのフラット配列．
