...
基盤とし，Best-First PNS，反復深化(IDS)，Frontier Variant を組み合わせた
3フェーズ探索を採用する．
...
flowchart TD
    ...
    E --> F[Phase 2: IDS-MID]
    F -->|remaining budget × 3/4| G[IDS loop: depth 2,4,8,...,max]
    G --> H[mid root, INF-1, INF-1, ply=0]
    H --> I{root proved\nor disproved?}
    I -->|yes| J[extract PV from TT]
    I -->|no| K[TT cleanup + NM promotion]
    K -->|next depth| G
    I -->|timeout/nodes| N{budget remaining?}
    N -->|no| L[return Unknown]
    N -->|yes| O[Phase 3: Frontier Variant]
    O -->|PNS→MID cycle| P{root proved?}
    P -->|yes| J
    P -->|no| L
...
**実装:** `cross_deduce_deferred` (pns.rs)
