...
基盤とし，Best-First PNS と反復深化(IDS)を組み合わせた2フェーズ探索を採用する．
...
`rust/maou_shogi/src/dfpn.rs` に全ての探索ロジックを実装(約12,200行)．
...
**実装:** `pns_main()` 関数 (dfpn.rs:6049-6360)．

- **アリーナ**: `Vec<PnsNode>` (dfpn.rs:554-580)，上限 `PNS_MAX_ARENA_NODES = 5,000,000`
...
**実装:** `mid_fallback()` 関数 (dfpn.rs:5761-6030)．
...
flowchart TD
    ...
    E --> F[Phase 2: IDS-MID]
    F -->|remaining budget| G[IDS loop: depth 2,4,8,...,max]
    G --> H[mid root, INF-1, INF-1, ply=0]
    H --> I{root proved\nor disproved?}
    I -->|yes| J[extract PV from TT]
    I -->|no| K[TT cleanup + NM promotion]
    K -->|next depth| G
    I -->|timeout/nodes| L[return Unknown]
    D --> M[return Checkmate]
    J --> M
```
...
**実装:** dfpn.rs:57-68, 3614-3630, 3692
...
**実装:** dfpn.rs:3146-3158, 3862-3907

- `ZERO_PROGRESS_LIMIT = 16`: 連続16回の閾値低下で進展なしと判定
- `STAGNATION_LIMIT = 4`: 連続4回のゼロ進展で MID ループを終了
...
**実装:** dfpn.rs:3386, 3464, 3533-3542
...
**実装:** dfpn.rs:161-203 (`snda_dedup` 関数), 3220-3298, 3552-3563
...
#### `heuristic_or_pn` (dfpn.rs:4058-4111)
...
#### `heuristic_and_pn` (dfpn.rs:4031-4056)
...
#### `edge_cost_or` (OR ノードの王手): dfpn.rs:91-103
...
#### `edge_cost_and` (AND ノードの応手): dfpn.rs:114-126
...
**実装:** dfpn.rs:2559-2644

#### AND 子ノード(OR 局面)の検出: dfpn.rs:2560-2618
...
#### OR 子ノード(AND 局面)の検出: dfpn.rs:2619-2643
...
#### `has_mate_in_1_with` ヘルパー: dfpn.rs:4161-4177
...
**実装:** dfpn.rs:415-447 (`hand_gte` 関数), 661-710 (TT lookup)

- TT キー: `position_key(board)` = 盤面ハッシュ(持ち駒を**含まない**)
- TT 値: `Vec<DfPnEntry>` で同一盤面に対して最大16エントリを保持
- Lookup 時: 証明エントリを先に走査(証明優先)，その後反証エントリを走査
...
**実装:** dfpn.rs:449-480 (`hand_gte_forward_chain` 関数)
...
- **最大エントリ数**: `MAX_TT_ENTRIES_PER_POSITION = 16`
...
- **容量超過時**: 弱支配エントリを置換，なければ最古エントリを上書き

### 6.4 TT ガベージコレクション

**実装:** dfpn.rs:1085-1195, 2235-2253
...
**実装:** dfpn.rs:533, 717-732, 766-776
...
TT 全体: `FxHashMap<u64, Vec<DfPnEntry>>` (FxHash で高速化)
...
**実装:** dfpn.rs:534-539, 1009-1077, 1104-1106, 2310-2325
...
**実装:** dfpn.rs:497-520 (`propagate_nm_remaining` 関数)
...
**実装:** `compute_futile_and_chain_squares` (dfpn.rs)
...
**補助関数:** `king_can_escape_after_slider_capture` (dfpn.rs)
...
**実装:** `generate_chain_drops` (dfpn.rs)
...
**実装:** `try_prefilter_block` (dfpn.rs)
...
**実装:** `cross_deduce_deferred` (dfpn.rs)
...
**実装:** 定数 `INTERPOSE_DN_BIAS = 8` (dfpn.rs:55)
...
**実装:** dfpn.rs:1521-1525 (テーブル), 1853-1866 (記録), 2440-2454 (適用)
...
**実装:** `sacrifice_check_boost` (dfpn.rs:128-153)
...
**テスト:** `test_tsume_6_29te` (dfpn.rs:7831), `test_tsume_6_29te_no_pns` (dfpn.rs:7923)
...
**テスト:** `test_tsume_39te_aigoma` (dfpn.rs:8449, `#[ignore]`)
...
**現状:** テストは 10M ノード / 60秒の予算で `#[ignore]` として設定されており，
明示的に実行すると ~420K ノードで解ける．
ただし，デフォルトの1Mノード予算では ply 24 以降が解けず，
MID 閾値飢餓(§10.4)の影響でソルバーの汎用性に課題が残る．

#### backward 解析結果 (1M ノード/180秒)

| Ply | 残り手数 | ノード数 | TT entries (proven/disproven/intermediate) | 結果 |
|-----|---------|---------|-------------------------------------------|------|
| 38 | 1 | 21 | 54 (2/18/34) | Mate(1) |
| 36 | 3 | 2 | 8 (4/0/4) | Mate(3) |
| 34 | 5 | 12 | 37 (12/2/23) | Mate(5) |
| 32 | 7 | 17 | 47 (14/2/31) | Mate(7) |
| 30 | 9 | 22 | 59 (16/6/37) | Mate(9) |
| 28 | 11 | 74 | 198 (51/18/129) | Mate(11) |
| 26 | 13 | 83 | 221 (53/24/144) | Mate(13) |
| **24** | **15** | **1,000,000** | **8,713 (8,304/109/300)** | **Unknown** |

ply 26 → ply 24 でノード数が 83 → 1,000,000+ に急増．
原因はチェーン合駒の指数的分岐．
ply 24 では TT エントリの 95% が証明済み(proven=8,304)であり，
`deferred_children` による逐次活性化のメモリ効率は高い．

#### ply 25 AND ノード応手スクリーニング (250K ノード/30秒)
...
| P/L/N/S/G/B\*2g | 各1 | Mate(1) | 2筋合駒(即詰み) |
| P\*3g | 421 | Mate(15) | |
| B\*3g | 42 | Mate(11) | |
| N\*3g | 177 | Mate(11) | |
| P\*4g | 3,643 | Mate(15) | |
...
| Move | 消費ノード | 特徴 |
|------|-----------|------|
| 1g1f | 250,000 | 玉逃げ，深い読みが必要 |
| N\*6g | 250,000 | 桂合，チェーン構造 |
| P\*7g | 250,000 | 歩合，チェーン構造 |
| N\*7g | 250,000 | 桂合，チェーン構造 |

未解決応手の共通特徴:
1. **チェーン構造の再帰的展開**: 取り進み後に再び多数の合駒が発生
2. **TT エントリの集中**: 未解決チェーン応手では TT エントリの 90%+ が証明済みで
   中間エントリが極めて少ない(65〜73)．逐次活性化が効果的にメモリ消費を抑制
...
v0.20.33 で Phase 3 として実装(§11.7)．

### 10.5 39手詰め全体ベンチマーク (v0.20.33)

39手詰め合駒問題(SFEN: `9/1+R+N1kP2S/6pn1/9/9/5+B3/1R2S4/3p5/9 b NPb4g2sn4l14p 1`)
を depth=41, max\_nodes=10M, timeout=120s で計測した結果:

| 構成 | ノード数 | TT 位置数 | TT エントリ数 | 時間 | NPS | 結果 |
|------|---------|----------|-------------|------|-----|------|
| ベースライン (v0.20.32) | 10,000,000 | 11,125,267 | 14,290,056 | 44.1s | ~227K | UNKNOWN |
| + Frontier Variant (v0.20.33) | 9,995,631 | 11,186,175 | 14,266,367 | 46.0s | ~217K | UNKNOWN |

**分析:**

10M ノードでは 39手詰めを証明できない．
IDS-dfpn の各深さ(2→4→8→16→32→41)で MID が予算を消費するが，
フルデプス(41)の MID で閾値飢餓(§10.4)により深い ply に到達できず停滞する．
Frontier Variant (Phase 3) は残り予算(~2.5M ノード)で PNS→MID サイクルを
実行するが，予算不足で証明に至らない．

**9手詰めベンチマーク:**

| 構成 | ノード数 | TT 位置数 | TT エントリ数 | 時間 |
|------|---------|----------|-------------|------|
| ベースライン | 50 | 193 | 197 | 0.002s |
| + Frontier Variant | 50 | 193 | 197 | 0.002s |

浅い問題では Phase 1 (PNS) で即座に証明されるため，最適化の影響なし．

---

## 11. 最適化案の評価

v0.20.33 にて §11.1〜11.7 (§11.6 を除く) の最適化案を実装・計測し，
採用/不採用を決定した．計測環境: DevContainer (2 vCPU)，release ビルド．
...
### 11.4 TT フラットハッシュテーブル化 — 未実装

`FxHashMap<u64, Vec<DfPnEntry>>` を固定サイズのフラットハッシュテーブルに置換．

- **未実装理由:** 実装コスト高．TT は証明駒/反証駒のパレートフロンティア管理(§6.3)，
  前方チェーン比較(§6.2)，経路依存フラグ(§7.1)など複雑なセマンティクスを持ち，
  単純な open addressing では正確に再現できない．

### 11.5 Lazy Move Generation — 未実装

OR ノードで手を段階的に生成する:
TT best move → killer → 捕獲王手 → 成王手 → その他

- **未実装理由:** 実装コスト高．MID ループの構造が child init フェーズ
  (インライン詰み検出，TT プレフィルタ，DFPN-E)を全子に対して事前実行する
  設計であり，段階的手生成との両立が困難．
...
**実装 (v0.20.33):** IDS-dfpn のフルデプス MID に残り予算の 3/4 を割り当て，
残り 1/4 で PNS→MID サイクルを繰り返す Phase 3 として実装．
各サイクルで PNS(残り予算の 1/20，上限 50K)→ MID(残り予算の 1/4)を交互実行する．

**計測結果 (39手詰め, 10M ノード):**

| 方式 | NPS | TT エントリ数 | TT 位置数 | 時間 | 結果 |
|------|-----|-------------|----------|------|------|
| ベースライン (IDS-dfpn のみ) | ~227K | 14,290,056 | 11,125,267 | 44.1s | UNKNOWN |
| Frontier Variant | ~217K | 14,266,367 | 11,186,175 | 46.0s | UNKNOWN |

- **NPS 低下:** ~4.5%．PNS フェーズのアリーナ確保・ノード走査のオーバーヘッド．
- **TT 影響:** エントリ数はほぼ同等(−0.2%)．PNS→MID の TT 相乗効果は限定的．
- **採用理由:** 39手詰め(10M ノード)では効果が限定的だが，
  §10.4 で実証済みの閾値飢餓問題の構造的解決策として，
  ノード予算が十分な場合(50M+ ノード)に MID 単独より効率的な探索が期待できる．
  PNS のグローバル最適選択が MID のローカル閾値配分より優れるケースは
  深い問題ほど顕著になる．
