...
#### backward 解析結果 (1M ノード/180秒，v0.20.35 チェーン AND pn\_floor boost)

| Ply | 残り手数 | ノード数 | TT エントリ | 時間 | 結果 |
|-----|---------|---------|-----------|------|------|
| 38 | 1 | 14 | 39 | 0.09s | Mate(1) |
| 36 | 3 | 2 | 8 | 0.08s | Mate(3) |
| 34 | 5 | 12 | 36 | 0.07s | Mate(5) |
| 32 | 7 | 17 | 46 | 0.07s | Mate(7) |
| 30 | 9 | 22 | 56 | 0.07s | Mate(9) |
| 28 | 11 | 89 | 203 | 0.07s | Mate(11) |
| 26 | 13 | 103 | 198 | 0.06s | Mate(13) |
| **24** | **15** | **396,636** | **343,999** | **1.28s** | **Mate(21)** |
| **22** | **17** | **1,000,000** | **573,936** | **12.6s** | **Unknown** |

**改善:** ply 24 が 1M ノード Unknown → **397K ノードで Mate(21) に解決**．
チェーン AND の pn\_floor を DN\_FLOOR(=100) に引き上げたことで，
ply 25 の 23応手に十分な pn 予算が配分されるようになった．
境界は ply 24 → ply 22 に 2 ply 前進．

**TT 内訳 (tt\_diag):**
- ply 24 (Mate): proven=6,108, disproven=246,130, intermediate=91,761
  → disproven が 72%．不詰証明(合駒の不詰除外)が探索の主要コスト．
- ply 22 (Unknown): proven=16,252, disproven=557,684, **intermediate=0**
  → TT が完全に proven/disproven で埋まり中間ノードがゼロ．
  1M ノードの探索予算が TT の既存エントリのスラッシングに消費されている．
  ply 23 AND の PV 応手 1f1g が 250K ノード(テスト予算上限)で Unknown であり，
  その先のチェーン構造が新たなボトルネック．
...
v0.20.34 のフラット TT 化(§11.4)により，旧版で未解決だった4応手が**全て解決**した:
- 1g1f, N\*6g: NPS 向上により予算内で**詰み証明**に到達
- P\*7g, N\*7g: NPS 向上により**不詰証明**(NoMate)が可能に

**重要な結論: 23応手を個別に探索すればすべて 200K ノード以内で解ける．**
最も重い応手は P\*7g / N\*7g の不詰証明(各 ~200K ノード)であり，
23応手の合計は約 1.5M ノードに収まる．
にもかかわらず ply 24 全体は 1M ノードでも UNKNOWN(§10.5 では 100M でも UNKNOWN)である．

この乖離の原因は **MID の閾値配分の非効率性**(§10.4)にある:
MID がルートから ply 24 に到達するまでに閾値が枯渇し，
ply 24 の各応手に十分なノード予算を配分できない．
個別には各 200K ノードで解ける問題群が，
統合探索では閾値制御のオーバーヘッドで 100M+ ノードでも解けない．

**今後の改善方針:**

v0.20.35 で以下のアプローチを検証した結果，**OR ノードの 1+ε 手切替と
閾値飢餓対策は根本的に衝突する**ことが判明した(§10.4):
- heuristic\_or\_pn のスケーリング: AND の sum(pn) 膨張で NPS 壊滅
- OR ノードの pn フロア(eff\_pn\_th/2, DN\_FLOOR, depth 条件付き): 手切替を無効化し停滞
- ε の深さ適応加算: 手切替への干渉は小さいが TT 停滞は解消せず

MID 閾値制御の枠内での改善には構造的限界がある．有効な方向性:

1. **Frontier Variant の軽量化**: PNS アリーナ確保のオーバーヘッドが NPS を
   ~27% 低下させる(§11.7)．アリーナの再利用や増分更新で軽量化できれば，
   MID → Frontier の切り替えコストが減少し，実効的な探索力が向上する．
2. **IDS フルデプスの予算配分最適化**: MID の停滞を検出して動的に
   Frontier に切り替える方式が有効．MID が新しい TT エントリを生成しなく
   なった時点で停滞と判定し，残り予算を Frontier に回す．
3. **並列 df-pn (§11.6)**: マルチスレッドでの探索は構造的にスケールし，
...
| depth-adaptive heuristic\_or\_pn | sum(pn) 膨張で NPS 230K に低下，TT 完全停滞 |
| OR pn\_floor=eff\_pn\_th/2 | 1+ε 手切替を無効化し NPS 低下 + TT 停滞 |
| OR pn\_floor=DN\_FLOOR (ply>=16) | IDS 浅い反復で TT 汚染，NPS 385K に低下 |
| 深さ適応 ε (ply>=16 depth>=31) | NPS 719K だが TT 完全停滞 |
| **チェーン AND pn\_floor boost** | **ply 24 サブ問題 1M Unknown → 397K Mate．NPS 影響なし** |

Frontier variant が閾値飢餓の構造的回避策として IDS フルデプスステップに統合(§11.7)．
ただし PNS オーバーヘッドにより NPS が ~27% 低下し，100M ノードでも 39手詰めは未解決(§10.5)．

§10.2 の backward 解析により，ply 25 の 23応手は**個別には全て 200K ノード以内で解ける**ことが
判明した．にもかかわらず統合探索では 100M ノードでも未解決であり，
**問題の本質は合駒の分岐数ではなく閾値制御の非効率性**にある．

**v0.20.35 の改善と残課題:**

チェーン AND pn\_floor boost (§3.3) により ply 24 サブ問題が 397K ノードで解決．
しかし OR ノードの sibling\_based(2〜5)を拡大する試みは全て失敗した:
- heuristic\_or\_pn のスケーリング: AND の sum(pn) 膨張で探索効率が壊滅
- OR ノードの pn フロア: 1+ε の手切替メカニズムを破壊し停滞
- 深さ適応 ε: 手切替への干渉は小さいが TT 停滞は解消せず

**OR ノードの 1+ε 手切替と閾値飢餓対策は根本的に衝突する**ことが判明した．
1+ε は「best child の pn が second\_best + ε を超えたら切替」であり，
sibling\_based を大きくする = best child に深入りを許す = 復帰時に全子が
同等の pn になりスラッシングが発生する．
MID 閾値制御の枠内での改善は構造的に限界があり，
Frontier Variant の軽量化(PNS オーバーヘッド削減)が最も有望な方向性と考えられる．
...
4. **未解決の根本原因**: 100M ノードでも 39手詰めは UNKNOWN．
   ply 25 の個別応手分析(§10.2)では 23応手全てが 200K ノード以内で解けるが，
   MID がルートから ply 24 に到達するまでに閾値が枯渇し(§10.4)，
   個別に解ける問題群に十分な予算を配分できない．
   ボトルネックはチェーン合駒の分岐数ではなく，**MID の閾値制御の非効率性**である．
