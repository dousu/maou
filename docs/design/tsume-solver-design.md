...
1. **閾値制御の根本改善**: `heuristic_or_pn` の返り値(1〜3)が小さすぎることが
   閾値飢餓の主因(§10.4)．KomoringHeights の初期 pn=10〜80 に近づける調整が有効．
   ただし heuristic 値の拡大は浅い問題の NPS に影響するため慎重な調整が必要．
2. **Frontier Variant の改善**: 現在の Frontier は PNS アリーナ確保のオーバーヘッドが
   NPS を ~27% 低下させる(§11.7)．アリーナの再利用や増分更新で軽量化できれば，
   MID → Frontier の切り替えコストが減少し，実効的な探索力が向上する．
3. **IDS フルデプスの予算配分最適化**: 現在は MID に 1/2 → Frontier に 1/2 の固定分割だが，
   MID の停滞を検出して動的に Frontier に切り替える方式が有効．
