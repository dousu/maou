...
**Periodic GC:** TT が capacity の 80% / 90% に達した場合に
remaining の浅いエントリを除去する(v0.21.1 で容量ベースに変更)．
ただしクラスタレベルの飽和(全体 52% でも特定クラスタが満杯)には無力であり，
`replace_weakest_for_disproof` による積極的置換で対応する．
