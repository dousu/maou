...
1. **PN\_UNIT の最適値探索**: PN\_UNIT=1 で ply 24 サブ問題は 397K ノードで
   解けるが，ルートからの統合探索は 100M ノードでも未解決．
   PN\_UNIT を拡大することで OR ノードの sibling\_based が増大し，
   子 AND への pn 予算配分が改善される．§3.5 の基盤により
   PN\_UNIT の変更はスケーリング等価性が保証されるため，
   値の探索を安全に行える．
