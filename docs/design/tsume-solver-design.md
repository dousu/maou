...
**PN\_UNIT=1 と PN\_UNIT=64 の差異:**
小規模問題(〜2000 ノード)は完全一致するが，大規模問題(70K+ ノード)で
ply 分布に微差が生じる．P\*4g サブ問題(71K ノード)の診断で ply 0-2 は
完全一致，**ply 3 から分岐**(61 vs 58 ノード)することを確認．

原因は `pn_floor = eff_pn_th / 2` 等の**相対比率の整数除算精度**:
PN\_UNIT=1 で `eff_pn_th = 5` → `pn_floor = 2`(切り捨て),
PN\_UNIT=64 で `eff_pn_th = 320` → `pn_floor = 160`（`2 * 64 = 128 ≠ 160`）.
PN\_UNIT が大きいほど除算の解像度が上がり，探索の分岐選択が微妙に変化する．
これはスケーリング漏れではなく，**精度向上による正常な差異**であり，
PN\_UNIT を拡大する本来の目的(閾値の余裕確保)と整合する．
...
**現状 (v0.20.35):** 100M ノードでも UNKNOWN(未解決)．
...
**メカニズム:** `heuristic_or_pn` の返り値 1〜3 により，
OR ノードの 1+ε 閾値 `second_best * 5/4 + 1` が 2〜4 にしかならない．
子 AND ノードは pn\_threshold=2〜4 を受け取り，
23応手への配分がほぼ不可能になる．
KomoringHeights は初期 pn=10〜80 を使用しており，この問題が発生しにくい．

**エビデンス — 全体ベンチマーク (v0.20.34, depth=41, timeout=600s):**
...
MID only は 50M→100M で TT エントリが**完全に同一**．
追加 50M ノードが新たな局面を一切探索できておらず，完全な停滞状態．
Frontier Variant(§11.7)では TT が成長し続けるが，NPS が ~20% 低下する．

**構造的制約 — OR ノードの 1+ε 手切替との衝突:**

1+ε は「best child の pn が second\_best + ε を超えたら切替」で動作する．
閾値飢餓を緩和するには sibling\_based を拡大する必要があるが，
sibling\_based を大きくする = best child に深入りを許す
= 復帰時に全子が同等の pn になりスラッシングが発生する．

v0.20.35 で以下を検証し，全て上記の衝突により失敗した:

| 方針 | 手法 | 失敗理由 |
|------|------|---------|
| pn 初期値の拡大 | depth-adaptive heuristic\_or\_pn | AND の sum(pn) 膨張で NPS 壊滅(230K) |
| OR pn フロア | eff\_pn\_th/2, DN\_FLOOR, depth 条件付き | 手切替を無効化し TT 完全停滞 |
| ε の拡大 | 深さ適応 ε | TT 停滞は解消せず |

**成功した手法:**

| 手法 | 効果 | 理由 |
|------|------|------|
| チェーン AND pn\_floor boost (§3.3) | ply 24 サブ: 1M Unknown → 397K Mate | OR ノードを迂回しチェーン AND 内部のみ改善 |
| Frontier Variant (§11.7) | MID 停滞後も TT 成長継続 | 閾値なしの best-first でノード選択 |

#### 今後の改善方針

MID 閾値制御の枠内での改善は構造的に限界がある．
以下の方向性が有望:

1. **Frontier Variant の軽量化**: PNS アリーナ確保のオーバーヘッドが NPS を
   ~27% 低下させる(§11.7)．アリーナの再利用や増分更新で NPS 低下を抑えれば，
   MID 停滞後の Frontier フェーズで解決に到達できる可能性がある．
2. **IDS フルデプスの予算配分最適化**: 現在は MID 1/2 → Frontier 1/2 の固定分割．
   MID が新しい TT エントリを生成しなくなった時点で停滞と判定し，
   残り予算を動的に Frontier に回す方式が有効．
