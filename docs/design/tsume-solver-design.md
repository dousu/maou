...
v0.22.1 でリニアプロービング(8M エントリ，tombstone 方式，MAX\_PROBE=128)を
実装・評価した．以下に全ベンチマーク結果を示す:

**39 手詰め(10M nodes / 60s，同条件比較):**

| 方式 | NPS | overflow | 結果 |
|------|-----|---------|------|
| baseline (クラスタ) | ~89K | 449K | UNKNOWN |
...
1. **Periodic GC の rebuild コスト**: リニアプロービングでは一括削除(retain\_proofs,
   gc\_by\_amount)にテーブル全体のリビルド(生存エントリ収集→8M スロットクリア→
   再挿入)が必要．クラスタ方式では単純な pos\_key=0 セットで済む(rebuild 不要)．
   29 手詰め(50M/300s)ではこのコストで探索速度が低下しタイムアウト．
2. **LOOKUP\_PROBE の制約**: tombstone 蓄積でプローブ距離が延伸するため，
   短いプローブ距離(LOOKUP\_PROBE=24)を使うと proof エントリを発見できず
   PV が不完全(25 手)になる．MAX\_PROBE=128 を使えば正確だが NPS が低下．
3. **overflow 削減と NPS のトレードオフ**: overflow は 98% 削減されるが，
   GC rebuild コストが NPS を相殺し，大規模探索(29 手詰め 50M)では退行．

**構造的な課題:**
クラスタ方式の GC は O(N) の線形スキャンのみ(pos\_key=0 でクリア)．
リニアプロービングの GC は O(N) スキャン + O(N) クリア + O(K×probe) 再挿入の
3 パスが必要で，かつ再挿入時にプローブ衝突が発生する．
この差が Frontier サイクル毎の retain\_proofs() と
Periodic GC (1M ノード毎) で累積し，実効 NPS を低下させる．
