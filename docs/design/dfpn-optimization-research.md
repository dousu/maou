...
**maou_shogi:** 未実装．DFPN-E(§2.3)による静的エッジコストのみ．
...
**maou_shogi:** 部分的に実装．
`depth` パラメータによる手動の深さ制限と，`find_shortest` モードでの
PV 長制限による2パスの反復深化を実装している．
ただし初回探索時に浅い深さから段階的に探索する
自動的な深さ段階の最適化は未実装．
...
**maou_shogi:** 未実装．探索フレームワーク全体の変更が必要．
...
|--------|------|---|---------|-----------|------|
| 1 | IDS-dfpn(自動深さ段階) | §8.1 | 中 | 低 | 既存 depth パラメータの自動段階化 |
| 2 | TT Best Move保存 | §3.4 | 中 | 中 | PV復元の高速化 + §7.1 の前提 |
| 3 | 動的手順改善 | §7.1 | 中〜大 | 中 | TT Best Move + killer move(§3.4 が前提) |
| 4 | Weak Proof Number | §4.4 | 中 | 中 | 二重計数問題の解決(分岐係数ベース) |
| 5 | Best-First PN Search | §9.1 | 大 | 高 | フレームワーク全体の変更 |
| 6 | 並列 df-pn | §10.1 | 大 | 高 | TT スレッドセーフ化 + 負荷分散 |
