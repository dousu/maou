...
`epsilon = second_best / 4 + 1` を使用し，
閾値 `= second_best + epsilon ≈ second_best * 5/4 + 1` で乗算型を近似．
OR ノードでは `child_pn_th = min(eff_pn_th, second_best + epsilon)`,
AND ノードでは `child_dn_th = min(eff_dn_th, second_best + epsilon)` に適用．
...
- `heuristic_or_pn`: OR 子(攻め方局面)の初期 pn．王手数と玉の安全な逃げ場で調整．
  逃げ場0 → 1，逃げ場≥4 → `1 + escapes/3`，王手≤2かつ逃げ場≥3 → `2 + escapes/2`，
  その他 → 1．返り値は 1-5 程度の小さい整数．
- `heuristic_and_pn`: AND 子(守備方局面)の初期 pn．応手数と玉の安全な逃げ場で調整．
  逃げ場0 → `num_defenses * 2/3`，逃げ場≥3 → `num_defenses + escapes/2`，
  その他 → `num_defenses`．
- dn は `depth_biased_dn(base, ply) = max(base, ceil(R * ply))` で深さ依存(§2.4)．
...
手の質と玉との距離に基づくエッジコストを初期 pn に加算する．

- `edge_cost_or`(OR ノードの王手): 成/取=0，近い静か手(距離≤2)=1，遠い静か手(距離≥3)=2
- `edge_cost_and`(AND ノードの応手): 合駒(drop)=0，玉の逃げ/駒移動=1，駒取り=2
...
**maou_shogi:** ✅ 実装済み(v0.16.0)．`mid_fallback()` 関数として実装．
`solve()` の Phase 2(PNS 未解決時)で自動的に呼び出される．
倍増ステップ(2, 4, 8, 16, ..., depth)で反復深化する．
...
アリーナ上限(`PNS_MAX_ARENA_NODES = 2_000_000`)到達時は
Phase 2 で IDS-dfpn (`mid_fallback()`, §8.1) にフォールバックする．
...
`pns_store_to_tt()` は PNS 終了時にアリーナの**証明済み(pn=0)・反証済み(dn=0)ノードのみ**を
TT に転写する．中間ノード(pn>0 かつ dn>0)は保存しない．
これにより TT の肥大化を防ぎ，Phase 2 の MID が中間値に束縛されずに探索できる．
証明済み OR ノードでは `store_with_best_move` で最善手も TT に記録する．
...
| 手法 | 状態 | 期待効果 | 備考 |
|------|------|---------|------|
| 1+εトリック(乗算型) | ✅ 実装済み | 大 | `second_best + second_best/4 + 1` (≈ 5/4 乗算) |
...
| IDS-dfpn(自動深さ段階) | ✅ 実装済み | 中 | `mid_fallback()` 倍増ステップ + retain\_proofs(v0.16.0) |
| TT Best Move保存 | ✅ 実装済み | 中 | DfPnEntry に best\_move: u16(v0.16.0) |
| 動的手順改善 | ✅ 実装済み | 中〜大 | TT Best Move + Killer Move(2スロット/ply)(v0.16.0, v0.18.3) |
| WPN(Weak Proof Number) | ✅ 実装済み | 中 | max(pn) + (unproven\_count - 1) で二重計数緩和(v0.17.0) |
| Best-First PNS | ✅ 実装済み | 大 | Phase 1 PNS(2M arena) + Phase 2 IDS-MID(v0.18.0) |

### 既知の課題: MID 閾値飢餓(Threshold Starvation)

Phase 2 の IDS-MID で**閾値飢餓**が発生し，39手詰め級の長手数問題で
MID が深い ply に到達できない問題が確認されている．

**原因**: `heuristic_or_pn` の返り値が 1-5 と小さいため，
1+ε 閾値 `second_best * 5/4 + 1` も 2-7 程度にしかならない．
MID の `min(parent_th, epsilon_th)` により，子ノードに渡される閾値が
深い ply で急速に枯渇する．一方 KomoringHeights は初期 pn=10-80 の範囲を
使用しており，標準的な `second_best + 1` でも十分な閾値が得られる．

**検討済みアプローチ**:

| アプローチ | 結果 |
|-----------|------|
| PN\_SCALE=64 (全初期値を64倍) | max\_ply=16 で停滞，469Mノード/180sで未解決 |
| MID\_MIN\_THRESHOLD=256 | ply 42到達するが500Mノードでスラッシング |
| ε=1 (2x乗算) | 過剰探索で11分以上 |
| IDS-dfpn 単体 | root dn が閾値超過で即座に返る |
| Frontier variant (PNS→局所MID) | 深いフロンティア(ply 22)は1943ノードで証明成功 |
