...
**maou_shogi:** 保守的 SNDA 実装済み(v0.11.0)．
TT エントリに `source` フィールドを追加し，同一 source グループの
重複分を控除する方式．完全な max 集約は過度な過小評価のリスクがあるため，
`min(group) × (group_size - 1)` のみを控除する保守的アプローチを採用．
...
**maou_shogi:** `path`(FxHashSet)によるループ検出のみ．
経路依存/非依存の区別は未実装．
...
| SNDA(過大評価対策) | ✅ 実装済み | 大(長手数) | 保守的方式(v0.11.0) |
...
|--------|------|---------|-----------|------|
| 1 | DFPN-E(エッジコスト型) | 中〜大 | 中 | エッジベースの初期値．df-pn+より効果が高いと報告 |
| 2 | TCA(過小評価対策) | 中〜大 | 中 | 循環グラフでの完全性保証．df-pn(r)のunderestimation対策 |
| 3 | GHI対策強化 | 正確性+効率 | 高 | dual TT方式(経路依存/非依存の区別) |
| 4 | TT Best Move保存 | 中 | 中 | PV復元の高速化(再探索不要) |
