...
GitHub Actions の **Build GPU Wheel** ワークフロー (workflow_dispatch，
ブランチ選択可) が GPU 対応 wheel (features: onnx-cuda + onnx-tensorrt，
release ビルド) と provider ライブラリ tarball を Release **`latest-gpu`** に
添付する．Colab では以下だけで実 NN 探索が動く (Rust toolchain / cargo
ビルド不要 — 従量課金の実行時間を約 15 分節約できる):

```python
# 1. wheel と provider 群の取得 (Python バージョンに合う wheel を選ぶ)
!pip install https://github.com/dousu/maou/releases/download/latest-gpu/maou-<version>-cp312-cp312-manylinux_2_35_x86_64.whl
!wget -q https://github.com/dousu/maou/releases/download/latest-gpu/onnxruntime-gpu-providers.tar.gz
!mkdir -p /content/ort-libs && tar xzf onnxruntime-gpu-providers.tar.gz -C /content/ort-libs

# 2. TensorRT ランタイム (10 系 pin — §5 参照) + ldconfig 登録
...
# 3. 実行 (Stats に nps が出る．TRT エンジンの初回ビルドは数十秒〜数分)
!maou search --sfen "<SFEN>" --model-path /content/model_fp16.onnx \
    --tensorrt --cuda --threads 2 --batch-size 256 --time-ms 30000 --root-dfpn
```

- wheel は onnxruntime 本体を**静的リンク済み**．実行時に必要な外部
  ライブラリは provider 3 つ (tarball 内: shared/cuda/tensorrt — dlopen
  解決) と TensorRT (pip) + Colab 備え付けの CUDA 12 のみ．
