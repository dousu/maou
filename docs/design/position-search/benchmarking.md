...
### 事前ビルド GPU wheel を使う (`maou search` CLI — Rust ビルド不要)

GitHub Actions の **Build GPU Wheel** ワークフローが GPU 対応 wheel
(features: onnx-cuda + onnx-tensorrt，release ビルド) を Release
**`latest-gpu`** に添付する．Colab では以下だけで実 NN 探索が動く (Rust
toolchain / cargo ビルド不要)．**provider ライブラリは pip の onnxruntime-gpu
(wheel の静的リンク onnxruntime と同版 1.22 に pin 済み) の `capi/` から供給**
するため，別途 tarball の wget は不要．
