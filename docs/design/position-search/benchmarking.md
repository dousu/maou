...
### 事前ビルド wheel を使う (`maou search` CLI — Rust ビルド不要)

GitHub Actions の **Build Wheel** ワークフローが main push ごとに統合 wheel
(features: onnx-cuda + onnx-tensorrt，release ビルド，manylinux_2_28) を
Release **`latest`** に添付する (旧 `latest-gpu` は廃止 —
reviews/2026-07-16-unified-wheel-build.md)．Colab では以下だけで実 NN 探索が
動く (Rust toolchain / cargo ビルド不要)．**provider ライブラリは pip の
onnxruntime-gpu (wheel の静的リンク onnxruntime と同版 1.22 に pin 済み) の
`capi/` から供給**するため，別途 tarball の wget は不要．

```python
# 0. GPU プリフライト (未選択なら ランタイム > ランタイムのタイプを変更 > GPU)
!nvidia-smi -L
```

```python
# 1. wheel + GPU 推論依存を pip 一発で取得 (実行中の Python 版に合う wheel を
#    自動選択)．maou[tensorrt-infer] が onnxruntime-gpu==1.22.* (provider .so を
#    capi/ に同梱) と tensorrt-cu12==10.* (libnvinfer.so.10) を版一致で引く．
#    maou search は cshogi/torch を使わないため numpy 2.x のまま (衝突なし)．
...
```python
# 2. pip 同梱の .so を loader パスに載せる (ldconfig)．provider は
#    onnxruntime/capi/，libnvinfer は tensorrt_libs/，CUDA/cuDNN は Colab の
#    torch 同梱 nvidia libs から解決する．
import glob
...
with open("/etc/ld.so.conf.d/maou.conf", "w") as f:
    f.write("\n".join(dirs) + "\n")
!ldconfig 2>/dev/null   # "not a symbolic link" 警告 (無害) は捨てる
...
#    例: from google.colab import drive; drive.mount("/content/drive") など
```

```python
# 4. North-star NPS 計測 (単発で正しい NPS)．初回推論のエンジンビルドは
#    warmup_ms に分離され計測区間 (nps/elapsed_ms) に入らない．実配備と同条件で
#    root-dfpn を併走させる．
!maou search \
...
- provider の版は wheel の静的リンク onnxruntime (1.22 系) と一致させてある
  (`maou[tensorrt-infer]` の pin)．ズレると dlopen 時に ABI 不一致で落ちる．
  `maou[tensorrt-infer]` は onnxruntime-gpu / tensorrt-cu12 系のみを引き，
  cshogi (numpy<1.27 を強制) を含まないので Colab の numpy 2.x を保つ．
- onnx_bench でのバッチ/スレッド掃引を行う場合は従来どおり以下の cargo
  ビルド手順を使う (bench バイナリは wheel に含まれない)．

### リポジトリからビルドする (onnx_bench 掃引用)
