...
ときはこれに従う．下の § 根拠が記録しているとおり主要エンジンの順序とも
一致する — lc0 は `terminal rank → N → Q → P`，dlshogi は
`proven → move_count → nnrate`．

**playout が 1 回も回らなかったときの勝率** (maou_search v0.29.0)．同じ状況で
best_move は未訪問のままなので Q が存在しない．未訪問の Q は 0.0 だが，これは
「敗勢確定」ではなく「データなし」であり，そのまま報告すると評価値が飽和値
(-16578) になって「勝率 0 を確信した」ように見え，USI の投了判定
(`resign_value`，`agent.rs` の `decide_bestmove`) まで誤らせる．root 局面は
探索前に必ず NN 評価されている (§5.1 の warmup とは別に，探索の入口で行う
root 評価) ので，**その value (= 探索抜きの評価，`maou evaluate` と同じ値) を
返す**．探索できなかったときは 0 手読みの評価へ縮退する，という整理
(実測: `maou search --time-ms 1` と `maou evaluate` が同一の
`Eval 279.93 / WinRate 0.6146` を返す)．合法手なしの局面は**本物の敗勢**
なので 0.0 のままとする．
...
- 対局レイヤー導入後は「ルート移動時の subtree 再利用 + 残りを解放」が主機構に
  なる可能性があり，単発探索の GC はその補助と位置づける．対局レイヤー
  (USI エンジン) の設計は [usi-engine](../usi-engine/index.md) に常設し，
  subtree 再利用はそのマイルストーン M3 で実装・効果計測する．

## 8. 詰み探索統合 (8.1/8.2/8.3/8.5 実装済み，詰み探索はデフォルト有効)

詰み探索は **root-dfpn / leaf-mate / 受け方向 (被詰み) 探索をデフォルト有効**
(SearchOptions::default()) とする．いずれも余剰 CPU (専用スレッド) で動き，
詰みのない局面では NPS を落とさない (Colab A100: NPS 95-99% 保持)．役割分担と
PV-mate 棄却は §8.4，攻守の対称性は §8.5．
...
- **North-star の問い「MCTS がいくら探索しても詰みで評価が一変しないか」= YES**．
  ...
  直接的な答え (§8.1)．ただしこれは**攻め方向に限った答え**だった — 受け方向の
  同じ問いには §8.5 が答える．

### 8.5 受け方向の詰み探索 (被詰み検出) (実装済み — default-on)

§8.1〜§8.4 は全て「**手番側が詰ませられるか**」しか解いていない．その結果，
同じ局面が手番によって全く違って見える非対称が残っていた．

**観測 (自己対局 30 局の全数調査)**: 28/30 局で攻撃側が詰みを証明する一方，
詰まされる側の認識は中央値 1 手・**最大 11 手**遅れ，3 局は最後まで気付かない．
最悪例では詰まされる 11 手前から一貫して互角〜優勢を報告していた
(ply117 −141 / ply119 −70 / ply121 −17 / **ply123 +1170**，同じ時点で相手は
毎手 30000 を報告)．しかもこの区間の手番側は**全て王手中**で合法手は 3〜10 手，
攻撃側は T1〜T2 秒で即断，守備側は T11 秒の全予算を使って何も見つけていない．

**原因は 3 つの非対称の合成**:

1. **dfpn が攻め方向にしか呼べない** — 攻め方の決定は
   `dfpn/search/mod.rs` の `self.attacker = board.turn` 1 行のみ．
2. **詰み探索の出力と木の即時伝播規則が噛み合わない** — 即時発火するのは
   「子の確定値が 0.0」の OR ルールだけ (§8.3)．攻め方向の詰み探索が注入
   できるのは `proven::WIN` (親から見て 1.0) なので，親の確定には全兄弟の
   AND 集約が要る．**詰み探索から `proven::LOSS` が入る経路が無かった**．
3. **確定値は走行平均に希釈される** — 確定値は PUCT の Q に直接は入らない．

**設計**: dfpn の攻め方を明示指定できるようにし (`DfPnSolver::set_defensive`)，
**root を AND ノード**にして「手番側が詰まされるか」を同じ探索器で解く．
探索本体は `board.turn == attacker` の相対判定に統一されているため，
探索・TT・proof_hand・heuristics は無改修で追従する．

- **root** (`SearchOptions::defensive_mate`，**既定 true**): 攻め方向とは別
  スレッドで受け方向 dfpn を回す (`root_defensive_mate_nodes`，既定 500,000)．
  証明できたら **勝率 0.0 と最長抵抗の応手** (受け方向 PV の初手) を報告する．
  **MCTS は停止させない** — 詰まされていても指し手は必要で，早期停止に得が
  無い一方，証明が誤っていた場合の被害を広げるため．
- **葉** (`leaf_defensive_mate_nodes`，既定 2,000): **手番側が王手されている
  葉に限り**受け方向 df-pn を回す．証明できたら葉を `proven::LOSS` にする —
  親から見た確定値は 0.0 なので **OR ルールで即座に親が勝ち確定**になり，
  「その手を指すと詰まされる」が 1 手で伝播する (上の原因 2 を塞ぐ)．
  非王手の葉は分岐が大きいので対象外 (詰めろプローブは未実装 — §11)．
- **root 候補手の敗着フィルタ** (`root_child_filter_max_moves` 既定 16 /
  `root_child_filter_nodes` 既定 500,000): **まだ詰まされていないが，指し方を
  間違えると詰まされる**帯を担当する．root の各合法手を指した後の局面
  (相手手番) を**通常の攻め方向探索**で解き，相手に詰みがある手を
  `proven = Some(0.0)` に畳んで既存の負け確定除外に合流させる．
  - この帯は他のどの機構にも掛からない — root 受け方向 dfpn は「既に詰んで
    いる」ときしか発火せず，木の proven 伝播は葉の小予算 (50 / 2,000 ノード)
    では長手数の詰みを証明できないため．
  - **`find_shortest=false` が必須**．既定 (true) は最小性を確定できないと
    `Unknown` を返すので，長手数の詰みが「不明」に化けて敗着を見逃す
    (実測: 同じ局面が find_shortest=true では unknown，false では mate-41)．
  - 合法手数で gate するのは，総コストが子の数に比例する一方，「指した瞬間に
    詰まされる」型の敗着が**逃げ場の少ない局面に集中する**ため
    (実測した敗着局面の合法手は 3〜10)．
  - 停止フラグ (MCTS 終了) で途中打ち切りされる．部分結果でも「その手が負け」
    は正しいので健全．
  - **予算は段階的に配る** (`1,000` → `nodes/10` → `nodes`)．1 手ずつフル
    予算で潰すと重い手に予算を吸われ，後ろの手が未検査のまま打ち切られる —
    未検査は「安全」ではなく「未知」なので敗着が素通りする．全手を安い予算で
    1 巡すれば mate-1 級の即死は必ず先に落ちる．予算切れの手だけが次段へ進み，
    不詰を**証明できた**手は以降スキップする．
  - **置換表は段をまたいで引き継がれない**．df-pn の TT は `solve` ごとに
    ローカルで確保・破棄される (`solve` 間で持続するのは王手生成キャッシュ
    だけ)．したがって段階予算は「浅い段の探索を捨てて深い段でやり直す」形に
    なる — その再探索コストは**実測 34.7% あった**．
    内訳は探索のやり直しではなく **TT の確保・ゼロ初期化**が主で，浅い段でも
    production 既定の TT 下限 (2^18 = 16MB) を毎回 memset していた．
    **浅い段を小 TT (`new_leaf_mate` 相当) に切り替えたところ −0.5% (単発フル
    予算と同等) になった**．即死級 (mate-1 = 2 ノード / mate-3 = 10 ノード) を
    落とすのに大きな TT は要らない．
    - 本質的な解決は **`solve` 間の TT 再利用**だが，TT の `hand` が「攻め方
      持駒」の意味を持つため attacker 色をキーに含める必要があり，加えて
      千日手テーブルを局面をまたいで使い回すと不健全になり得る (root が
      変わると経路が変わる)．現状は再探索コストが実質ゼロなので見送る．
  - **並列化する** (`defensive_mate_threads`，既定 1)．子ごとの判定は独立
    なので，CPU が空く環境 (GPU 律速の対局・解析) では並列度を上げるほど同じ
    壁時計で大きな予算まで到達できる．子ごとのコストが数ノード〜数十万ノードと
    桁で違うため，静的分割ではなく共有カーソルの work-stealing にする．
    実測 (release):

    | 局面 | 1 スレッド | 2 | 4 |
    |---|---|---|---|
    | ply117 (4 子，うち 1 子が mate-41) | 2,720ms | 1,795ms | 1,701ms |
    | ply125 (10 子，全て詰み) | 802ms | 650ms | 650ms |

    **天井は「最も重い 1 子」** (Amdahl) なので，並列度は子の数が多いときに
    効き，1 本の重い証明には効かない．スレッドと予算は補完関係にある —
    並列度で多子のコストを吸収し，その分 `root_child_filter_nodes` を上げて
    最も重い子に十分なノードを与える．メモリは並列度に比例する
    (df-pn は `solve` ごとに置換表を確保するため 500,000 ノードで約 88MB/本)．

**コスト (実測，release / 上の最悪例の局面)**:

| 測定 | ノード数 |
|---|---|
| 攻め方 OR-root (相手側が同じ詰みを証明) | 321,508 |
| **受け方 AND-root (ply119)** | **308,066** (mate-40 / 1,234ms) |
| 同 (ply123) | 111,116 (mate-36 / 466ms) |
| 詰まされている局面での**攻め方向** root dfpn | **1** (即 disproven) |

3 つの含意がある．**(a) 受けの証明コスト ≒ 攻めの証明コスト** — 本命以外の
応手は数〜十ノードで反証されるので「分岐数 × 攻めコスト」にはならない．
**(b) 攻守のコストは逆相関する** — 詰まされている局面では攻め方向が即死する
ので，2 本走らせても実コストは和にならない．**(c) 王手中に限れば分岐は 3〜10**．
「全ての葉で大きな dfpn を回す」必要はない．

**いつ「詰まされ確定」に変わるか (同じ対局の先手番を順に受け方向で解いた結果)**:

| ply | 合法手 | 判定 | 所要 |
|---|---|---|---|
| 103〜113 | 101〜164 | safe (反証) | **33〜50ms** |
| 115 | 4 | safe (反証) | 1,409ms |
| **117** | **4** | **safe (反証)** | 788ms |
| 119 | 3 | MATED in 40 | 1,213ms |
| 121 / 123 / 125 | 7 / 3 / 10 | MATED in 38 / 36 / 34 | 931 / 518 / 503ms |

**境目は ply117→119**．つまり ply117 では**まだ受かっていた**．その 4 手を
一つずつ「指した後に相手が詰ませられるか」で判定すると:
...
4 手全部の判定に release で計 ~2.4 秒 — 対局中の予算は 11 秒あった．
**負けは「詰んでから見えなかった」のではなく，まだ受かる局面で敗着を選んだ
時点で決まっていた**．これが root 候補手の敗着フィルタを置く理由である．
なお ply115 も 4 手中 3 手が負け (mate-13 / 11 / 1) で，ここは正しく唯一の
安全手 6h5h を選べていた．

**発火量と実コスト (自己対局 30 局 = 3,760 局面の全数走査，release)**．
GPU A/B の前に実装を直接叩いて出したもの (計測規律 §10.2)．

敗着フィルタの gate (合法手 ≤ 16) 該当率:
...
gate 内の合法手は 2〜7 手が大半 (最大 14)．**発火はコストが安く価値が高い
終盤に集中する**．

1 局面あたりの所要 (286 局面すべてで合計 106 秒):

| p50 | p75 | p90 | p95 | p99 | max |
|---|---|---|---|---|---|
| 17ms | 112ms | 911ms | 1,977ms | 5,388ms | 14,472ms |

**1 手の予算 (11 秒級) を超えるのは 3,760 局面中 1 局面 (0.03%)** で，それも
停止フラグで打ち切られる (超過はしない — 部分結果でも「その手が負け」は正しい)．

**効果**:

| 指標 | 値 |
|---|---|
| 敗着が 1 手以上ある局面 | 158 (gate の 55.2%) |
| うち全手が負け (既に詰み，選びようがない) | 80 |
| **実際に指した手が敗着だった局面** | **87** |
| **うち安全な代替手が存在した (= フィルタが救える)** | **7** |
| 救える局面を含む対局 | **7 / 30 局** |

つまり **30 局中 7 局 (23%) に「避けられた強制詰みの敗着」が含まれていた**．
残る 80 局面は既に詰んでいて選びようがない — この切り分けが，フィルタの効果を
「詰みを見つけた回数」ではなく「**負けを避けられた回数**」で測る根拠になる．

**soundness の要注意点** (受け方向 root は王手中とは限らないことに起因):

- `verify_proof` の**無駄合いフィルタ**は「AND ノード = 王手中ゆえ駒打ちは
  合駒」を前提にしている．非王手の AND root では前提が崩れ，全防御が無駄合い
  扱いされると `best_move=None` → 「受けなし = 詰み」の**偽証明**になる．
  → 非王手なら無駄合い除外を適用しない (`dfpn/search/pv.rs`)．
- `find_shortest` の最短下限は攻め方向 root が 1 (奇数手)，受け方向 root は
  2 (偶数手)．
- TT を solve 間で再利用する設計にするなら，TT の `hand` が「攻め方持駒」の
  意味を持つため **attacker 色をキーに含める**必要がある (現状は solve ごとに
  再確保するので未発生)．
