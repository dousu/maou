...
- x = スナップショット番号 (盤面ナビの手数)，y = **先手視点固定の
  2 択: 勝率 / 評価値** (デフォルトは勝率)．analyze-game JSON の
  `winrate` / `eval_cp` は手番視点なので，表示時に
  `side_to_move == "w"` の行を `1 - winrate` / `-eval_cp` に変換する
  (interface 層の純関数とし単体テストを書く)．
  - **後手視点の切替は持たない** (user 決定 2026-08-22)．後手視点は
    先手視点の鏡映 (勝率 1 - x / 評価値符号反転) で同じ 1 本の曲線を
    読み替えるだけなので，ワークベンチではレールの操作子を増やさない
    ことを優先した．変換関数 (`sente_winrate` / `sente_eval_cp`) は
    そのまま残る．
  - レポートの `positions[i].ply` は «その手を指す直前の局面» なので，
    x 軸ではスナップショット番号 (`ply - 1`) に置く．現在位置の縦線・
    悪手の ●・詰みの ★・`data-scrub` のクリック → 手数が同じ軸に乗る．
...
- 候補手テーブル: `candidates` 上位 N (N は 1〜num-candidates の
  数値入力) を順位/日本語表記/訪問数/勝率 (手番視点のまま明示) で表示．
  **prior と詰み確定値は行のホバー (title) で出す** (user 決定
  2026-08-22) — 右レール幅では 6 列が詰まるため，常時見せる 4 列と
  拾える 2 値に分けた．
