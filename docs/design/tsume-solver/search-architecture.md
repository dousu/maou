...
OR / AND の判定は全て `board.turn == self.attacker` の**相対判定**であり，
`depth % 2` のような絶対位置依存は無い．したがって攻め方をどちらに置くかは
`solve_report_impl` の 1 行 (`self.attacker = ...`) だけで決まる．

- **既定 (攻め方向)**: 攻め方 = root の手番．root は **OR ノード**で，
  「手番側が詰ませられるか」を解く．詰み手数は**奇数**．
- **`set_defensive(true)` (受け方向)**: 攻め方 = root の手番の相手．root は
  **AND ノード**で，「**手番側が詰まされるか**」を解く．詰み手数は**偶数**で，
  PV の初手は受け方 (root 手番側) の最長抵抗手になる．
  公開 API は `solve_tsume_defense`．用途は MCTS の被詰み検出
  ([position-search §8.5](../position-search/index.md))．

受け方向 root で注意が要る箇所は 3 つ — いずれも「AND ノード = 王手されている」
という暗黙の前提に由来する (**受け方向 root は王手中とは限らない**):

1. **無駄合いフィルタ** (`search/pv.rs`) は「王手中の AND ノードゆえ駒打ちは
   合駒」を前提にする．非王手 root で適用すると全防御が無駄合い扱いされ
   「受けなし = 詰み」の**偽証明**になり得るため，`is_in_check` で条件付ける．
2. **`find_shortest` の最短下限**は攻め方向 1 / 受け方向 2 (手数の偶奇)．
3. **TT を solve 間で再利用する場合**，TT の `hand` は「攻め方持駒」の意味を
   持つので **attacker 色をキーに含めない限り unsound** になる
   (現状は solve ごとに TT を再確保するので未発生)．
