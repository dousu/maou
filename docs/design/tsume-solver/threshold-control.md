...
child_thφ   = min(thφ, second_φ + second_φ/8 + 1)   // 1+ε 子 φ 閾値 (ε=1/8; 3.4.4)
child_thδ   = new_thdelta_for_best_move(thδ)          // 残り δ 予算を best child に配分
```

子 φ 予算はノード内の 1+ε (ε=1/8, ×1.125) を採用する (3.4.4)．標準式 `second_φ + 1` より
子の切替が減り閾値反復の再展開が減る．測定 (2026-07-02): 39te full 最小化 nodes −29%
(24,773,536 → 17,545,528)・wall −13%，29te −25% (531,296 → 396,516)．最短手数・canonical
PV・161 tests は全て不変．**閾値は df-pn の効率のみに作用し健全性に影響しない** (健全性は
STRICT verify が担保する; [loop-ghi §7.5](loop-ghi.md))．なお ε をさらに緩める
(`second+second/8+4` 等) と nodes は減るが verify/do_move 増で wall が悪化する (過緩和)．
...
**反証の scope 化** ([loop-ghi.md §7.2](loop-ghi.md)) で健全に扱う．旧機構の計測は
[legacy/threshold 関連](legacy/README.md) (benchmarks / pn-dn-distribution) 参照．
