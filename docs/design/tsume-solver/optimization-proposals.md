...
### 11.11 v0.25.x 実装知見に基づく次の改善候補 (v0.25.5 時点)

v0.25.0〜v0.25.5 の実装・計測過程で発見した知見と残存課題を整理し，
...
- refut_tt_hits: 0 → 177+ (F3 default)

#### 高 ROI（データ裏付けあり / 短期実装可能）

##### 候補 N-1: B-2 adaptive disproof threshold の default 化

v0.25.0 で threshold=3 が ply 18 で NPS +54% を達成したが，no-mate test
退行のため default 化を見送った (§10.2.8)．

v0.25.5 時点で M-A (refutable depth floor) が false-NoMate を根絶済みのため，
**depth ≥ 23 限定** で adaptive threshold を default 化できる可能性がある．
`enable_adaptive_disproof_remaining_threshold()` の呼出を solve() で自動化．

- **期待効果**: F3 (nodes -75%) + B-2 (NPS +54%) の相乗
- **リスク**: 中 (no-mate test + backward regression を再検証)
- **実装コスト**: 低 (既存 flag の default 変更)
...
##### 候補 N-3: ply 16 以降の backward 実測

v0.25.5 で ply 18 が 100M に収まったため，ply 16 (depth=25) に挑戦可能．
v0.24.78 では 500M / 3,600s で timeout だった．

- **期待効果**: 39 手詰めの次の境界突破可能性の判定
- **リスク**: なし (計測のみ)
- **実装コスト**: なし (テスト実行のみ)
