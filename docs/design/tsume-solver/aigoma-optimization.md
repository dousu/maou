...
| MID boundary `all_checks_refutable_by_tt` | 可視 | 可視 |
| PNS `look_up_pn_dn` (`skip_refutable_disproof=true`) | 可視 | **不可視** |
| refutable check 高速パス `all_checks_refutable_by_tt_or_refutable` | 可視 | 可視 |

TT レベルでは confirmed / refutable を区別せず，`look_up_proven` および
`has_refutable_or_confirmed_disproof` は両方を可視として扱う．
PNS 側の不可視化は solver の `skip_refutable_disproof` フラグを
`look_up_pn_dn_impl` で参照することで実現する (TT 側ではなく呼び出し側で制御)．
...
- **PNS の lookup**: `skip_refutable_disproof=true` で不可視化．arena-limited
  ...
  cascade を防止．
- **MID boundary check** (`all_checks_refutable_by_tt`): `look_up` 経由で
  confirmed / refutable 両方を参照．refutable は arena-limited provisional
  disproof だが，MID は threshold-based 探索で検証可能なため混入しても
  false NM にはならない (後述「なぜ arena-limited disproof として扱うか」参照)．
