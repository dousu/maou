...
2. **どんな詰将棋問題も解ける**: リソースパラメータ (`depth`, `nodes`, `timeout`) の増加で対応．
   **超長手数** (ミクロコスモス級 1525 手) も対象で，`depth` 上限は 2047，千日手テーブルも
   generation GC で bound される ([transposition-table §6.7](transposition-table.md))．
...
- **`solve_tsume(sfen, depth, nodes)`**: デフォルトタイムアウト (300 秒) で解く．
  `depth` 既定 31 (上限 2047; 長手数詰将棋対応)，`nodes` 既定 1,048,576．
...
- **`solve_tsume_report_with_timeout(..., collect_progress)`**:
  上と同一の探索を行い，結果に加え診断メタデータ `SearchReport` を返す
  ([`DfPnSolver::solve_report`] 相当)．`unknown` の内訳判断・可解性見積りに使う．
- 返り値 `TsumeResult`:
...
- 診断メタデータ `SearchReport` (`solve_report` / `*_report_with_timeout` 経由):
  - `result: TsumeResult` + `root_pn` / `root_dn` (停止時 root pn/dn) / `elapsed_ms`．
  - `mate_len_found: Option<u32>`: 検証済み詰み手数．`Unknown` でも詰みがあれば `Some(d)`
    ("≤d 手の詰みが存在，最短だけ未確定")．
  - `shortest_confirmed: bool` / `stop_reason: StopReason` (`Solved` / `Disproven` /
    `MinimalityUnconfirmed` / `NodesExhausted` / `Timeout` / `FalseProof` / `Inconclusive`)．
  - `best_mate: Vec<Move>`: `MinimalityUnconfirmed` の `Unknown` でも残った検証済み詰み手順
    (`mate_len_found` と同じ手数)．最短を確定できず打ち切っても探索の成果を保持する
    (最短保証なし; `Checkmate` 時は空で手順は `result` 側)．
  - `progress: Vec<ProgressSample>`: root 反復ごとの pn/dn/nodes/経過時間
    (`collect_progress=true` 時のみ; per-node コスト無し)．
  - **全て探索終了時に 1 回だけ構築するため探索性能に影響しない**．
