...
**現状 (v0.24.78):** ルートからの完全解法は依然未達成．
backward 解析の解決境界:

| 予算 | 境界 | 結果 |
|:---|:---|:---|
| **10M nodes** | ply 18 (残り 21 手) | ply 20 まで解決 (Mate(19), 9.87M nodes, 169s) |
| **500M nodes** | ply 16 (残り 23 手) | ply 18 まで解決 (Mate(21), 387M nodes, 2,384s) |
...
**§10.2.6** で ply 18 の診断結果と残存課題 (カテゴリ A〜D) を整理．
...
1. この概要
2. §10.2.1 問題の構造 (不変)
3. §10.2.6 500M backward 解析 (最新測定)
4. `aigoma-optimization.md §8.9` Refutable Disproof (最新アルゴリズム)

設計経緯・失敗した施策の詳細は §10.2.2〜§10.2.5 (時系列) を参照．
...
### 10.2.16 N-7: スライダー打ち駒への shallow DN penalty (v0.27.0)

**背景:**

39手詰め (SFEN: `9/1+R+N1kP2S/6pn1/9/9/5+B3/1R2S4/3p5/9 b NPb4g2sn4l14p 1`) のply 25
AND node で，L*6g・B*6g・B*7g の 3 守備手が depth=25/15M 以内でも Unknown のまま残る
(gap 問題)．gap の主因は ply 18–24 での探索空間爆発であり，スライダー打ち (香・角・飛)
が合駒連鎖の手前に長い delay chain を形成する．N-7 はこれらスライダー打ちの DN (反証数)
を距離比例ボーナスで追加ペナルティし，AND node での探索優先度を下げる最適化．

**実装 (v0.27.0):**

- `is_slider_drop(m)`: Lance/Bishop/Rook の打ち駒か判定するヘルパー
- chain AND node: `dist_bias` を 2 倍 (スライダーは `dist_bias × 2`，非スライダーは `dist_bias × 1`)
- mixed chain AND node: 同様に chain drop に対して `dist_bias × 2`
- 非 chain AND node (`n7_and_king_sq`): 距離比例ペナルティ `max(Δcol, Δrow).saturating_sub(1) × PN_UNIT` を追加
- カウンタ: `diag_n7_slider_bonus` (tt_diag feature)

**ply 24 OR node 診断 (v0.27.0) — `39手詰め root` :**
...
| depth | v0.26.0 NPS | v0.27.0 NPS | 変化 |
|-------|-------------|-------------|------|
| 17 | ~5.7k (64.1s/642K) | 10.0k | **+75%** |
| 21 | ~13.4k | 8.6k | **-36%** |
| 25 | ~77.6k | 83.9k | **+8%** |

**評価:**

- depth=17: NPS +75% と大幅改善．N-7 がスライダーを早期に枝刈りし，Mate(15) の証明経路を
  効率的に探索できた．
- depth=21: NPS -36% の退行．`working_intermediate_hits` が -41% 減少し，proven TT も
  -33% 減少．N-7 の並べ替えが depth=21 特有の TT キャッシュ効率を損なった可能性がある．
- depth=25: NPS +8% の微改善．max_ply=24 に達しており，予算内で証明完了には至っていない．
- **gap 未解消**: Phase 0b の Unknown 3 手 (L*6g・B*6g・B*7g) は変化なし．

**残存課題:**

N-7 は depth=17 では有効だが，depth=21 での退行が確認された．gap の根本原因 (3 守備手の
反証コストが budget を超過) は未解消．depth=21 退行の原因調査および追加の gap 解消策が必要．
