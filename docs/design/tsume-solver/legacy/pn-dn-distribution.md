...
### 4.7 [課題 G] IDS 間の WorkingTT 引継ぎ改善 — pn=INF ソフトキャップ (実施済み・不採用)

#### 背景と仮説

`retain_working_intermediates` (tt.rs) の保持条件 `entry.pn < u32::MAX` により
IDS depth 遷移時に pn=INF のエントリが全廃棄される．
§4.5 で確認した depth=24 TT の **85% が pn=INF** という事実を踏まえ，
「pn=INF をソフトキャップ (RETAIN_INF_CAP = 4096×PN_UNIT = 65536) に変換して保持し，
次 IDS depth の初期ヒントとして再利用する」という改善を試みた．

#### 実験結果 (backward_10m: 39手詰め逆順サブ問題 10M nodes)

| Ply | v0.54.3 Nodes | v0.55.0 試験版 Nodes | 変化 | TT_pos 変化 |
|-----|--------------|---------------------|------|------------|
| 24 | 718,463 | 1,215,581 | **+69% 悪化** | 54,736 → 65,770 |
| 22 | 7,625,471 | 7,623,933 | ~0% | 26,540 → 26,493 |
| 20 | 5,333,924 | 9,573,164 | **+79% 悪化** (予算限界近) | 337,060 → **33,900** |
| 18 | 10,000,000 (Unknown) | 10,000,000 (Unknown) | 変化なし | 変化なし |
| 16以下 | Unknown | Unknown | 変化なし | — |

**10M 境界は ply 18 のまま変化なし．** ply 20 の TT_pos が 337K → 33K (90% 蒸発) と
TT 汚染が顕在化した．

#### 失敗原因の分析

**根本的誤り: WPN 累積 pn は次 depth での有効な下限値ではない．**

df-pn+ の設計原則として:
- pn(position, remaining=R) ≥ pn(position, remaining=R+Δ)  
  (より多くの depth が利用できると証明は容易 → pn は depth 増加と共に単調減少)
- 旧 depth D で保持した pn は新 depth D+Δ の**上限**であり，下限ではない

pn=INF はさらに WPN による人工的な膨張を受けており，これを pn=65536 として保持すると:
1. **AND ノードの pn 過大評価**: AND ノードの pn = Σ(子の pn)．多数の子が pn=65536 → 親 AND の pn が大幅に膨張．solver が AND ノードを困難と誤認し探索を先延ばし
2. **TT 汚染 (hash collision)**: 保持件数が ~15% (v0.54.3) → ~100% (v0.55.0) に増加し，有用エントリを evict (ply 20 の TT_pos 337K→33K が証拠)

**ply 22 が影響を受けなかった理由**:
- depth=17 の探索中に pn=65536 エントリの多くが更新される
- depth=19 (final) に入る時点では既に自己修正済み
- depth=21 (ply 20) は delta=4 が大きく，更多くのエントリが 17→21 で引き継がれ汚染が蓄積

#### 代替アプローチとの比較

元のコードコメント「pn=INF の intermediate は depth-limited 証明不能であり，
remaining をシフトしても新 depth で同じブロックが再発する」が正確であった．
depth D での pn=INF エントリを捨て，depth D+Δ で fresh な `heuristic_or_pn` 評価
(典型値 24–32,768) を再利用するほうが探索誘導として正確である．

#### 結論

`pn < u32::MAX` 条件は **counter-check 安全性とは無関係**に，
**WPN 膨張 pn を次 depth に持ち越さない** という探索健全性上の制約として正当である．

pn=INF ソフトキャップ方式は **不採用** (§5 の「対応しない事項」扱い)．
...
| pn=INF ソフトキャップ (retain_working_intermediates で pn=INF → 65536 に変換保持) | pn=INF は WPN 累積による depth 依存膨張値であり，次 depth の有効な下限値にならない．ply 24 +69%，ply 20 +79% regression，TT_pos 337K→33K 汚染 (§4.7)．原理的に fresh heuristic 再評価が正確 |
...
[完了 (このセッション)]
 ...
 └── 課題 G: pn=INF ソフトキャップ — 実施・不採用 (§4.7 参照)
       → ply 24: +69%，ply 20: +79% の深刻な regression
       → pn=INF エントリ保持による TT 汚染 (ply 20 TT_pos: 337K→33K)
       → 10M 境界 (ply 18) に改善なし
       → 根本原因: WPN 累積 pn は depth 依存で次 depth の有効な下限値にならない
