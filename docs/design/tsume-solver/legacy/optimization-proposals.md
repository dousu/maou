...
> [!WARNING]
> **legacy 文書 (原本)** — 旧二エンジン期の最適化案採否評価．現行版は
> [`../optimization-proposals.md`](../optimization-proposals.md)．本文書は方法論保全のため
> verbatim で残す．位置づけは [README.md](README.md)．節番号・機構名・版数は旧 docs 基準．
...
| loop\_detect | — | — | 0.9% | 41 | E5: 配列スキャン; v0.55.11 で path\_set 追加 O(1) 化 |
...
| E5 | 経路スタック化 + path_set O(1) 化 | **採用** | v0.23.0: FxHashSet→[u64; 48]，loop\_detect 41ns/call; v0.55.11: path\_set 追加で contains O(depth)→O(1) |
