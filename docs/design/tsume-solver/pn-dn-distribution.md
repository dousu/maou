...
| depth | v0.49.0 pn | v0.51.1 pn | v0.52.1 pn | v0.49.0 dn | v0.51.1 dn | v0.52.1 dn |
|-------|-----------|-----------|-----------|-----------|-----------|-----------|
| 8 | 0.395 | 0.395 | 0.411 | 0.128 | 0.155 | **0.220** |
| 16 | 0.473 | **0.412** ↑ | 0.467 | 0.531 | 0.611 ↓ | **0.433** ↑ |
| 20 | 0.473 | **0.411** ↑ | 0.458 | 0.507 | 0.646 ↓ | **0.438** ↑ |
| 24 | 0.507 | 0.503 | — | 0.683 | 0.748 ↓ | — |

目標 (< 0.3) に対して，現状 (v0.52.1) で pn KL は 0.458-0.467 (depth=16-20)，dn KL は 0.433-0.438 (depth=16-20)．
dn KL は v0.51.1 の 0.611-0.646 から大幅改善した (§3.21)．

KL が高い主要因 (§4.3/§4.4 調査結果より):
1. **pn: WPN AND 多段累積による bucket 30 (near-INF) 集積 (depth=16-20 での主犯，v0.52.1 計測):**
   v0.52.1 で TT が縮小したことで，高 pn 中間エントリの相対比率が増大し，
   depth=16-20 の max 寄与 bucket が bucket 6→**bucket 30** に移行した (§4.4 更新参照)
2. **pn: heuristic_or_pn 離散スパイク (depth=4-8 での主犯):** bucket 12 への集中 (depth=4: 寄与 0.591，depth=8: 寄与 0.133)
3. **dn: AND-min 伝播による PN_UNIT 収束:** bucket 5 (dn=16=PN_UNIT) への張り付き (v0.52.1 では bucket 9 (dn≈256) に改善)
...
**現状 (v0.52.1):** dn KL depth=16-20 で 0.433-0.438 (目標 < 0.3 に対して未達だが大幅改善)．
pn KL はほぼ横ばい (depth=16-20 で 0.458-0.467)．両指標とも目標 < 0.3 には未達．
...
**pn KL の主犯 bucket (v0.49.0 計測，v0.52.1 更新):**

| depth | v0.49.0 KL | 主犯 bucket (v0.49.0) | 寄与値 | v0.52.1 KL | 主犯 bucket (v0.52.1) | 寄与値 |
|-------|-----------|----------------------|--------|-----------|----------------------|--------|
| 4 | 0.891 | bucket 12 (≈2048) | 0.591 | 0.891 | bucket 12 (≈2048) | 0.591 |
| 8 | 0.395 | bucket 7 (≈64) | 0.100 | 0.411 | bucket 12 (≈2048) | 0.133 |
| 16 | 0.473 | bucket 6 (≈32) | 0.226 | 0.467 | **bucket 30 (≈2^29)** | **0.175** |
| 20 | 0.473 | bucket 6 (≈32) | 0.226 | 0.458 | **bucket 30 (≈2^29)** | **0.180** |

**dn KL の主犯 bucket (v0.49.0 計測，v0.52.1 更新):**

| depth | v0.49.0 KL | 主犯 bucket (v0.49.0) | 寄与値 | v0.52.1 KL | 主犯 bucket (v0.52.1) | 寄与値 |
|-------|-----------|----------------------|--------|-----------|----------------------|--------|
| 8 | 0.128 | bucket 10 (≈512) | 0.148 | 0.220 | bucket 9 (≈256) | 0.250 |
| 16 | 0.531 | **bucket 5 (=PN_UNIT)** | **0.446** | 0.433 | bucket 9 (≈256) | 0.282 |
| 20 | 0.507 | **bucket 5 (=PN_UNIT)** | **0.413** | 0.438 | bucket 9 (≈256) | 0.279 |

**根本原因と変化 (v0.49.0→v0.52.1):**

1. **pn KL (depth=4-8):** `heuristic_or_pn` の離散初期化 → bucket 12 (pn≈4096=256S) への集中．
   (se, nc) の有限組み合わせによる離散スパイクが残存．浅い depth では中間エントリが少なく相対的に目立つ．

2. **pn KL (depth=16-20):** v0.52.1 では bucket 6 スパイクが解消された代わりに，
   **bucket 30 (near-INF) が主犯に移行**した．これは WPN AND 多段累積 (§4.3) の結果として
   TT エントリ縮小 (depth=20: 300K→113K) により高 pn 中間エントリの相対比率が増大したため．
   対策の方向性が「離散スパイク解消」から「右テール平滑化」に変化した．

3. **dn KL:** AND-min 伝播による PN_UNIT への下限収束 → v0.52.1 で heuristic_or_dn pn 非依存化により
   bucket 5 集積が緩和され，bucket 9 に移行．depth=16-20 で 0.178-0.208 ポイント改善 ✓

**対応済み (v0.52.1):**

- **dn 側**: `heuristic_or_dn` を pn 非依存 (se, nc) 直接マッピングに変更 (§3.21)
  → dn KL の max 寄与 bucket が bucket 5 (PN_UNIT) → bucket 9 に改善 ✓
  → depth=16-20 で dn KL が 0.178-0.208 ポイント改善 ✓

**残存課題:**

- **pn 右テール (depth=16-20 の主犯):** bucket 30 (near-INF) 中間エントリの存在
  → WPN AND 多段累積が根本原因 (§4.3)．WPN_GAMMA_SHIFT 増大は逆効果 (§3.20)．
  → pn=INF に達する前の中間 AND ノードをより速く確定させる仕組みが必要
- **pn 離散スパイク (depth=4-8):** bucket 12 への集中
  → heuristic_or_pn の初期値に (pos_key 等で) わずかな分散を加える
- KL 目標 < 0.3 は依然未達 (pn: 0.458-0.467，dn: 0.433-0.438 at depth=16-20)
...
[優先度 中]
 └── 課題 B: KL < 0.3 達成 (現状 pn: 0.458-0.467，dn: 0.433-0.438 @ depth=16-20)
       → pn 側: v0.52.1 で max 寄与 bucket が bucket 12 に移動；heuristic_or_pn の離散スパイク残存
