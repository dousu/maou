...
**現状 (v0.43.0，depth=16-28):** pn σ_ln=0.360-0.368，dn σ_ln=0.234-0.268
...
**pn σ_ln (現状 0.368, 目標 ≥ 0.5):**
- AND pn = sum(child_pn) が積み上がりを生む → 改善の余地あり
- `heuristic_or_pn` の初期値分散改善で継続的な改善が見込める
- 課題: depth=16-28 でほぼ横ばい (0.360-0.368)，目標 0.5 まで引き続き改善が必要
...
**次のアクション候補:**
- `heuristic_or_pn`: safe_escapes=1-2 の pn 初期値のさらなる分散で pn σ_ln ≥ 0.5 へ
...
[優先度 高 — pn σ_ln ≥ 0.5 の達成]
 ├── heuristic_or_pn: safe_escapes=1-2 のさらなる分散
 │    └── 現状 pn σ_ln ≈ 0.36，目標 0.5 まで +0.14 の改善が必要
 └── depth=32+ での pn/dn 分布計測 (v0.43.0 では 28 より深い data 未取得)

[優先度 中]
 ├── 課題 B: KL(対数正規) 改善 (depth=16 で pn KL=0.375，目標 < 0.3)
 └── 39手詰め backward 解析 (§10.2) で v0.43.0 の実効性を確認
