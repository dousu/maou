...
**失敗の根本原因 — AND-min 伝播の支配性:**

dn 分布の狭さは WPN γ や clamp の問題ではなく，df-pn の構造的非対称性による:

```
pn 伝播:
  OR  pn = min(child_pn)   ← 小さくなる方向
  AND pn = sum(child_pn)   ← 大きく積み上がる方向  ← 右テール形成
  → AND pn が depth に比例して積み上がり，σ_ln が広がる

dn 伝播:
  OR  dn = WPN sum(child_dn) ≈ max  ← max を軸に小さく蓄積
  AND dn = min(child_dn)   ← 「最も逃げやすい一手」に引き下げられる  ← 支配的
  → AND-min が depth ごとに最小値に向かって収束させる
```

1. **最小値決定則:** AND dn = min(child_OR_dn)．探索済みの OR 葉に dn=4S が一つでも存在すれば AND dn = 4S に収束する
2. **γ 緩和の無効性:** OR dn を γ_dn=4 で大きくしても，次の AND-min で 4S に引き下げられる
3. **clamp 下限緩和の逆効果:** 下限を 1S にすると初期化時 cluster 点が bucket 6→4 に下がり σ_ln が悪化
...
pn と dn で σ_ln の改善難易度が大きく異なることが v0.39.0 実験で判明した:

**pn σ_ln (現状 0.381, 目標 ≥ 0.5):**
- AND pn = sum(child_pn) が積み上がりを生む → 改善の余地あり
- `heuristic_or_pn` の初期値分散改善で継続的な改善が見込める
- 課題: 深い depth (32+) では初期化の効果が薄れ，σ_ln が横ばいになる

**dn σ_ln (現状 0.163, 目標 ≥ 0.5): 構造的制約あり**
- AND dn = min(child_dn) が depth ごとに最小値に収束させる
- OR dn の WPN γ 緩和も AND-min 一回で打ち消される
- clamp 下限緩和は cluster 点を下げるだけで σ_ln が悪化 (§3.10)
- **dn σ_ln ≥ 0.5 は AND-min 伝播を前提とする df-pn では達成困難と判断**

**次のアクション候補:**
- `heuristic_or_pn`: safe_escapes=1-2 の pn 初期値をさらに下方 (bucket 4〜5) に分散して pn σ_ln を改善
- pn σ_ln の深 depth での持続改善 (初期化の効果が depth=32 で薄れる問題)
- dn σ_ln 目標 (≥ 0.5) は現実的に達成不可のため，**dn 目標は σ_ln ≥ 0.2 に下方修正を検討**
...
| 並列探索 | §11.6 の方針により非採用 |
| dn σ_ln ≥ 0.5 の達成 | AND-min 伝播という df-pn の構造的制約により，初期化・WPN 調整では達成不可 (§3.10 で実験的に確認) |
...
[dn σ_ln 改善 — 構造的制約により断念]
 └── dn σ_ln 目標 ≥ 0.5 は達成困難．現実的目標を ≥ 0.2 に下方修正する
...
**σ_ln(pn) ≥ 0.5，σ_ln(dn) ≥ 0.2，KL(対数正規) < 0.3** を近期目標とする (dn 目標を下方修正)．
