...
- **dn IDS 爆発問題 (v0.40.0/v0.41.0 共通):** AND leaf dn への safe_escapes ベース追加は
  「クラスタ点の移動」に過ぎず根本解決に至っていない．heuristic_and_dn_blended を v0.38.0 に戻すか，
  クラスタを分散させる連続スケーリング (pn×escape の連続関数) を検討する
...
[dn σ_ln 改善 — heuristic_and_dn 幾何平均ブレンド計測済み (§3.12)]
 └── v0.41.0: sqrt(pn_dn × escape_dn) ブレンド — 計測完了
       ├── 回帰テスト (ply24): 52s → 25.98s (2× 高速化) ✓
       ├── depth=20 TT: 134K → 2.0M (爆発，v0.40.0 と同等) ✗
       ├── dn σ_ln 全 depth で悪化 (depth=20: 0.217 → 0.179) ✗
       └── 到達 depth: 40 → 28 (50M nodes で) ✗
       結論: クラスタ点が 64S→32S に移動しただけで爆発メカニズムは同一

[優先度 高 — dn IDS 爆発問題の根本解決]
 ├── 方針 A: v0.38.0 相当に戻す (heuristic_dn_from_pn 単体) — dn σ_ln 改善を断念
 ├── 方針 B: pn×escape の連続関数で dn を決定 — クラスタを避けながら safe_escapes を活かす
 └── 方針 C: OR-WPN γ を 6 より大きくし (sum 寄与をさらに抑制) TT 爆発を抑える

[優先度 中 — pn σ_ln 改善の継続]
 ├── heuristic_or_pn: safe_escapes=1-2 のさらなる下方分散 (bucket 4〜5)
 │    └── 目標: pn σ_ln を depth=32 でも ≥ 0.4 維持
