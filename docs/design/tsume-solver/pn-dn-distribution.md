...
**現状 (v0.37.0，depth=32):** pn σ_ln=0.338，dn σ_ln=0.186 (目標 ≥ 0.5)

dn は 95% が bucket 3.7〜7.7 の約 4 幅に集中しており，AND ノードの argmin dn による
子選択精度が低い．pn も bucket 4〜20 の 16 幅に収まっており，改善の余地が大きい．

**根本原因:**
1. `heuristic_or_pn` の初期 pn が bucket 4-7 (1.5S〜5S) の狭い範囲に集中
2. `heuristic_dn_from_pn` の clamp(4S，64S) が dn の初期値を bucket 6〜10 に制限
3. WPN 伝播が ln(bucket) 空間での分散を十分に広げない (γ=6 では非最大子の寄与が 1/64)

**次のアクション候補:**
- `heuristic_or_pn`: safe_escapes=3-7 の値域拡大 (bucket 8-14 への分散) を検討
- `heuristic_dn_from_pn`: 上限を下げる (64S → 32S 等) か全体の clamp 幅を広げる方向を検討
- 各変更後に `analyze_pn_dn_dist.py` で σ_ln の改善を確認
...
[優先度 高 — σ_ln 改善フェーズ]
 ├── heuristic_or_pn: safe_escapes=3-7 の値域拡大 (bucket 8-14 への分散)
 │    └── 目標: σ_ln(pn) ≥ 0.5
 ├── heuristic_dn_from_pn: 上限/clamp 幅の調整で dn 分布を広げる
 │    └── 目標: σ_ln(dn) ≥ 0.5
