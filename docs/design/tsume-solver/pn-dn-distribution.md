...
### 3.24 IDS 直接ジャンプ閾値引き下げ — 採用 (v0.54.2 → v0.54.3)

**動機:** §4.6 調査により，`saved_depth <= 19` の直接ジャンプポリシーが
ply 22 (saved_depth=19) でノード数 9.44M という逆転を引き起こしていると判明．
段階的 IDS (2→4→8→16→19) に切り替えることで TT 暖機を改善できる．

**変更内容:**

| バージョン | 閾値 | 変更 |
|-----------|------|------|
| v0.54.2 | `saved_depth <= 19` → `saved_depth <= 15` | ply 22/24 を段階的 IDS に切り替え |
| v0.54.3 | `saved_depth <= 15` → `saved_depth <= 17` | ply 24 を直接ジャンプに戻す (回帰解消) |

最終閾値 `saved_depth <= 17` での IDS 進行:

| ply | saved_depth | IDS 進行 (v0.54.0) | IDS 進行 (v0.54.3) |
|-----|-------------|--------------------|--------------------|
| 24 | 17 | `2→4→17` (直接) | `2→4→17` (直接，変化なし) |
| 22 | 19 | `2→4→19` (直接) | `2→4→8→16→19` (段階的) ← 修正 |
| 20 | 21 | `2→4→8→16→17→21` (段階的，変化なし) | — |

**39手詰め backward_10m 解析 (v0.54.2 計測, v0.54.3 予測):**

| ply | remain | saved_depth | nodes (v0.54.0) | nodes (v0.54.2) | nodes (v0.54.3 予測) | 変化 |
|-----|--------|-------------|-----------------|-----------------|----------------------|------|
| 24 | 15 | 17 | 682,965 (1m) | 718,463 | **~682,965** (直接ジャンプ復帰) | v0.54.0 と同等 |
| 22 | 17 | 19 | 9,439,291 | **7,625,471** | 7,625,471 (段階的，変化なし) | **−19.2% ↓** |
| 20 | 19 | 21 | 5,333,924 | 5,333,924 | 5,333,924 (変化なし) | 0% |
| 18 | 21 | — | Unknown | Unknown | Unknown | 境界維持 |

v0.54.2 で ply 24 が +5.2% 回帰したのは，depth=16 暖機コスト (~447K nodes) が
depth=17 での節約 (~413K nodes: 682K→269K) を上回ったため:
- 直接ジャンプ: depth=17 cold ≈ 683K 節点
- 段階的 IDS: depth=16 (~447K) + depth=17 warm (~270K) ≈ 717K 節点 → 微増

v0.54.3 の閾値 `<= 17` により ply 24 は直接ジャンプに戻り，回帰を解消する．

**v0.54.2 backward_10m 全結果 (参考):**

```
Ply    Remain  Nodes         Time(s)  MaxPly  TT_pos    Result
38     1       10            0.06     2       19        Mate(1)
36     3       2             0.06     1       4         Mate(3)
34     5       9             0.06     3       17        Mate(5)
32     7       14            0.07     5       25        Mate(7)
30     9       19            0.06     7       29        Mate(9)
28     11      75            0.06     12      118       Mate(11)
26     13      90            0.06     14      122       Mate(13)
24     15      718,463       71.05    16      54,736    Mate(15)  ← v0.54.2 回帰 (+5.2%)
22     17      7,625,471     213.26   18      26,540    Mate(17)  ← v0.54.2 改善 (−19.2%)
20     19      5,333,924     32.70    20      337,060   Mate(19)
18     21      10,000,000    99.71    22      0         Unknown   ← 10M 境界
```

**テスト結果:** `cargo test --release -p maou_shogi` 全 136 テスト通過 ✓
...
上記の結論は「pn=INF エントリをそのまま保持する」場合に成立するが，
**pn=INF を有限値にキャップしてから保持する**アプローチは別の検討が必要:

- counter-check バグの本質は `dn=0` エントリによる「詰み証明済み」の偽陽性
- `retain` の `pn > 0 && dn > 0` フィルタ (is_intermediate) がこれを防ぐ
- `pn=INF` の廃棄は counter-check fix の**必要条件ではなく**，当時の過保守な判断

pn=INF → LARGE_CAP 変換 + dn > 0 フィルタ維持という設計により
counter-check 安全性を保ちながら TT 引継ぎを改善できる可能性がある (§4.7)．
...
### 4.6 [課題 F → 解決済み] IDS 直接ジャンプポリシーによる TT 暖機不足 — ply 22 > ply 20 逆転の根本原因
...
`collect_pn_dn_dist_per_depth()` API による IDS per-depth ブレークダウン:

**ply 22 (remaining=17, saved_depth=19) — 直接ジャンプ:**
...
**ply 20 (remaining=19, saved_depth=21) — 段階的 IDS:**

```
  ids_depth  cumul_nodes      incr_nodes       TT_work
  2          110,008          110,008          9
  4          110,068          60               89
  8          111,601          1,533            3,562
  16         559,089          447,488          250,219
  17         2,919,316        2,360,227        311,197
  21         5,333,924        2,414,608        (final depth, no snapshot)
```

`TT_work` = 各 ids_depth フェーズ終了時点の TT エントリ数 (次 depth の入力 TT サイズ)．

**ply 22 vs ply 20 の最終深さ直前 TT:**

| | ply 22 (直接ジャンプ) | ply 20 (段階的 IDS) |
|--|----------------------|---------------------|
| IDS 進行 | `2→4→`**`19`** | `2→4→8→16→17→`**`21`** |
| 最終深さ入力 TT | **121 件** (ids_depth=4 終了時) | **311,197 件** (ids_depth=17 終了時) |
| 入力 TT 差 | — | **2,572 倍** |
| 最終深さ増分ノード | 9,289,204 | 2,414,608 |
| 最終深さノード比 | **3.85 倍** | — |
| 総ノード数 | 9,439,291 | 5,333,924 |
| nodes/総TT 比 | 252 nodes/entry | 16 nodes/entry |

ply 20 は 6 フェーズの段階的 TT 構築により，最終深さ (ids_depth=21) を
わずか 2.41M ノードで突破できる．ply 22 は TT=121 件のほぼ空の状態から
ids_depth=19 を開始し，9.29M ノードを消費する．

**IDS-17 中間ステップの効果:**

ply 20 の ids_depth=17 フェーズ (2.36M ノード，TT: 250K → 311K) は，
単独では最も高コストなフェーズだが，最終 ids_depth=21 での探索コストを
大幅に削減する「投資」として機能している．

#### ply 22 各深さの独立コスト (sub-depth 分析)

各深さをフレッシュ TT から独立実行した場合のコスト
(10M ノード上限 + 120s 時間上限; どちらか先に到達した時点で打ち切り):

| depth | nodes | time(s) | TT_pos | result | 打ち切り理由 |
|-------|-------|---------|--------|--------|------------|
| 4 | 158 | 0.22 | 122 | NoMate | 完了 |
| 8 | 4,469 | 0.70 | 7,304 | NoMate | 完了 |
| 12 | 1,659,294 | 12.76 | 39,827 | Unknown | 完了 (深さ不足) |
| 16 | 10,000,000 | 45.03 | — | Unknown | ノード上限 |
| 17 | 8,565,192 | 120.51 | — | Unknown | 時間上限 |
| 18 | 10,000,000 | 29.88 | — | Unknown | ノード上限 |
| 19 | 4,163,321 | 120.14 | — | Unknown | 時間上限 |

- depth=4, 8 はフレッシュ TT でも軽量 (冷スタートコストが小さい)
- depth=12 は 1.66M ノードで "Unknown" 止まり (詰みは 17 手，depth 12 では到達不能)
- depth=16–19 はいずれも単独では 10M ノード・120s 以内に解けない
- depth=19 フレッシュ (4.16M ノード/120s) vs IDS 2→4→19 (9.29M ノード/260s):
  IDS の TT 121 件でも最終的には解決できているが，フレッシュ depth=19 では
  120s 時間切れで解けない — 最小限の暖機でもゼロよりはるかにマシ

ply 20 の段階的 IDS では depth=8 (1,533 ノード), depth=16 (447K ノード),
depth=17 (2,360K ノード) が warm-up として機能し，最終 depth=21 が
わずか 2.41M ノードで解けるようになっている．
...
ply 20 のデータから `saved_depth=21` での IDS-17 ステップが確認された:
`2→4→8→16→17→21` の 6 ステップで TT=311K 件を構築する．
同様に `saved_depth=19` を段階的にした場合は `2→4→8→16→17→19` となる見込み
(ids_depth=17 → next=21 > 19 → `next.min(saved_depth)=19` で終端)．
実際の効果は benchmark で検証が必要．

**対策実施 (v0.54.3):** 閾値を `saved_depth <= 19` → `saved_depth <= 17` に引き下げ (§3.24)．
- ply 22: 9,439,291 → **7,625,471 nodes (−19.2%)** ✓
- ply 20: 5,333,924 → 5,333,924 (変化なし) ✓
- 10M 境界: ply 18 で変化なし ✓
- ply 24 (saved_depth=17): 直接ジャンプを維持し回帰なし ✓

**現在の優先度:** 解決済み

---

### 4.7 [課題 G] IDS 間の WorkingTT 引継ぎ改善 — pn=INF ソフトキャップと件数予算

#### 背景と現状

`retain_working_intermediates` (tt.rs:2159) の保持条件:

```rust
let keep = is_intermediate       // pn > 0 && dn > 0
    && entry.pn < u32::MAX       // ← pn=INF を全廃棄
    && !is_path_dep
    && is_depth_limited          // remaining < REMAINING_INFINITE
    && rem >= min_remaining
    && new_rem < REMAINING_INFINITE;
```

§4.5 で確認したように depth=24 TT の **85% (約 615K 件) が pn=INF** であり全廃棄される．
これが depth=28+ の cold start 回帰の原因である (§4.5 参照)．

同様に v0.54.3 で修正した ply 22 問題 (§4.6) も，IDS 深さ遷移ごとに
pn=INF エントリが廃棄されることで TT 暖機が不十分になっていた側面がある．

#### counter-check 安全性の再評価

pn=INF を廃棄している理由は当初「counter-check バグ防止」とされていたが，
§4.5 の分析で counter-check fix の本質は **`dn > 0` フィルタ** (is_intermediate) であると確認済み:

- counter-check バグ: `dn=0` エントリが delta-shift 後に「詰み証明済み」として再利用される偽陽性
- `pn > 0 && dn > 0` フィルタ: dn=0 エントリを排除 → counter-check バグを防止
- `pn < u32::MAX` フィルタ: counter-check fix の**必要条件ではなく**過保守

したがって `pn=INF` を **有限値にキャップしてから保持** しても counter-check バグは再発しない．

#### 設計方針: 2 層アーキテクチャ

```
Layer 1 — 適格条件の拡張: pn=INF ソフトキャップ
  pn=INF → RETAIN_INF_CAP (有限) に変換
  "depth D では証明不能 (hard claim)"
    → "depth D では困難だが D+Δ では証明可能かも (soft hint)"

Layer 2 — 件数予算: 充填率制御
  保持件数が budget N を超える場合，優先度の低いエントリを削除
  TT の充填率を manageable に保ち lookup 効率を維持
```

#### Layer 1: pn=INF ソフトキャップの実装

```rust
// tt.rs retain_working_intermediates の変更案
let keep = is_intermediate          // pn > 0 && dn > 0 (counter-check 安全性は維持)
    // && entry.pn < u32::MAX       ← この条件を削除
    && !is_path_dep
    && is_depth_limited
    && rem >= min_remaining
    && new_rem < REMAINING_INFINITE;
if keep {
    if entry.pn == u32::MAX {
        entry.pn = RETAIN_INF_CAP;  // hard claim → soft hint へ降格
    }
...
**`RETAIN_INF_CAP` の設定指針:**

`heuristic_or_pn` の最大値域 (~4096×PN_UNIT) に揃えると整合的．
小さすぎると solver が無駄に展開し，大きすぎると INF と同等にスキップされる．

既存の `retain_pn_dn_cap` パラメータを活用できる:
`retain_pn_dn_cap = 4096×PN_UNIT` に設定すれば `pn=INF → min(INF, 4096×PN_UNIT)`
が自動適用され，Layer 1 と Layer 2 の値上限を同一パラメータで制御可能．

#### Layer 2: 件数予算と優先度指標

pn=INF エントリを保持対象に加えると，保持件数が大幅に増加し
TT の充填率が上昇して hash collision が増加する可能性がある．

**優先度指標: pn 昇順 (低 pn 優先)**

| 根拠 | 説明 |
|------|------|
| 次 depth で最初に展開される局面 | 証明経路を辿る上で最も直接的に有用 |
| pn が低い = 「証明しやすい」 | 残りの探索コストが低く，TT に残す価値が高い |
| pn=RETAIN_INF_CAP は低優先度 | budget 超過時に最初に削除される → 自然な eviction |

**実装案の比較:**

| 案 | 計算量 | 優先度品質 | 実装コスト |
|----|--------|-----------|-----------|
| 単純カウント打ち切り (hash 順) | O(TT) | 低 (ランダム同等) | 最小 |
| `retain_pn_dn_cap` の動的設定 | O(TT) × 2pass | 中 | 小 |
| ヒープ top-N (pn 昇順) | O(Q log N) | 高 | 中程度 |

ハッシュテーブルの配置はランダムに近いため，単純カウント打ち切りでも
実質的に pn 値のランダムサンプリングと同等の効果が得られる可能性がある．
まず単純案で効果を計測してから高精度案に移行するのが低リスク．

`retain_max_count: usize` を新パラメータとして追加し，
`min_remaining` と組み合わせて件数・深さの両面から制御する:
...
pub(super) fn retain_working_intermediates(
    ...
    min_remaining: u16,
    delta_remaining: u16,
    max_count: usize,      // 新パラメータ; usize::MAX で無制限
) -> usize {
    let mut kept = 0;
    for fe in self.working.iter_mut() {
        if qualifies(fe) && kept < max_count {
            cap_and_keep(fe); kept += 1;
        } else {
            fe.pos_key = 0;
        }
    }
...
| 指標 | 現状 (v0.54.3) | 改善後 (期待) |
|------|---------------|--------------|
| depth=24→28 の保持率 | ~15% (非 INF のみ) | ~100% (INF → LARGE_CAP) |
| ply 22 の最終 depth 入力 TT | ~250K 件 (depth=16 由来) | 更なる充実の可能性 |
| depth=28+ backward 境界 | ply 18 以下は 10M 以内不解 | 30M-100M 予算での改善の可能性 |
| 39手詰め main search の到達 depth | depth=40 (50M nodes, v0.54.0) | 更なる深化の可能性 |

**計測方法:**
- `#[cfg(feature = "tt_diag")]` の `diag_retained_pn_dist` で保持エントリの pn 分布を確認
- `test_tsume_39te_backward_10m` で ply 22/20 の nodes 変化を確認
- TT 充填率の変化は `drain_working_overflow()` カウンタで監視
- depth=28+ の影響は 30M-100M 予算の backward 解析が必要

**現在の優先度:** 優先度 中
(現行 10M 予算の backward 境界 (ply 18) には影響しないが，
深い詰将棋・100M+ 予算での解析に高インパクトが期待される)
