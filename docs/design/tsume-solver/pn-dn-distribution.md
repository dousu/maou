...
1. **pn: heuristic_or_pn 離散スパイク:** bucket 6-7 への集中 (v0.51.1 では bucket 7 に移行，v0.52.1 で max 寄与 bucket 12 に変化)
2. **dn: AND-min 伝播による PN_UNIT 収束:** bucket 5 (dn=16=PN_UNIT) への張り付き (v0.52.1 では bucket 9 (dn≈256) に改善)
...
- **pn 側**: `heuristic_or_pn` の bucket 6-7 への集中を分散させる
  → v0.52.1 では max 寄与 bucket が bucket 12 (2^11) に移動しており，
    新たなスパイクが発生している可能性あり
