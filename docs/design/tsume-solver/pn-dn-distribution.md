...
**計測予定:** 50M nodes backward 解析で depth=20 の TT サイズおよび dn σ_ln を v0.38.0・v0.40.0 と比較．
...
[dn σ_ln 改善 — heuristic_and_dn 幾何平均ブレンドを採用中]
 └── v0.41.0: sqrt(pn_dn × escape_dn) ブレンド — 計測待ち (§3.12)
       ├── 仮説: safe_escapes=3+ ノードの dn 削減で TT 爆発を回避
       └── [SLOW] 50M backward 解析で depth=20 TT サイズおよび dn σ_ln を計測予定

[優先度 高 — v0.41.0 計測]
 ├── [SLOW] test_tsume_39te_ply24_mate15_regression: 回帰確認 (TT 爆発がないか)
 └── [SLOW] 50M backward 解析: depth=20 TT・dn σ_ln を v0.38.0/v0.40.0 と比較
