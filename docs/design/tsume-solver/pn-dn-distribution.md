...
**現状 (v0.51.1，depth=16-24):** pn σ_ln=0.378-0.391，dn σ_ln=0.252-0.275
...
| depth | v0.49.0 pn | v0.51.1 pn | v0.49.0 dn | v0.51.1 dn |
|-------|-----------|-----------|-----------|-----------|
| 8 | 0.395 | 0.395 | 0.128 | 0.155 |
| 16 | 0.473 | **0.412** ↑ | 0.531 | **0.611** ↓ |
| 20 | 0.473 | **0.411** ↑ | 0.507 | **0.646** ↓ |
| 24 | 0.507 | 0.503 | 0.683 | **0.748** ↓ |

目標 (< 0.3) に対して，v0.51.1 で pn KL は depth=16/20 で 0.061-0.062 改善したが，
dn KL は全 depth で悪化 (depth=20 で +0.139)．詳細な根本原因は §4.3/§4.4 で調査済み．

KL が高い主要因 (§4.3/§4.4 調査結果より):
1. **pn: heuristic_or_pn 離散スパイク:** bucket 6-7 への集中 (v0.51.1 では分散済みだが bucket 7 に移行)
2. **dn: AND-min 伝播による PN_UNIT 収束:** bucket 5 (dn=16=PN_UNIT) への張り付き (depth=24 で KL 寄与 0.748)
...
**残存課題:** dn KL の悪化メカニズム (heuristic_or_pn↑ → heuristic_dn↓ → AND min が PN_UNIT 収束) を
直接対処する手段が未確立．§4.4 の dn 側対応方針を参照．
...
[優先度 中]
 └── 課題 B: dn KL のさらなる改善 (現状 depth=20 で 0.646；目標 < 0.3)
       → dn 側の直接対策: heuristic_dn_from_pn の反比例を緩和する代替式の検討
         (例: dn = C * pn^(-α) で α < 0.5 に変更，または dn 初期値にバリエーションを追加)
       → pn 側: depth=24 の pn KL 0.503 はまだ高め (bucket 7 集中, §4.4)
