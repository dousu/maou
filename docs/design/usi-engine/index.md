...
#### 8.1.1 手数カーブ `w(手数)` — 中盤重み付け

序盤は定跡化された分岐で探索時間の限界効用が低く，終盤は dfpn の詰み判定が
効く．そこで `w` を `peak_ply` 頂点の山型の折れ線とし，中盤に配分を寄せる．
**既定は off** (`w ≡ 1.0`)．棋力に効くレバーなので，自己対局 A/B で
「より強い」を確認してから既定化する．
...
1. **乗数は裁量枠 (`残時間 / horizon` = バンクの取り崩し) にのみ掛け，
   ...
   終盤が加算ペースに収束したうえでバンクを中盤へ寄せられる．三態の
   分岐を増やさずに両方が正しくなる．
2. **底は序盤側と終盤側で別に持つ**．序盤は「探索時間を与えても結果が
   あまり変わらない」と強く言えるが，終盤は「少し短くても問題ない場合が
   **少し**ある」という弱い主張しか立たない．対称な山にすると，毎手
   戻ってくる時間が無い切れ負けで終盤が必要以上に痩せる (実測: 5 分
   切れ負けの ply 99 で 1.2s → 0.3s)．終盤の底は既定 1.0 倍 = 一律配分と
   同じに置き，A/B の差分を中盤の山だけに絞る．
3. **自己正規化**．配分の土台が常にその時点の残時間の分数なので，カーブは
   配分を歪めるだけで総消費を増やす方向には倒れない (使えば残りが減り，
   以降の配分が自動的に小さくなる)．

パラメータ (既定): 頂点 `peak_ply = 55` / 半幅 `half_width_ply = 35` /
頂点の乗数 `1800` permille / 序盤側の底 `700` / 終盤側の底 `1000`．
いずれも A/B の出発点であって決着値ではない．

**一律配分 (現行) は単調減少である**という事実がこの検討で確定した．
`残時間 / horizon` は等比減衰なので，残時間が最大の初手が必ず最大配分に
なる．カーブはこの山を中盤へ移すもので，「厚くする」より先に「初手に
寄っていたものをずらす」機構と理解するのが正しい．

なお `horizon_moves` 固定に由来する**未消化**は別問題として残る．50 手
指すと `(39/40)^50 ≈ 0.28` が構造的に残り，5 分切れ負けで 84.6s (カーブ
off) / 68.1s (on) を使い残す．終盤側の底を 1.0 より上げれば同じノブで
末端消化を表現できるが，`horizon` を 20 に縮める (= 速く使う) 方向は
過去の A/B で **−89 Elo** なので「速く使えば強い」ではない
([verification.md §4.4](verification.md))．
