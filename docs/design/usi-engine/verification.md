...
- 実効速度 (L4 / ViT 19.8M fp16 / threads 1): **単発 30 秒 = 10,909
  ...
  まで評価バッチが埋まらないため．
- 単発 30 秒の実測幅は **4,595-10,909 playouts/秒**．同一コマンド・同一ビルドで
  1 時間のうちに 10,095 → 4,595 = **2.2 倍の低下**を観測した．**原因は未特定**．
  会計の水増しではない (空回りは 0.008% = 303,171 playouts に対し 24 件，
  `elapsed_ms` も 30,031 / 30,065 でほぼ同一) が，**GPU が遅くなったのか，
  バッチ充填率が落ちたのかは 2.2 倍の観測時点では区別できていなかった**:
  ...
  **充填率が下がると GPU の仕事量が同じでも `playouts`/`nps` が下がる**．
  ...
  `collisions` を出すようにした (`maou 0.60.2`)．**次に測るときは
