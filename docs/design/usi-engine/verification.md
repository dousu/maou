...
### 4.4.1 未決: 手数カーブ (中盤重み付け) の棋力影響 — 未測定

`--ab-mode timecurve` (A = カーブ on / B = 一律配分)．設計は
[index.md §8.1.1](index.md)．**既定 off の解除条件は「A/B で有意に勝つ」**
であり，それまで既定化しない (VETO: レバーは A/B で確認してから既定化)．

**regime ゲート** — 通さないと測っても意味が無い:

1. **持ち時間モード必須**．固定 playout 予算では配分そのものが存在しない
   ので，`--ab-mode timecurve` は `--clock-ms` 無しでは弾かれる．
2. **GPU で測る**．将棋は 1 手進むごとにノードが爆発的に増えるので，
   中盤に積んだ時間が探索の質に変わるかは NN の速度に依存する．
   DevContainer の 22 p/s では中盤の増分が棋力差になる前に飽和する
   (compass: CPU→GPU の外挿は双方向に危険)．
3. **機構の発火を別軸で確認する**．A/B の前に，A 側の 1 手あたり消費が
   実際に中盤で山を作っているかを対局ログで見る．「効果なし」の前に
   「そもそも掛かっていない」を潰す (レバー A/B の TRIPWIRE)．

**手順** (Colab L4 / `--threads 1 --batch-size 64 --tensorrt --cuda`):

```
maou selfplay --ab-mode timecurve --clock-ms 300000 --inc-ms 10000 \
  --games <n> --model-path <onnx> --threads 1 --batch-size 64 \
  --tensorrt --cuda --trt-cache-dir <dir>
```

持ち時間は実戦相当に取る (floodgate 合わせなら 300000 + 10000)．
**n は Elo 分解能から決める**: n=40 は ~150 Elo 級しか検出できないので，
カーブの効果が数十 Elo なら n を増やすか，効果が出ないという結論は
「この n では検出できない」までしか主張しない．

パラメータ (`--time-curve-peak-ply` ほか 4 つ) は A/B の**両者に同じ値**が
入り，差は on/off だけになる．山の位置や振幅を変えたい場合は，パラメータを
変えた条件で A/B を回し直す (A 側だけ変えて比較しない)．

**切れ負けでの regime 差に注意**．終盤側の底は既定 1.0 倍 = 一律配分と同じ
なので，フィッシャー/秒読みと切れ負けでは効き方が違う．片方の結果を
もう片方へ外挿しない．
