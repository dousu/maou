...
print("判定:", "機構は効いている" if tally["A"] < tally["B"]
      else "差なし (両者 0 なら基準率が低すぎて判別力が無い)" if tally["A"] == tally["B"]
      else "要調査")
```
...
#### 4.7.2 修正後の再測定 (2026-08-02/03) — **結論: 発火は実証，Elo 差は検出限界未満**

playout 流入の修正 (`d8f3d78`) 後に Stage A / C / D を再実行した．

| Stage | 修正前 | 修正後 |
|---|---|---|
| A-1 選ばれる手の visits | 24 (0.02%) | **94,976 (73.4%)** |
| A-1 敗着への流入 | 123,306 (99.4%) | 34,054 |
| **C: A スコア** | 45.0% (15W 6D 19L) | **55.0% (22W 0D 18L)** |
| **C: Elo** | **−35** [−141, +72] / t = −0.81 | **+35** [−72, +141] / t = **+0.57** |
| C: ペア | A 優位 4/20 (同着 11) | A 優位 7/20 (同着 8) |
| D: 避けられた敗着 | A=1 / B=1 | **A=0 / B=0** |
| 局数・手数 | 4,561 手 / 引き分け 6 | 5,604 手 (平均 140) / **引き分け 0** |

**符号が反転した (−35 → +35, 70 Elo の振れ)**．修正が効いた方向と整合するが，
**どちらも有意ではない**ので，これ自体を証拠として扱ってはいけない．
有効なのは修正後の測定のみ (修正前は壊れた版の測定値)．

#### これ以上 A/B に GPU 時間を使わない根拠

実測のペア分散 (mean +0.100 / SE 0.176 / 20 ペア → ペア SD 0.787) から，
**t = 2.0 に到達するには 248 ペア = 496 局**が要る．L4 実測 40 局 = 21.1 時間
なので **約 11 日**．逆に **n=40 が検出できるのは ~128 Elo 以上**の効果だけ．

Stage D も **A=0 / B=0** で飽和しており，判別力が無い (そもそも件数は上界)．
**この regime でこの効果量を A/B で確定させるのは費用対効果に合わない**．

#### B 側 (レバー off) は代替経路で守られてはいない

Stage D が **A=0 / B=0** だったため「B にも受け方向が効いているのでは」という
疑いが出た．**効いていない**．

コード: `defensive_mate` で gate される箇所は 5 つ (root 受け方向 dfpn の
spawn / 王手中の葉の受け方向 leaf-mate / `apply_root_filter` / root PUCT の
確定済み子の除外 / mate worker の起動) で，`--ab-mode defmate` は
`engine_b.defensive_mate = Some(false)` を置くため B では全て無効．

実測 (同じ ply117 局面を `--no-defensive-mate` で):
...
**攻め方向の機構 (leaf-mate + MCTS-Solver の AND-OR 伝播) が捕まえられるのは
mate-1 の 5h5i だけ**で，mate-29 / mate-41 は対局中の 5 倍の時間を与えても
証明できず，負ける手を選び続ける．**攻め方向は受け方向の代替にならない**．

したがって B=0 は「B が守られている」ではなく，**この型の敗着の基準率が
40 局あたり 0〜1 件しかない**ことを意味する．加えて Stage D は**同一局面の
対照比較ではない** — A の挙動が変われば以降の進行も変わり，A と B が直面する
局面集合はそもそも別物になる (round 2 は引き分け 0 / 平均 140 手，round 1 は
引き分け 6 / 平均 114 手と進行自体が違う)．**Stage D はこの regime では
判別力を持たない**．

#### 結論と扱い

**「発火したが Elo 差は検出限界未満」** であり，**「発火しなかった」ではない**．
根拠:

- **機構は実証済み** — A-1 の対照 (受け方向 off) は実際に負けた敗着 `4f4g` を
  選ぶ．on では `5h6i` を選び，playout の 73.4% がその手に入る
- **コストは実質ゼロ** — 静かな局面で nps −0.9% (むしろ微増)，充填率不変
- **点推定は正** (+35 Elo)，ただし CI が 0 を含む

したがって **既定 on を維持する**．ただし **「A/B で確認済み」ではない** —
compass § VETO の例外 (user の明示判断) のまま扱う．覆すには 500 局規模の
A/B が要るので，**再測定は他の理由 (回帰の疑い等) が出たときに限る**．
