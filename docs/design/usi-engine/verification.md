...
- 実効速度 (L4 / ViT 19.8M fp16 / threads 1): **単発 30 秒 = 10,909
  ...
  まで評価バッチが埋まらないため．
- 単発 30 秒の実測幅は **4,595-10,909 playouts/秒**．会計修正後の再計測で
  空回りは **0.008%** (303,171 playouts に対し 24 件) しかないので，この幅は
  水増しではなく **Colab の session 変動**である (同一コマンドで 1 時間のうちに
  10,095 → 4,595 = **2.2 倍の低下**を観測)．
  ⇒ **絶対値を session をまたいで比較してはいけない．throughput 比を出す計測では，
  比較対象の天井 (単発 30 秒) を同じ session 内で測り直すこと**．
