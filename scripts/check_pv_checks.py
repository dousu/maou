...
def is_in_check(board: PyBoard) -> bool:
    """現在の手番側の玉が王手されているかを判定する.

    相手の合法手に玉を取る手が含まれているか確認する方法は使えないので，
    代わりに: 相手側から見て玉の位置に利きがあるか確認する．
    簡易的に: 現在の局面のSFENと合法手から判定する．
    """
    # solve_tsume で1手詰めかどうかではなく，
    # 実際に王手されているかを調べたい
    # PyBoardのAPIでは直接の王手判定がないので，
    # 合法手の中に「玉を取る手」があるかで判定する
    # → これは「手番側が相手の玉を取れるか」= 相手が王手放置したか

    # 現在の手番の合法手を取得し，相手玉の位置への手があるか確認
    # この方法だと王手判定ではなく「相手の前の手が合法か」の判定になる

    # 別の方法: PVの手が攻め方(先手)の手なら，その手の後に
    # 後手の玉が王手されているか → solve_tsumeが王手生成のみなので
    # その局面でsolve_tsume(depth=1)して1手で解けるか確認
