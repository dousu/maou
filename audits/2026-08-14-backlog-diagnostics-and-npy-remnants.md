...
した．**`AskUserQuestion` は上げていない** — 判断帯の 1 件は PR 本文に
決定を載せる形にし，ユーザがその PR に対して同一セッション内で回答した．
...
クラス毎の PR 分割ができないため，レビュー単位は commit が担った．
自動帯 3 commit と判断帯 1 commit が同居している．

判断帯は `9191a0b` (N9, `scripts/` 2 本の削除) の 1 件のみ．PR
[#498](https://github.com/dousu/maou/pull/498) を未マージで開いて判断を
載せたところ，**同じセッション内でユーザが「削除する」と回答**した．
5e の「ユーザがそのセッション内で答えた場合は，決定を適用し QA を回し
直して 5d の条件下でマージする」に従い，実装は既にその向きだったので
差分の変更は無く，QA を回し直したうえで `main` へマージした．

`.feather` 版への書き直し案は採らない．理由は上記 `9191a0b` の節のとおり
で，**修理ではなく 4 つの変わった API に対する書き下ろし**になるため．

N9 の行はこの PR 内で削除した — 6a の separability test により，collapse
した run では行削除と修正が同じ PR に載って一緒に受理/棄却されるので
乖離しない．**in flight で残した PR は無い．**
