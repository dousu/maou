...
`AskUserQuestion` を 1 回，**受理 1 問 + 設計判断 3 問**で上げ，4 件とも
回答を得た．

### Q1 (受理, multiSelect) — PR #502 に載せた 2 件

> この run が出した修正のうち，マージしてよいものを選んでください
...
→ `232358e` (P6) と `aad00d9` (P4) をそのままマージ．

### Q2 (設計判断) — Deferred 5/6/7 の G1 の扱い

> G1「この環境で測れない」だけを理由に 6 run 滞留している．どう扱うか

| 選択肢 | 結果 |
|---|---|
| **意味論的等価なものは出荷可 (推奨)** | **選択された** |
| 3 行とも GPU 測定まで凍結 | — |
| Deferred 5 は削除，6/7 は凍結 | — |
| 3 行とも backlog から削除 | — |

**1 つの回答が 3 行を governs した**が，効き方は行ごとに違う:

- **Deferred 6 — G1 を retire**．`stream.synchronize()` →
  `compute_stream.wait_stream(stream)` は意味論的に自明な等価変換で，
  `record_stream()` は既に入っている．**人間待ちではない通常作業**に
  なった (残るのは 1 行の置換 + 回帰テスト)．
- **Deferred 5 — G1 は残る**．意味論的に自明な置換ではなく，産出者が
  ゼロという前提に立つ性能改善なので，mask を配線する変更と同時に
  GPU 上で測って直す方針は不変．
- **Deferred 7 — G1 は残る**．device スカラーへの蓄積は値が
  materialize するタイミングを変え，その値が
  `gradient_accumulation_steps` を書き換えるので**数値等価性の確認が
  必要**．

**実装は次 run 以降** — Deferred 6 は着手可能だが，受理済みの PR #502 に
後から scope を足すことになるため見送った (前 run が O5(c) で同じ判断を
している)．

### Q3 (設計判断) — D13 の向き

> `ColumnarBatch` を直接スライスする方向へ踏み込むか

| 選択肢 | 結果 |
|---|---|
| **ColumnarBatch を直接スライスする (推奨)** | **選択された** |
| 現状維持 (structured array を維持) | — |
| `get_items` だけベクトル化 | — |
| まず計測してから決める | — |

**1 つの回答が (a) の根本と (b) の両方を governs する**．**G4 retire**，
**G2 は残る** (`app/learning/dataset.py` と ABC に触るので path 外)．
**実装は次 run 以降**．

### Q4 (設計判断) — Deferred 2 の向き

> ~600 行の Stage 統合をやるか，重複を受け入れるか

| 選択肢 | 結果 |
|---|---|
| 重複を受け入れて backlog から落とす (推奨) | — |
| **統合する (TrainingLoop 差も設計に含める)** | **選択された** |
| 工場の 28 行一致だけ統合する | — |
| Deferred 3 の結果を見てから決める | — |

**推奨した「落とす」は却下された**．4 本目の軸である `TrainingLoop`
サブクラスの差は**戦略として注入する**設計に含める．**G4 retire**，
**G3 は残る** (等価性をこの環境で示せないので出荷にはユーザ側での実地
確認が要る)．**実装は次 run 以降**．

### 予算に入らなかった設計判断 (次 run の待ち行列)

1. **O9 のキー列** — 決定 (b) 決定的ハッシュ条件の要になるが，
   「現行スキーマに一意キーの保証があるか」が未調査なので，質問の前に
   調査が要る．順位を下げた
2. **B4 (D14(b) + N6-2) の着手順** — 決定済みで人間待ちではないため
   設計判断ではない．次 run の筆頭作業候補

## 決定の throughput

| 区分 | 件数 |
|---|---|
| resolved (行を削除) | **2** (O5, Deferred 3) |
| decided (行は残るが人間待ちを脱した) | **2** (D13, Deferred 2) + **1** (Deferred 6 の G1 retire) |
| re-triaged (人間待ちのまま文言を鋭くした) | **2** (D14(b)+N6-2, O9) |

**decided が 3 件**あり，うち Deferred 6 は 1 行の置換まで縮んでいる．
