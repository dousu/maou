...
**Deferred 3 の「`isinstance` 分岐は選び分けのためだけ」は誤りで，
統合コストも過小評価している** (2026-08-13, `686bcae`)．record は
`stage_component_factory.py:866-872` (現 `:876-882`) の分岐が
「同一の 2 クラスを選び分けるため *だけ*」に存在するとして，統合すれば
そのまま消せるとしている．実際にはこの分岐の `else` 腕は
**未対応の head 型を `TypeError` で弾く検証**を担っており，落とすと
これまで拒否されていた head 型が黙って通るようになる．

さらに record が触れていない罠がある: 6 つの名前が「公開でテストから
参照されている」ことは書かれているが，**そのテストが何を主張して
いるか**が見られていない．`tests/maou/app/learning/test_stage_component_factory.py:297`
は `isinstance(components.model, Stage1ModelAdapter)`，`:398` は
`isinstance(..., Stage2ModelAdapter)` を主張しているので，両名を同一
クラスの別名にすると**どちらのアサーションも自動的に通り，2 本の
テストが識別力を失う** — Stage 1 のテストが Stage 2 のアダプタを
