...
**ユーザは (a) を選択．** 最上流で一度だけ保証するので全経路が自動的に
正しくなり，罠が構造的に消える．却下理由も記録した — (b) は
`streaming_file_source.py` など `FileManager` を通らない経路に効かず
同じ罠が別の場所で再発しうる，(c) は writeable チェックが存在する
理由 (テンソルが共有ストレージへ書き戻すのを防ぐ) を壊す．

**settles**: D13 行．**実質的に再発していた G4 を retire した** —
D13 は再び「決定済み，あとは作業」に戻り，残るのは **G2** だけ．

**本 run では実装しない．** 残る作業は `_explode_list_column` の
writeable 保証に加えて `KifDataset` 側のスライス経路の新設・
`FileManager.get_item`/`get_items` の縮退・hcpe 経路との分岐整理で，
**G2** (`app/learning/dataset.py` と ABC に触る) の作業量が本 run の
枠に入らなかった．行に決定と，(1)〜(4) の順序付き作業を書いたので，
次 run が通常作業として拾える — **次 run の先頭候補**．

なお行には**回帰テストの書き方の罠**も書いた: writeable 保証の
テストは **polars 側 dtype が目標 dtype と一致する列**
(`List(UInt8)` → `uint8` 等) で書かないと空虚になる．Python の list
から組んだ DataFrame は int64/float64 になり `astype` が挟まって
必ず writeable になるためで，本 run が D13 の罠を見つけたのと同じ
理由である．

### 予算に入らなかった設計判断 (次 run の待ち行列)

本 run は 4 問の枠のうち 2 問しか使っていないので**待ち行列に
積み残しは無い**．
...
**なし．** Q1 / Q2 とも**同一セッション内で回答を得た**ので，判断帯の
PR を開いたまま持ち越していない．

**PR #504** (base: `main`) が本 run の唯一の PR で，N10 の修正と
`audits/` の台帳・記録が同居している (指定ブランチ 1 本の制約で
クラス毎の分割ができないため — レビュー単位は commit が担う:
`033d49f` = 修正，`57d9bba` + 本記録の追記 = 台帳・記録)．
Q1 の受理を得てマージした．

backlog 行を 1 つも削除していないため，6a の分離可能性の試験は
本 run では発火しない (削除が無いので分岐が無い)．Q2 の決定は
**行に書いて残す**種類の産出物なので，同じ PR に載っている．
...
```
6 = 1 resolved + 0 in flight + 1 decided + 4 re-triaged + 0 new rows + 0 not-a-finding
```

- **resolved (1)**: N10 — 本 run 内で起票・修正・PR 化・マージ．行を
  作っていないので削除する行も無い．6a の分離可能性の試験では
  「修正と同じ PR」に当たる (指定ブランチ 1 本なので必然)．
- **decided (1)**: **D13** — Q2 でユーザが (a) を選択．行に決定を
  書き，実装可能性の問題として実質的に再発していた **G4 を retire
  した**．修正は未出荷 (G2 の作業量が枠に入らなかった) なので行は
  残る．**人間待ちではなくなった**ので re-triaged とは別に数える．
- **re-triaged (4)**: Deferred 2 / Deferred 5 / Deferred 7 / O9．
  うち **1 件 (Deferred 2) は "changed shape"** で，文言を鋭くする
...
1. **D13 が一度後退してから前進した** — 「あとは作業」から「もう一度
   決めが要る」へ落ち，同じ run 内で決めが取れて「あとは作業」へ
   戻った．差し引きゼロに見えるが，**着手してから判ったのでは遅い**
   種類の情報 (本番データでしか落ちない罠) を，実装前に捕まえて
   決着させている．
2. **Deferred 2 の決定が前提を失っている**と判ったこと．
3. **O9 の修正に回帰テストの土台作りが含まれる**と確定したこと．

いずれも再検証がその段で捕まえた情報で，これがこの run の産出物に
あたる．なお **1 件を decided に動かせた**のは，backlog の残りが
環境ゲート (G1/G3) 中心である現状では素直な進捗である．
