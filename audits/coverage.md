...
| [2026-08-12 backlog auto-band-and-p4](2026-08-12-backlog-auto-band-and-p4.md) N5 | `rust/maou_shogi` + `rust/maou_usi` | **dfpn の TT 確保が探索の実費を桁違いに上回っていた問題．2026-08-12 に一連の PR で解消済み (#464 / #469 / #470 / #471)．残りは低優先の 1 件のみ．** 当初これを「`test_usi_go_mate_e2e` が flaky」とテスト側の問題として起票したのは**誤り**で，ユーザ指摘 (「1手詰に5秒はロジック上あり得ない．棋力にも影響する」) を受けた計測でエンジン側の欠陥と判明した．**機序**: TT 要素数は `clamp(nodes*2, 1<<18, 1<<23)` で，`Entry::null()` が全ゼロでないため `alloc_zeroed` に落ちず**確保時に全バイトを書く**．**消化した内容**: (#464) `go mate` が `max_nodes = u64::MAX` をそのまま TT サイズへ流していたのを `set_tt_nodes_hint` で停止条件と切り離し，`RootDfpnNodes` を USI に公開．(#469) TT プールを `isready` で前払いし，初手の時間超過を消した．(#470) 空 slot を全ゼロ表現 (`hand` のビット反転) にして確保を memset/`alloc_zeroed` へ．(#471) 「詰み証明済だが PV 復元不能」を `info string` で時間切れと区別できるようにした．**効果 (実測)**: `go mate` 初回 4.856s → **0.052s**，`go` 初手 (byoyomi 1000) 1.889〜3.487s (予算超過) → **0.502s = 定常時と同値**，peak RSS 745MB → 462MB．健全性は `test_29te` が canonical 396,516 ノード・同一 PV で通ること (探索が 1 ノードもずれない) と，`test_false_proof_hunt` を 2 シード (GAMES=120/80, `MATE1PLY_VERIFY=1`) で回して `FALSE_MATE(bug)` / `STRICT VERIFY None` ともに 0 件であることで確認した．**実施しないと決めた項目**: 既定 `RootDfpnNodes` を下げる案は**却下** (2026-08-12 判断)．当初の動機だった確保コストは #469/#470 で消え，残る対価は reach の縮小のみになったため．`search.rs:284-288` が 2M を「~41手級まで捕捉，41te は ~1.2M ノード」と根拠づけている．**残り 1 件 (低優先)**: 貸し出し時の `fill` が **O(size)** のまま残る (既定 352MB で 0.037s/探索)．**訂正**: #470 の「全ゼロ表現」で消えたのは**確保コストだけ**で，fill は 0.043s → 0.037s の 14% 改善どまりだった — 352MB ÷ 0.037s = 9.5GB/s と**既にメモリ帯域に張り付いており最適化の余地がない**．消すには世代スタンプで fill 自体を不要にするしかない (TT サイズを縮めれば比例して減るが，それは却下した候補2 と同じ対価を払う)．**優先度が低い理由**: dfpn は MCTS と別スレッドで並走するので実戦では wall clock に出ず (`go#1 = 0.502s` = 予算ぴったりで確認済み)，消費するのは CPU 時間であって持ち時間ではない．`go mate` では critical path に乗るが，詰み探索の予算は通常秒オーダーなので 37ms は誤差．プロファイルで実害が見えてから着手する． |
