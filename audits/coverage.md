...
- [2026-08-13 backlog truncated-feather-diagnostics](2026-08-13-backlog-truncated-feather-diagnostics.md)
  (**行の削除なし** — D15 を消化したが PR #493 が未マージ．D15 は
  "changed shape" で，記録の「size/footer 検査が要る」が誤りだったので
  [元記録](2026-08-10-src-maou-infra-file-system.md) に訂正を追記した．
  N6-2 行の前 run の記述も訂正済み)
...
| [2026-08-10 infra/file_system](2026-08-10-src-maou-infra-file-system.md) D15 | `src/maou/infra/file_system` | **`.feather` で終わる中途書き込みファイルを誰も守っていない．** step 2.5 key 6 の sweep で，`.gstmp`/`.tmp`/`.partial`/`.crc` については露出サイトがゼロと確認できた一方，**中断した `save_*_df` や in-place な `rsync`/`gsutil cp` が残す「拡張子が `.feather` のまま不完全なファイル」**はどのフィルタも通過する．`path_utils.py:10-13` が記述する `OSError: failed to fill whole buffer` がまさにこれ．`_is_temp_artifact` は末尾拡張子リストで判定するので**原理的に捕捉できず**，size/footer 検査が要る．**2026-08-13 の再検証で分かったこと**: 行数スキャン (`domain/data/arrow_format.scan_row_count`) は全ファイルの Arrow footer を読むので，途中書きファイルは**そこで既に落ちている** — 行が引く `OSError: failed to fill whole buffer` がまさにそれ．つまり「素通しして壊れたデータを読む」のではなく「原因を指さないメッセージで落ちる」が実態．したがって判断は「全ファイルに検査を足すか」ではなく，**「既に落ちているこの失敗を，どこでどう分かりやすくするか」**に寄せられる (前者より格段に安い)．**2026-08-13 に消化・PR 未マージ** (`/audit-backlog`, [記録](2026-08-13-backlog-truncated-feather-diagnostics.md), [PR #493](https://github.com/dousu/maou/pull/493))．実機で再現して上記の見立てを確認し (途中書き → `ComputeError: out-of-spec: InvalidFooter`，0 バイト → `OSError: failed to fill whole buffer`，**どちらもファイル名を含まない**)，`domain/data/arrow_format.feather_read_errors_name_the_file` が `add_note` でパス・サイズ・推定原因を書き足すようにした (型は変えないので `OSError` を捕まえている呼び出し側はそのまま動く)．**行の未決点だった G4 (「運用上のリスクとして実在するか」「検査を足すか」) は両方とも解消**した．元記録の「size/footer 検査が要る」は誤りだったので訂正を追記済み．**PR がマージされたらこの行を削除すること**．**この行の存在理由**: 元は record の § Cross-module sweep (worklist ではない節) にしか書いておらず，backlog 行がないため `/audit-backlog` から永久に不可視だった (2026-08-10 のユーザー指摘で発見)． |
