...
| [2026-08-13 backlog bundling-knobs-and-loss-aliasing](2026-08-13-backlog-bundling-knobs-and-loss-aliasing.md) N9 | `scripts/` | **`.npy` 時代の遺物が `scripts/` に残っている．** `scripts/debug_training.py:30` (`sorted(data_dir.glob("*.npy"))` + `:32` の「No .npy files found」エラー) と `scripts/verify_bce_training.py:23` (同じ glob) が `.npy` を入力として探すが，パイプラインは 2026-08-13 の N-2 で `.npy` 対応を全削除しており，`FileDataSource` は `.feather` 以外を `ValueError` で弾く (`file_data_source.py:204-206`)．同じ遺物だった `scripts/benchmark_file_datasource.py` は**この run でユーザ承認のうえ削除済み**だが，この 2 本は選択された item の外なので触っていない (**G2**: 黙って範囲を広げない)．**判断が要る点**: 削除するか `.feather` 版へ書き直すか — `benchmark_file_datasource.py` と同じ二択だが，こちらは訓練デバッグ用でベンチマークより使い道が残っている可能性がある．実行可能性は未確認 (glob が空振りする以外の欠陥があるかは見ていない)． |
