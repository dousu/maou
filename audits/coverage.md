...
| [2026-08-12 backlog auto-band-and-p4](2026-08-12-backlog-auto-band-and-p4.md) N5 | `rust/maou_shogi` + `rust/maou_usi` | **dfpn の置換表が探索の実費を桁違いに上回る固定費になっており，初回探索が時間予算を破る (棋力に直結)．** 当初これは「`test_usi_go_mate_e2e` が flaky」というテスト側の問題として起票したが，**誤りだった** — ユーザ指摘 (「1手詰に5秒はロジック上あり得ない」) を受けた計測でエンジン側の欠陥と判明した．**機序**: TT 要素数は `dfpn/search/mod.rs:560` で `clamp(max_nodes * 2, min_tt_entries, 1<<23)`．`go mate` は `maou_usi/src/backend.rs:277` で **`max_nodes = u64::MAX`** を渡す (「打ち切りは時間と stop で行う」ため) ので，**局面の難易度に関係なく常に上限 `1<<23` = 8,388,608 エントリ**になる．さらに `Entry::null()` が全ゼロでないため `vec![v; n]` が `alloc_zeroed` に落ちず (`dfpn/tt/mod.rs:65` が明記)，**確保時に全バイトを書く**．主 TT 8,388,608×64B = 537MB + 千日手表 ×24B = 201MB = **約 738MB**．**実測 (2026-08-12)**: 1手詰 `4k4/9/4P4/9/9/9/9/9/9 b G 1` を同一プロセスで反復 — 既定は **1回目 6.6〜23.1 秒 / 2回目以降 0.085 秒**，peak RSS **745MB**．`TTSIZE=65536` にすると **1回目 0.005 秒**，peak RSS 46MB で**同じ `G*5b` を返す** — **探索の実費は 1〜5ms** で，残り全部が確保と初回書き込み．**通常探索も同じ**: `go btime 60000 wtime 60000 byoyomi 1000` の**初回が 3.487 秒** (予算 1000ms の 3.5 倍)，2 回目以降 0.503 秒，peak RSS 555MB．`TTSIZE=65536` なら初回から 0.503 秒・164MB．**つまり実戦の初手で時間超過しうる**．**なぜ見逃されていたか**: `37129ba` のバッファプールが 2 回目以降を安くするので，**プロセスごとの初回にしか出ない**．USI の e2e テストは毎回新プロセスを起こすので必ず初回費用を払い，ページ確保が遅い回に予算超過して落ちる — flaky の正体はこれ．**修正の方向は複数あり判断が要る**: (a) `go mate` の node 予算を時間予算から導く (u64::MAX をやめる)，(b) `Entry::null()` を全ゼロ表現にして `alloc_zeroed` に載せる (遅延ゼロページ化．全探索に効くが entry 表現を触る)，(c) TT を遅延確保・漸進拡大にする，(d) `isready` 中にプールを温める (USI 的には正しい場所だが 738MB は減らない)．**併せて**: `maou_usi/src/protocol.rs:395` は `Timeout` と `Mate(空手順)` と `CheckmateNoPv` をすべて `checkmate timeout` に畳むため，時間切れと「解けたが手順を復元できない」が区別できず，今回の切り分けコストの一因になった． **既定値決定のための追加実測 (2026-08-12)．** TT サイズと (i) 初回確保 (ii) 2回目以降 (iii) 難問の求解 の関係を USI 越しに測った．**確保コストは約 7ms/MB の線形** (2^18=22MB:0.205s / 2^19=44MB:0.156s / 2^20=88MB:0.741s / 2^21=176MB:1.500s / 2^22=352MB:2.485s / 2^23=704MB:4.856s)．**重要**: プールは貸し出し時に初期値で `fill` するので **2回目以降も O(size) を払う** (2^23 で 0.091s / 2^18 で 0.003s) — 初回だけの問題ではなく**毎手の固定費**である．**強さの利得は測れなかった**: mate-29 (canonical，約40万ノード) は 2^18/2^20/2^22/2^23 の**すべてで同じ 29 手**を返し，所要は 2^18=1.504s / 2^20=1.565s / 2^22=3.610s / 2^23=4.856s で**小さいほど速い** (既定は 2^18 の約3倍遅い)．mate-39 (約1760万ノード) は 60 秒予算で 2^18 も既定 2^23 も **どちらも解けない** (既定は 69.97s と予算を10秒超過してもいる — 確保時間が予算に計上されていない)．**したがって既定を大幅に小さくするのは全測定点で改善**であり，大規模問題向けにノブを公開する方針 (ユーザ提案) が妥当．**ただし未公開**: `root_dfpn_nodes` は Python API (`maou_rust/src/maou_search.rs:300`) からは設定できるが，**USI の `setoption` には宣言もハンドラも無い** (`RootDfpnNodes` が存在しない) ので，対局時に増やす手段が今は無い — 方針を採るなら公開もセットで要る．**設計上の分岐点**: `go mate` に実ノード数を渡すと TT サイズだけでなく**探索の停止条件も**そのノード数になる (現在は時間と stop のみ)．(i) 停止条件も兼ねさせる (単純だが難問が早期に timeout する) か (ii) 停止は時間のまま TT サイズ用のヒントだけ分離する かで挙動が変わる． |
