...
| [2026-08-12 backlog auto-band-and-p4](2026-08-12-backlog-auto-band-and-p4.md) N5 | `tests/maou/infra/console` | **`test_usi_go_mate_e2e` が全件実行だと落ちる (flaky)．** `go mate 5000` で**1手詰** (`G*5b`) を解かせるテストが，`pytest` 全件実行の中では `checkmate timeout` を返して落ちる．単体実行 (`pytest tests/maou/infra/console/test_usi_cli.py`) と本ファイル単位では通る．2026-08-12 に確認した範囲: `main` の `6d8ee6b` (PR #457 マージ**前**) でも同様に落ちるので**特定の変更による回帰ではない**．同一コンテナ内でも，同じ commit が通る回と落ちる回がある (負荷依存)．ビルドプロファイルは `target/py-ext` で正しく (debug 混入ではない)，メモリも 14GB 空きで枯渇していない．**問題はテストの側**: 失敗が `checkmate timeout` という形で出るため，**ソルバの回帰と見分けがつかない**．1手詰に 5 秒の壁時計バジェットを課す設計そのものを見直すか (ノード数バジェットにする / 単体実行前提のマークを付ける)，少なくとも timeout 時に「探索が間に合わなかった」ことが明示されるようにすべき．同種のものに `tests/maou/domain/data/test_dataframe_io.py::TestHCPEDataFrameSerialization::test_serialization_performance` (`serialize_time < 0.01` という壁時計 assert，コメント自身が "may vary based on hardware" と認めている) がある． |
