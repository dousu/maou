...
| [2026-08-12 backlog auto-band-and-p4](2026-08-12-backlog-auto-band-and-p4.md) N5 | `tests/maou/infra/console` | **`test_usi_go_mate_e2e` の壁時計 5 秒バジェットが薄すぎて flaky．** `go mate 5000` で**1手詰** (`G*5b`) を解かせるテストが，`pytest` 全件実行の中で `checkmate timeout` を返して落ちることがある．**2026-08-12 に 20 回反復して実測**: 19 回は **約 0.6 秒**で正解，1 回だけ **7.47 秒**かかって予算超過で timeout (再現率 約 5%)．**`go mate 60000` に上げると 30/30 正解**し，そのうち 1 回は **5.70 秒**かかっていた — 5000ms なら落ちていた回である．つまりソルバは毎回正しく 解いており，**時々プロセスが数秒ストールする**のを 5 秒バジェットが拾って偽陽性にしている (典型 0.6 秒に対し外れ値が 3-7 秒．dfpn の置換表確保が疑わしいが未確認)．**CPU 負荷が原因ではない** — 4 並列の busy loop 下でも 0.93 秒で正解した (むしろ速い)．コマンド順序の競合でもない — reader スレッドは mpsc チャネルで送り `rx.recv()` が FIFO 処理するので `position` は `go mate` より必ず先に適用される (`rust/maou_usi/src/stdio.rs:90`)．**`go mate` の後に sleep を入れても直らない**: テストは `checkmate ` 行が出るまでブロックして読むので早すぎる読み取りは起きておらず，`timeout` はエンジン自身が予算超過後に出した応答．**修正案**: バジェットを上げる (アサーションは `G*5b` 完全一致のままなので検出力は落ちない)．**あわせて直すべき**: `protocol.rs:395` は`CheckmateResult::Timeout` と `Mate(空手順)`/`CheckmateNoPv` を**すべて** `checkmate timeout` にまとめるため，**時間切れと「詰みは解けたが手順を復元できない」が区別できない** — ソルバ回帰と偽陽性の切り分けに毎回コストがかかる．同種の壁時計 assert に `tests/maou/domain/data/test_dataframe_io.py::TestHCPEDataFrameSerialization::test_serialization_performance` (`serialize_time < 0.01`，コメント自身が "may vary based on hardware" と認めている) がある． |
