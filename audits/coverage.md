...
| Found by | Target | Item |
|---|---|---|
| [2026-08-10 infra/file_system](2026-08-10-src-maou-infra-file-system.md) D12 | `src/maou/infra/file_system` | **contained な低リスク整理 (step 2 由来，D3-D11 に含まれなかった分)．** (a) `file_data_source.py:42-49` — `_DF_TO_NUMPY_CONVERTERS` の4エントリ中3つが到達不能 (`_use_columnar` が preprocessing/stage1/stage2 を覆うので numpy 経路は `hcpe` のみ)．`:353`/`:376` の `is not None` ガードは常に真．**潜在ハザード**: `if/elif` に `else` がないため，将来どちらにも該当しない型が来ると `:395` の `lengths.append` だけ実行され `cum_lengths` と `_file_entries` がズレ，全インデックス参照が静かに壊れる．(b) `:321-422` — `try/except ImportError` を `try/except Exception` の内側に入れ子にし，内側が文言を変えて再 raise → 外側が即座に捕捉．traceback が二重化するだけ．(c) `:312-320,396,398-411` — `cumulative_rows` は `sum(lengths)` の二重計算，`milestone_interval` は7行3分岐で `logger.info` を間引くだけ．(d) `:423-426,249` — `total_pages`/`total_rows`/`file_paths` はいずれも `_file_entries` から導出可能な重複状態 ((a) の desync ハザードを増幅する)．(e) `streaming_file_source.py:81-93` — 2つ目の検証 `array_type == "hcpe" and array_type not in _COLUMNAR_CONVERTERS` は右辺が恒真 (`_COLUMNAR_CONVERTERS` に `hcpe` キーがない)．実質「`_COLUMNAR_CONVERTERS` に含まれること」の1条件で足りる．**注意**: `tests/.../test_streaming_file_source.py` の `test_init_hcpe_rejected` がメッセージを検査している可能性があるので要確認．(f) `streaming_file_source.py:199` — `log_level` 変数はループ内で毎回同じ値を代入され，3つ目の呼び出し (`:219`) は `logger.debug` を直接使っていて一貫していない． |
| [2026-08-10 infra/file_system](2026-08-10-src-maou-infra-file-system.md) D13 | `src/maou/infra/file_system` | **ホットパスと重複ディスパッチ (step 2 由来，未 row 化分)．** (a) `file_data_source.py:837-841`→`:572-610` — `__getitem__` は**訓練サンプル1件ごと**に `self.cum_lengths[1:]` を新規確保し，スカラー `np.searchsorted` を呼び，`np.empty(1, dtype)` (preprocessing で 3,089B) を確保して6フィールドを memcpy する．batch 1024 で約3MB の memcpy + 1024回の小確保/バッチ．`dataset.py:230` の `torch.from_numpy` は「ゼロコピー」と称しているが，コピーは1層下で既に起きている．contained な部分的改善: `cum_lengths[1:]` を `__init__` へ巻き上げ，スカラー探索を `bisect.bisect_right` に．根本解決 (`KifDataset` が `ColumnarBatch` を直接スライスする) は `app/learning/dataset.py` と ABC を触るので path 外．(b) `:677-690`/`:846-857` — `get_items` は「バッチで取得」と謳いながら `get_item` を要素ごとに Python ループで呼ぶだけで何もバッチ化していない．`ColumnarBatch.slice` によるベクトル化が可能．(c) `:59` — `_STRUCTURED_DTYPES` は `interface/data_schema.get_dtype` の dispatch を再実装している (同モジュールを `:25` で既に import 済み)．テーブルと3つの import を落とせば `_structured_dtype: np.dtype \| None` の `assert ... is not None` (`:627-628`, `:747-748`) も不要になる． |
